...
func newConsulPlugin() *Consul {
	ctxt = context.TODO()

	address := "http://localhost:8500"
	token := ""
	client, _ := newConsulClient(address, token)
...
func set(t *testing.T, c *Consul, k string, ttl time.Duration, m *msg.Service) {
	b, err := json.Marshal(m)
	if err != nil {
		t.Fatal(err)
	}
	path, _ := msg.PathWithWildcard(k, c.PathPrefix)
	kv := &api.KVPair{
		Key:   strings.Replace(path, "/", "", 1),
		Value: b,
	}
	c.Client.KV().Put(kv, nil)
}

func delete(t *testing.T, c *Consul, k string) {
	path, _ := msg.PathWithWildcard(k, c.PathPrefix)
	c.Client.KV().Delete(strings.Replace(path, "/", "", 1), nil)
}

func TestLookup(t *testing.T) {
	consul := newConsulPlugin()
	for _, serv := range services {
		set(t, consul, serv.Key, 0, serv)
