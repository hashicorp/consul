...
type ProxyConfig struct {
	...
	// SuppressEnvoyHeaders controls whether Envoy rewrites the "server" response
	// header to "envoy" on all HTTP responses. When set to true, the
	// HttpConnectionManager ServerHeaderTransformation is set to PASS_THROUGH,
	// which prevents Envoy from overwriting the upstream's server header value.
	// This is the only effective mechanism: route-level response_headers_to_remove
	// and Lua envoy_on_response both run before the codec writes this header.
	// Applies to API Gateway HTTP listeners only. Defaults to false.
	//
	// Takes precedence over EnvoyServerHeaderName: if both are set, the header
	// is suppressed entirely rather than renamed.
	SuppressEnvoyHeaders bool `mapstructure:"envoy_suppress_envoy_headers"`

	// EnvoyServerHeaderName sets a custom value for the "server" response header
	// on all HTTP responses from the API Gateway. When set, the
	// HttpConnectionManager server_name is overwritten with this value and
	// ServerHeaderTransformation is set to OVERWRITE, so every response (whether
	// generated by Envoy itself or proxied from an upstream) will carry
	// "server: <value>". Ignored when SuppressEnvoyHeaders is true.
	// Applies to API Gateway HTTP listeners only. Defaults to "" (no override).
	EnvoyServerHeaderName string `mapstructure:"envoy_server_header_name"`
}
