...
func TestInferenceGatewayConfigEntry_CE_Validate(t *testing.T) {
	e := &InferenceGatewayConfigEntry{
		Name:      "gw",
		Processor: InferenceGatewayProcessor{FailureMode: InferenceGatewayFailureModeOpen},
	}
...
// runs before rejecting it: MakeConfigEntry, the kind, the failure-mode default, the
// case-folding of enum-valued fields, and the hash.
func TestInferenceGatewayConfigEntry_CE_Normalize(t *testing.T) {
	...
	e.PII = &InferenceGatewayPII{
		Scope:         "Request",
		DefaultAction: "MASK",
		Detectors:     []InferenceGatewayPIIDetector{{Name: "ssn", Action: "BLOCK"}},
	}
	require.NoError(t, e.Normalize())
...
// TestInferenceGatewayConfigEntry_CE_DecodeAndJSONRoundTrip verifies every field decodes
// and survives the JSON the HTTP API returns, so CE and ENT expose the same shape to API
// consumers.
