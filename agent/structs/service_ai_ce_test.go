...
func TestStructs_ServiceAI_Validate(t *testing.T) {
	cases := []struct {
		name    string
		ai      *ServiceAI
		wantErr bool
		// errCount, when > 0, asserts the number of aggregated errors.
		errCount int
	}{
		{
			name: "nil is valid",
		...
		{
			name: "valid internal inference-model",
			ai: &ServiceAI{
		...
		{
			name: "valid managed inference-model with vault auth",
			ai: &ServiceAI{
				Role: ServiceAIRoleInferenceModel,
				InferenceModel: &AIInferenceModel{
					Protocol: "openai",
					...
					Auth: &AIAuth{
						...
						Secret: &AISecret{Provider: "vault", Path: "secret/data/camp/openai", Field: "api_key"},
					},
				},
			},
		},
		...
		},
		{
			...
			ai: &ServiceAI{
				...
				Agent: &AIAgent{
					Inference: &AIAgentInference{Specialization: []string{"code-review", "coding"}},
					MCP: &AIAgentMCP{
						Port: 15101,
		...
		{
			name:    "unknown role",
			ai:      &ServiceAI{Role: "bogus"},
			wantErr: true,
		},
		{
			name: "role mismatch: mcp-server with inference_model block",
			ai: &ServiceAI{
				Role:           ServiceAIRoleMCPServer,
				InferenceModel: &AIInferenceModel{Protocol: "openai"},
			},
			wantErr: true,
		},
		{
			name: "two sub-blocks set",
			ai: &ServiceAI{
				Role:           ServiceAIRoleInferenceModel,
				InferenceModel: &AIInferenceModel{Protocol: "openai"},
				MCPServer:      &AIMCPServer{Transport: "sse", ProtocolVersion: "x"},
			},
			wantErr: true,
		},
		{
			...
			ai: &ServiceAI{
				Role:           ServiceAIRoleInferenceModel,
				InferenceModel: &AIInferenceModel{Protocol: "foo"},
			},
			wantErr: true,
		},
		{
			...
			ai: &ServiceAI{
				Role:      ServiceAIRoleMCPServer,
				MCPServer: &AIMCPServer{Transport: "grpc", ProtocolVersion: "2025-03-26"},
			},
			wantErr: true,
		},
		{
			name: "mcp-server missing protocol_version",
			ai: &ServiceAI{
				Role:      ServiceAIRoleMCPServer,
				MCPServer: &AIMCPServer{Transport: "streamable-http"},
			},
			wantErr: true,
		},
		{
			name: "negative mcp port",
			ai: &ServiceAI{
				Role:  ServiceAIRoleAgent,
				Agent: &AIAgent{MCP: &AIAgentMCP{Port: -1}},
			},
			wantErr: true,
		},
		{
			name: "bad approval_timeout",
			ai: &ServiceAI{
				Role: ServiceAIRoleAgent,
				Agent: &AIAgent{MCP: &AIAgentMCP{
					HITL: &AIAgentMCPHITL{Port: 16101, ApprovalTimeout: "notaduration"},
				}},
			},
			wantErr: true,
		},
		{
			...
			ai: &ServiceAI{
				Role:  ServiceAIRoleAgent,
				Agent: &AIAgent{RateLimits: &AIAgentRateLimits{ToolCallsPerMinute: -1}},
			},
			wantErr: true,
		},
		{
			name: "auth wrong type",
			ai: &ServiceAI{
				Role: ServiceAIRoleInferenceModel,
				InferenceModel: &AIInferenceModel{
					Protocol: "openai",
		...
		},
		{
			...
			ai: &ServiceAI{
				...
				InferenceModel: &AIInferenceModel{
					...
					Auth: &AIAuth{
						Type:   "bearer",
						Secret: &AISecret{Provider: "aws-sm", Path: "p", Field: "f"},
					},
				},
			},
			wantErr: true,
		...
		{
			name: "auth empty path and field reports multiple errors",
			ai: &ServiceAI{
				Role: ServiceAIRoleInferenceModel,
				InferenceModel: &AIInferenceModel{
					Protocol: "openai",
					Auth: &AIAuth{
						Type:   "bearer",
						Secret: &AISecret{Provider: "vault"},
					},
				},
			},
			wantErr:  true,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				require.Error(t, err)
				if tc.errCount > 0 {
					require.Len(t, multierrorWrapped(err), tc.errCount)
				}
			} else {
