...
// This file contains a few helpers needed by structgen generated code to covert
// between protobuf and structs package types.

// ToStructs implements the equivalent that structgen creates for mirrored
// struct definitions in structs package. We manually code this since
// HeaderValue is really jut a protobuf quirk and maps to a plain slice in the
// structs package so the generated methods don't work.)
func (v *HeaderValue) ToStructs() ([]string, error) {
	return v.Value, nil
}

// FromStructs implements the equivalent that structgen creates for mirrored
// struct definitions in structs package. We manually code this since
// HeaderValue is really jut a protobuf quirk and maps to a plain slice in the
// structs package so the generated methods don't work.
func (v *HeaderValue) FromStructs(s []string) error {
	v.Value = s
	return nil
}

// MapFromPBStruct converts a protobuf Struct into a map[string]interface{}
...
// MapToPBStruct converts a map[string]interface{} into a proto.Struct
...
// internally by MapToPBStruct when it encouters slices.
