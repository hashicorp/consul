...
// setFilteredByACLs sets an HTTP response header to indicate that the query
// results were filtered by enforcing ACLs. If the given filtered value is
// false the header will be omitted, as its ambiguous whether the results were
// not filterer or whether the endpoint doesn't yet support this header.
func setFilteredByACLs(resp http.ResponseWriter, filtered bool) {
	if filtered {
		resp.Header().Set("X-Consul-FilteredByACLs", "true")
	}
}
