...
// TODO: docs
func (m *Internal) ReachabilityProbe(
	args *structs.ReachabilityRequest,
	reply *structs.ReachabilityResponses) error {
	if done, err := m.srv.ForwardRPC("Internal.ReachabilityProbe", args, args, reply); done {
		return err
	}
	...
	if rule != nil && rule.OperatorRead(nil) != acl.Allow {
		accessorID := m.aclAccessorID(args.Token)
		m.logger.Warn("reachability probe blocked by ACLs", "accessorID", accessorID)
		return acl.ErrPermissionDenied
	}
	...
	var out []*structs.ReachabilityResponse
	if args.WAN {
		resp, err := m.checkSegmentReachability(m.srv.serfWAN)
		if err != nil {
			return err
		}
		...
		case "", api.AllSegments:
			var errs error
			for name, segment := range segments {
				resp, err := m.checkSegmentReachability(segment)
				if err != nil {
					if name == "" {
						name = "<default>"
					}
				...
				resp.Datacenter = m.srv.config.Datacenter
				out = append(out, resp)
			}
			if errs != nil {
				return errs
		...
		default:
			segment, ok := segments[args.Segment]
			if !ok {
				return fmt.Errorf("unknown segment %q", args.Segment)
			}
...
func (m *Internal) checkSegmentReachability(segment *serf.Serf) (*structs.ReachabilityResponse, error) {
	// members []serf.Member,
	...
	reply := &structs.ReachabilityResponse{
	...
	// TODO: if we switch from operator:read this should do Agent.filterMembers logic
	// TODO: but that is likely horrible for a diagnostic perspective
	...
	for !resp.Finished() {
		...
		reply.Acks = append(reply.Acks, a)
		numAcks++
	...
	reply.QueryTime = time.Now().Sub(start)
	reply.TimeToLastResponse = last.Sub(start)

	// // Print troubleshooting info for duplicate responses
	// if dups {
	// 	// c.Ui.Output(reach_duplicateResponses)
	// 	exit = 1
	// }
	...
	// 		}
	// 	}
	// 	// c.Ui.Output(reach_tooManyAcks)
	// 	// c.Ui.Output(reach_troubleshooting)
	// 	exit = 1
	...
	// 		}
	// 	}
	// 	// c.Ui.Output(reach_tooFewAcks)
	// 	// c.Ui.Output(reach_troubleshooting)
	// 	exit = 1
	// }
	// if exit != 0 {
	// 	return nil, fmt.Errorf("exit code %d", exit)
	// }
	return reply, nil
}

const (
	reach_tooManyAcks        = `This could mean Serf is detecting false-failures due to a misconfiguration or network issue.`
	reach_tooFewAcks         = `This could mean Serf gossip packets are being lost due to a misconfiguration or network issue.`
	reach_duplicateResponses = `Duplicate responses means there is a misconfiguration. Verify that node names are unique.`
	reach_troubleshooting    = `
