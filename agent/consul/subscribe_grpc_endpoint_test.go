...
func TestStreaming_Subscribe(t *testing.T) {
	...
	streamClient := agentpb.NewConsulClient(conn)
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	streamHandle, err := streamClient.Subscribe(ctx, &agentpb.SubscribeRequest{
		Topic: agentpb.Topic_ServiceHealth,
		Key:   "redis",
	})
	require.NoError(err)

	// Start a goroutine to read updates off the agentpb.
	eventCh := make(chan *agentpb.Event, 0)
	go testSendEvents(t, eventCh, streamHandle)

	var snapshotEvents []*agentpb.Event
	for i := 0; i < 3; i++ {
		select {
		case event := <-eventCh:
	...
	expected := []*agentpb.Event{
				...
				ServiceHealth: &agentpb.ServiceHealthUpdate{
					...
					CheckServiceNode: &agentpb.CheckServiceNode{
						...
						Service: &agentpb.NodeService{
							...
							Address: "3.4.5.6",
							Port:    8080,
							Weights: &agentpb.Weights{Passing: 1, Warning: 1},
							// Sad empty state
		...
		},
		{
			Topic:   agentpb.Topic_ServiceHealth,
			Key:     "redis",
			Payload: &agentpb.Event_EndOfSnapshot{EndOfSnapshot: true},
		},
	}
				...
				ServiceHealth: &agentpb.ServiceHealthUpdate{
					...
					CheckServiceNode: &agentpb.CheckServiceNode{
						Node: &agentpb.Node{
							...
							Datacenter: "dc1",
							Address:    "1.2.3.4",
							RaftIndex:  agentpb.RaftIndex{CreateIndex: 13, ModifyIndex: 13},
						},
						Service: &agentpb.NodeService{
							ID:        "redis1",
							Service:   "redis",
						...
						Checks: []*agentpb.HealthCheck{
							{
								...
								ServiceID:      "redis1",
								ServiceName:    "redis",
								RaftIndex:      agentpb.RaftIndex{CreateIndex: 14, ModifyIndex: 14},
								EnterpriseMeta: &agentpb.EnterpriseMeta{},
							},
						},
					},
...
func TestStreaming_Subscribe_SkipSnapshot(t *testing.T) {
	...
	var index uint64
	{
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		defer cancel()
		streamHandle, err := streamClient.Subscribe(ctx, &agentpb.SubscribeRequest{Topic: agentpb.Topic_ServiceHealth, Key: "redis"})
		require.NoError(err)
...
func TestStreaming_Subscribe_FilterACL(t *testing.T) {
	...
	// Start a Subscribe call to our streaming endpoint for the service we have access to.
	{
		...
		// Read events off the agentpb. We should not see any events for the filtered node.
		var snapshotEvents []*agentpb.Event
		for i := 0; i < 2; i++ {
			select {
			case event := <-eventCh:
...
// testSendEvents receives agentpb.Events from a given handle and sends them to the provided
// channel. This is meant to be run in a separate goroutine from the main test.
func testSendEvents(t *testing.T, ch chan *agentpb.Event, handle agentpb.Consul_SubscribeClient) {
	for {
...
func TestStreaming_TLSReload(t *testing.T) {
	...
	require.NoError(err)
	{
		...
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		defer cancel()
		_, err = streamClient.Subscribe(ctx, &agentpb.SubscribeRequest{Topic: agentpb.Topic_ServiceHealth, Key: "redis"})
...
func verifyMonotonicStreamUpdates(ctx context.Context, logger testLogger, client agentpb.ConsulClient, wg *sync.WaitGroup, i int, updateCount *uint64, errCh chan<- error) {
	defer wg.Done()
	streamHandle, err := client.Subscribe(ctx, &agentpb.SubscribeRequest{Topic: agentpb.Topic_ServiceHealth, Key: "redis"})
	if err != nil {
		if strings.Contains(err.Error(), "context deadline exceeded") ||
			strings.Contains(err.Error(), "context canceled") {
...
func svcOrErr(event *agentpb.Event) (*agentpb.NodeService, error) {
