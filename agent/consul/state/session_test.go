...
func TestStateStore_SessionCreate_SessionGet(t *testing.T) {
	...
	// Check mappings were inserted
	{
		...
		check, err := tx.First(tableSessionChecks, indexSession, sess.ID)
		if err != nil {
			t.Fatalf("err: %s", err)
		}
		if check == nil {
	...
	checks, err := tx.Get(tableSessionChecks, indexSession, sess2.ID)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
...
func TestStateStore_Session_Snapshot_Restore(t *testing.T) {
	...
	func() {
		...
		check, err := tx.First(tableSessionChecks, indexSession, session1)
		if err != nil {
			t.Fatalf("err: %s", err)
		}
		if check == nil {
...
func TestStateStore_Session_Invalidate_DeleteCheck(t *testing.T) {
	...
	tx := s.db.Txn(false)
	mapping, err := tx.First(tableSessionChecks, indexSession, session.ID)
	if err != nil {
		t.Fatalf("err: %s", err)
	}
	if mapping != nil {
