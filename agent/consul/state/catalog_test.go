...
func TestStateStore_ServiceNodes(t *testing.T) {
	...
	if !lib.StrContains(nodes[0].ServiceTags, "replica") {
		t.Fatalf("bad: %v", nodes)
	}
	...
	if !lib.StrContains(nodes[1].ServiceTags, "replica") {
		t.Fatalf("bad: %v", nodes)
	}
	...
	if !lib.StrContains(nodes[2].ServiceTags, "primary") {
		t.Fatalf("bad: %v", nodes)
	}
...
func TestStateStore_ServiceTagNodes(t *testing.T) {
	...
	if !lib.StrContains(nodes[0].ServiceTags, "primary") {
		t.Fatalf("bad: %v", nodes)
	}
	if nodes[0].ServicePort != 8000 {
		t.Fatalf("bad: %v", nodes)
