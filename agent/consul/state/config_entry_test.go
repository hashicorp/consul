...
func TestStore_ExportingServices(t *testing.T) {
	s := NewStateStore(nil)
	var c indexCounter
	...
	run := func(t *testing.T, tc testCase) {
		ws := memdb.NewWatchSet()
		defaultMeta := structs.DefaultEnterpriseMetaInDefaultPartition()
		idx, services, err := s.ExportedServices(ws, defaultMeta)
		require.NoError(t, err)
	...
	t.Run("wild card includes all services", func(t *testing.T) {
		...
		tc := testCase{
			expect: []structs.SimplifiedExportedService{
				...
				{
					Service:       structs.NewServiceName("backend", nil),
					ConsumerPeers: []string{"west"},
				},
			},
			idx: c.Last(),
	...
	t.Run("deleting the config entry clears the services", func(t *testing.T) {
		...
		idx, result, err := s.ExportedServices(nil, defaultMeta)
		require.NoError(t, err)
