...
func (s *EventSnapshot) doSnapshot() {
	...
	s.snapBuffer.Append([]Event{Event{
		Topic:   s.Request.Topic,
		Key:     s.Request.Key,
		Index:   idx,
		Payload: &Event_EndOfSnapshot{true},
	}})
	...
	for {
		// Find the next item that we should include. This may block for the next
		// item as noted above. For now it blocks indefinitely but this is just like having another subscriber
