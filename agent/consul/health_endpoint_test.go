...
func TestHealth_GatewayServiceNodes_Ingress(t *testing.T) {
	t.Parallel()
	dir1, s1 := testServer(t)
	defer os.RemoveAll(dir1)
	defer s1.Shutdown()
	...
	require.Equal(t, nodes[0].Checks[0].Status, api.HealthPassing)
	require.Equal(t, nodes[1].Node.Node, "bar")
	require.Equal(t, nodes[1].Service.Service, "db")
	require.Equal(t, nodes[1].Checks[0].Status, api.HealthWarning)
}

func TestHealth_GatewayServiceNodes_Ingress_ACL(t *testing.T) {
	t.Parallel()
	dir1, s1 := testServerWithConfig(t, func(c *Config) {
		c.ACLDatacenter = "dc1"
		c.ACLsEnabled = true
	...
	err = msgpackrpc.CallWithCodec(codec, "Health.GatewayServiceNodes", &req, &out2)
...
func TestHealth_GatewayServiceNodes_Terminating(t *testing.T) {
	t.Parallel()
	dir1, s1 := testServer(t)
	defer os.RemoveAll(dir1)
	defer s1.Shutdown()
...
func TestHealth_GatewayServiceNodes_Terminating_ACL(t *testing.T) {
	t.Parallel()
	dir1, s1 := testServerWithConfig(t, func(c *Config) {
		c.ACLDatacenter = "dc1"
		c.ACLsEnabled = true
