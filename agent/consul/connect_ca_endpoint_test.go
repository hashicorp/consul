...
func TestConnectCAConfig_GetSet_ACLDeny(t *testing.T) {
	...
	opReadToken, err := upsertTestTokenWithPolicyRules(
		codec, "root", "dc1", `operator = "read"`)
	require.NoError(t, err)

	opWriteToken, err := upsertTestTokenWithPolicyRules(
		codec, "root", "dc1", `operator = "write"`)
	require.NoError(t, err)
...
-----END CERTIFICATE-----`,
	...
	t.Run("allow get with operator:write", func(t *testing.T) {
		...
		assert.Equal(t, []uint8(newConfig.Config["RootCert"].(string)), reply.Config["RootCert"])
		assert.Equal(t, []uint8(newConfig.Config["PrivateKey"].(string)), reply.Config["PrivateKey"])
	})
}
