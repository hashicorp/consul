...
func (c *Cluster) updateTLSCertificatesInConfig(opts AutoConfigOptions, conf *config.Config) error {
	if c.srv.config.ConnectEnabled && c.srv.config.AutoEncryptAllowTLS {
...
func (c *Cluster) updateGossipEncryptionInConfig(_ AutoConfigOptions, conf *config.Config) error {
	...
	if lanKeyring := memberlistConfig.Keyring; lanKeyring != nil {
		...
		if len(pk) > 0 {
			conf.EncryptKey = base64.StdEncoding.EncodeToString(pk)
		}

		conf.EncryptVerifyIncoming = memberlistConfig.GossipVerifyIncoming
		conf.EncryptVerifyOutgoing = memberlistConfig.GossipVerifyOutgoing
	}
...
func (c *Cluster) updateTLSSettingsInConfig(_ AutoConfigOptions, conf *config.Config) error {
	...
	conf.VerifyServerHostname = c.srv.tlsConfigurator.VerifyServerHostname()
	base := c.srv.tlsConfigurator.Base()
	conf.VerifyOutgoing = base.VerifyOutgoing
	conf.TLSMinVersion = base.TLSMinVersion
	conf.TLSPreferServerCipherSuites = base.PreferServerCipherSuites

	var err error
	conf.TLSCipherSuites, err = tlsutil.CipherString(base.CipherSuites)
	return err
}
