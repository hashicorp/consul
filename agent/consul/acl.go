...
// for an ACL.
func (f *aclFilter) allowCheckServiceNode(node, service string, ent *acl.EnterpriseAuthorizerContext) bool {
	return f.allowNode(node, ent) && f.allowService(service, ent)
}
...
func (f *aclFilter) filterCheckServiceNodes(nodes *structs.CheckServiceNodes) {
	...
	for i := 0; i < len(csn); i++ {
		node := csn[i]
		// TODO (namespaces) update to call with an actual ent authz context once the catalog supports it
		if f.allowCheckServiceNode(node.Node.Node, node.Service.Service, nil) {
			continue
		}
