...
func (b *Builder) Build() (rt RuntimeConfig, err error) {
	...
	rt = RuntimeConfig{
		...
		DisablePrimaryGatewayFallback: b.boolVal(c.DisablePrimaryGatewayFallback),
...
func (b *Builder) Validate(rt RuntimeConfig) error {
	...
	if len(rt.PrimaryGateways) == 0 && rt.DisablePrimaryGatewayFallback {
		return fmt.Errorf("'disable_primary_gateway_fallback' should only be configured when 'primary_gateways' is set")
	}
