...
func v1EntMetaToV2Tenancy(reg *resource.Registration, entMeta *acl.EnterpriseMeta, tenancy *pbresource.Tenancy) {
	if (reg.Scope == pbresource.Scope_SCOPE_NAMESPACE || reg.Scope == pbresource.Scope_SCOPE_PARTITION) && tenancy.Partition == "" {
		tenancy.Partition = entMeta.PartitionOrDefault()
	}

	if reg.Scope == pbresource.Scope_SCOPE_NAMESPACE && tenancy.Namespace == "" {
