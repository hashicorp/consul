...
type MaterializedViewState interface {
	...
	// are received.
	Update(events []*stream.Event) error
	...
	// servers. The index the materialized view represents is maintained separtely
	// and passed in in case the return type needs an Index field populating. This
	// allows implementations to not worry about maintaining indexes seen during
	// Update.
...
// memory.
func MaterializedViewFromFetch(t StreamingCacheType, opts cache.FetchOptions, subReq stream.SubscribeRequest) *MaterializedView {
	...
	for {
		...
		if err != nil {
			...
			v.logger.Printf("[ERR] cache.streaming: subscribe call failed: %s"+
				" topic=%s, key=%s, failures=%d, backoffWait=%s", err, v.req.Topic,
				v.req.Key, failures, wait)
...
// backOffWait returns exponentially increasing durations after consecutive
// failures. The base waits are 0s 1s 2s 4s 8s...SubscribeBackoffMax.
//
// Additional jitter up to 2x the base is added.
func backOffWait(failures int) time.Duration {
	if failures == 1 {
	...
	waitTime := SubscribeBackoffMax
	if shift < 31 {
		waitTime = (1 << shift) * time.Second
	}
	if waitTime > SubscribeBackoffMax {
		waitTime = SubscribeBackoffMax
	}
...
func (v *MaterializedView) runSubscription() error {
	...
	s, err := v.client.Subscribe(ctx, &v.req)
	if err != nil {
		return err
	}

	snapshotEvents := make([]*stream.Event, 0)
	...
	for {
		...
		if event.GetReloadStream() {
			// Server has requested we reload the view and start with a fresh snapshot
		...
		if event.GetResumeStream() {
			...
			if !snapshotDone {
				// We've somehow got into a bad state here server things we have an
				// up-to-date snapshot but we don't think we do. Reset and start over.
				v.reset()
