...
// is never published — at best a stall until the bootstrap-gate timeout. Three
// exclusions matter:
//
//   - Listeners with a Conflicted status (ListenerIsReady is false).
//   - Listeners the controller has not bound yet (no entry in BoundListeners).
...
func (c *configSnapshotAPIGateway) activeUpstreamIDs() map[UpstreamID]struct{} {
	...
	for name, listener := range c.Listeners {
		// Only include upstreams for listeners that are ready, mirroring
		// getReadyListeners.
		if !c.GatewayConfig.ListenerIsReady(name) {
			continue
		}
		// A listener the controller has not bound yet produces no cluster;
		// getReadyListeners skips it too, so gating on it would only stall.
...
func (c *configSnapshotAPIGateway) synthesizeChains(datacenter string, listener structs.APIGatewayListener, boundListener structs.BoundAPIGatewayListener) ([]structs.IngressService, structs.Upstreams, []*structs.CompiledDiscoveryChain, []error, error) {
