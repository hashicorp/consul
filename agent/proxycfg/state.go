...
func reconcilePeeringWatches(
	...
	peerUpstreamEndpoints.ForEachKey(func(uid UpstreamID) bool {
		logger.Info("%%teradata%%connect_proxy handleUpdate looping through PeerUpstreamEndpoints:", "uid", uid.String())
		...
		logger.Info("%%teradata%%connect_proxy handleUpdate Cancel watch for PeerUpstreamEndpoints:", "uid", uid.String())
	...
	upstreamPeerTrustBundles.ForEachKey(func(peerName PeerName) bool {
		if _, ok := validPeerNames[peerName]; !ok {
			logger.Info("%%teradata%%connect_proxy handleUpdate Cancel watch for UpstreamPeerTrustBundles:", "peerName", peerName)
