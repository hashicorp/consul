...
const (
	ConnectCAService_WatchRoots_FullMethodName = "/hashicorp.consul.connectca.ConnectCAService/WatchRoots"
	ConnectCAService_Sign_FullMethodName       = "/hashicorp.consul.connectca.ConnectCAService/Sign"
)
...
func (c *connectCAServiceClient) WatchRoots(ctx context.Context, in *WatchRootsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[WatchRootsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ConnectCAService_ServiceDesc.Streams[0], ConnectCAService_WatchRoots_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type ConnectCAService_WatchRootsClient = grpc.ServerStreamingClient[WatchRootsResponse]

func (c *connectCAServiceClient) Sign(ctx context.Context, in *SignRequest, opts ...grpc.CallOption) (*SignResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(SignResponse)
	err := c.cc.Invoke(ctx, ConnectCAService_Sign_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// UnimplementedConnectCAServiceServer should be embedded to have
...
func (UnimplementedConnectCAServiceServer) WatchRoots(*WatchRootsRequest, grpc.ServerStreamingServer[WatchRootsResponse]) error {
...
func (UnimplementedConnectCAServiceServer) testEmbeddedByValue() {}
...
func RegisterConnectCAServiceServer(s grpc.ServiceRegistrar, srv ConnectCAServiceServer) {
	// If the following call pancis, it indicates UnimplementedConnectCAServiceServer was
...
func _ConnectCAService_WatchRoots_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ConnectCAServiceServer).WatchRoots(m, &grpc.GenericServerStream[WatchRootsRequest, WatchRootsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ConnectCAService_WatchRootsServer = grpc.ServerStreamingServer[WatchRootsResponse]
...
func _ConnectCAService_Sign_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: ConnectCAService_Sign_FullMethodName,
	}
