...
To update Connect CA provider on an existing cluster or to update any properties, such as tokens, of the CA provider,
please use the [Update CA Configuration Endpoint](/api/connect/ca#update-ca-configuration).

Consul has support for different CA providers to be used with the Consul Service Mesh.
...
1. Reference the Kubernetes secret in the `server.extraVolumes` value in the Helm chart.

Below we will go over this process for configuring Vault as the Connect CA.
However, other providers can be configured similarly, aside from providing the appropriate `ca_config`
...
To configure a Vault as a CA provider for Consul Connect,
first, create provider a configuration JSON file.
...
Finally, install the Helm chart with these values:
...
To rotate the Vault token in your CA provider configuration, you need to update the configuration file
and call the [Update CA Configuration Endpoint](/api/connect/ca#update-ca-configuration).
