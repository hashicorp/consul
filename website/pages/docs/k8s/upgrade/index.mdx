...
Consul on Kubernetes will need to be upgraded/updated if your Helm configuration
is changed, if a new Helm chart is released or if you wish to upgrade your Consul version.

### Helm Configuration Changes

If you make a change to your Helm values file, you will need to run `helm upgrade`
for those changes to take effect.

For example, if you've installed Consul with
...
In this example, I'm running version `0.24.0` (from `consul-0.24.0`).

1. Run `helm upgrade consul hashicorp/consul --version 0.24.0 -f /path/to/my/values.yaml`.

   **Before upgrading, be sure you've read the other sections on this page.**
...
1. You can then list all available versions:
...
1. To determine which version you have installed, run:
...
Here we can see we're running version `0.24.0` (from `consul-0.24.0`) and so
we may want to upgrade to the latest `0.24.1` version.

1. Read the changelog for that version and any versions in between: https://github.com/hashicorp/consul-helm/blob/master/CHANGELOG.md.

1. Upgrade by running `helm upgrade` with the `--version` flag:
...
If a new version of Consul is released, you will need to run a Helm upgrade
...
There is no built-in functionality to Helm that shows what a helm upgrade will
...
1. Take the same `helm upgrade` command you were planning to execute but run `helm diff upgrade` instead of `helm upgrade`:
...
Consul clients run as a daemonset across all Kubernernetes nodes. During an upgrade,
...
sidecar container that runs alongside each service.
...
node running that replica, it will be briefly deregistered from the mesh. Since
...
are deleted one by one or in batches. See below for details.
...
```bash
helm upgrade consul hashicorp/consul --version <your-version> -f /path/to/my/values.yaml
```
...
and running `helm upgrade` again. This will cause the first Consul server
to be stopped and restarted with the new image.

Wait until the Consul server cluster is healthy again (30s to a few minutes).
This can be confirmed by running `consul members` on one of the previous servers
and ensuring that all three servers are listed and are `alive`.
...
1. Manually run `kubectl delete pod <id>` for each consul daemonset pod
...
3. Cordon and drain each node to ensure there are no connect pods running on it and then delete the
   consul client pod on that node.

If you are using only the Service Sync functionality, you can run an upgrade without
being as careful since that component is more resilient to brief restarts of
Consul clients.
