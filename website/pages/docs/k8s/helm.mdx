...
- <a name="v-synccatalog" href="#v-synccatalog">
  ...
  - <a name="v-synccatalog-consulPrefix" href="#v-synccatalog-consulPrefix">
    ...
    names within Kubernetes remain unchanged. (Kubernetes -> Consul sync only)
...
- <a name="v-connectinject" href="#v-connectinject">
  ...
  - <a
    ...
    </a> (`string: ""`) - If not using `global.bootstrapACLs` and instead manually
    setting up an auth method for Connect inject, set this to the name of your Auth
    method.
  ...
  - <a
    ...
    and you are not setting `global.bootstrapACLs` to `true`. This token needs to
    have `operator = "write"` privileges so that it can create namespaces.
