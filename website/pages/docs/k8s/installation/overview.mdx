...
-> **Note:** Due to an anti-affinity rules of server agents,
Consul may not be working correctly with default configuration in local Kubernetes cluster.
If what you want is just testing the installation in local Kubernetes cluster,
configure the [`server.affinity` chart value.](/docs/k8s/helm#v-server-affinity).
