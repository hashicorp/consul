...
- Setting the `consul.hashicorp.com/connect-service-protocol` annotation on your
  connect pods, e.g.
...
- Helm config `connectInject.centralConfig.proxyDefaults`, e.g.
  ```yaml
  connectInject:
    centralConfig:
      proxyDefaults: |
...
Overall our migration process will consist of identifying which [config entries](/docs/agent/config-entries)
...
installation of Consul, i.e. they can not be changed through the Helm chart.
...
1. Finally, set `connectInject.centralConfig.defaultProtocol` to `""` and run a Helm upgrade to remind yourself that it is no longer used:
   ```yaml
   connectInject:
     centralConfig:
       defaultProtocol: ''
...
From new services from now on, you must create a `ServiceDefaults` custom resource
for each service.
...
1. Finally, set `connectInject.centralConfig.proxyDefaults` to `{}` and run a Helm upgrade to remind yourself that it is no longer used:
