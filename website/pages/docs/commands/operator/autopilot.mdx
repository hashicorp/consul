...
- `-disable-upgrade-migration` <span class="label-enterprise">Enterprise</span> - Controls whether Consul will avoid promoting
  new servers until it can perform a migration. Must be one of `[true|false]`.

- `-redundancy-zone-tag` <span class="label-enterprise">Enterprise</span> - Controls the [`-node-meta`](/docs/agent/options#_node_meta)
  key name used for separating servers into different redundancy zones.

- `-upgrade-version-tag` <span class="label-enterprise">Enterprise</span> - Controls the [`-node-meta`](/docs/agent/options#_node_meta)
