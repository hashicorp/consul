...
Sensu has a much more modern design as a DevOps Observability Pipeline
for DevOps and SRE, relying on local agents to run checks and pushing
results to a cluster of Sensu backends which ingest and handle the results
of the health checks from the agents. This model is more scalable than Nagios,
as it allows for much more horizontal scaling and a weaker coupling between 
the servers and agents. Sensu’s primary advantage is in heterogeneous ephemeral
cloud environments, or to integrate existing best-of-breed monitoring solutions.

Sensu’s monitoring-as-code approach and clean handling of ephemeral cloud 
environments integrates well with Consul’s architecture for dynamic cloud
deployments. For more about this integration between Sensu and Consul,
please see our Partners section.
