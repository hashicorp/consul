...
The "host:port" must be correct for the Consul server.
...
**TODO(sso): update this after john is done**
...
- Monitor Consul's log output. Important information about OIDC validation
  failures will be emitted.
...
- If you're seeing claim-related errors in logs, review the provider's docs
  ...
  implicit grant request to obtain a JWT that you can inspect. An example of
  how to decode the JWT (in this case located in the "access_token" field of a
  JSON response):
