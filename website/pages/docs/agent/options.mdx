...
- `cache` Cache configuration of agent. The configurable values are the following:
  ...
  - `entry_fetch_rate_limit`: configures the rate-limit at which the cache may refresh a single
    ...
    The default value is 2 requests/s at maximum and should be decreased on large
