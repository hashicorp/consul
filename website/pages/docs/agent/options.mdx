...
- `cache` Cache configuration of agent. The configurable values are the following:
  ...
  - `rate_limit_per_entry`: configures the rate-limit at which the cache may refresh a single
    ...
    The value is expressed with following syntax: `<dividend> / [divisor]<unit>`. Both dividend
    and divisor must be positive integers and if the divisor is omitted, its value is considered
    to be 1, so `25/m` and `25/1m` are considered equivalent and mean up to 25 updates on the
    entry per minute. The unit supported are: "ns", "us" (or "µs"), "ms", "s", "m", "h", spaces
    are ignored. The rate limit supports a burst of 2 quick updates.
    Thus, it is possible to get 2 updates quickly (if nothing did change for a while), but if
    service is unstable and changes all the time, the rate will be applied.
    Example: `10/3s` will rate limit the number of calls to 10 calls every 3 seconds.
