...
- `audit` <EnterpriseAlert inline /> - Added in Consul 1.8, the audit object allow users to enable auditing
    ...
    - `enable` - Controls whether Consul logs out each time a user
    performs an operation. ACLs must be enabled to use this feature. Defaults to `false`.

    - `sink` - This object provides users configuration for where Consul
    logs auditing events to. Sink is an object containing keys to sink objects, where the key is the name of the sink. 
      ...
      - `rotate_bytes` - Specifies how large an
        ...
        `rotate_duration` or  must be configured to enable audit logging.
