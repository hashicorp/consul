...
- `Listeners` `(array<IngressListener>: <optional>)` - A list of listeners that
  ...
  - `Services` `(array<IngressService>: <optional>)` - A list of services to be
    ...
    - `Hosts` `(array<string>: <optional>)` - A list of hosts that specify what
      ...
      The wildcard specifier, `*`, can be used as part of the host to match
      multiple hosts, but only in the leftmost DNS label. This ensures that all
      defined hosts are valid DNS records. For example, `*.example.com` is
      valid, while `example.*` and `*-suffix.example.com` are not.
