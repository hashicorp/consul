...
-> **1.8.0+:** This config entry is available in Consul versions 1.8.0 and newer.
  ...
  The `terminating-gateway` config entry kind allows for configuring terminating gateways
  to proxy traffic from services in the mesh to services registered with Consul that do not have a
  [Connect proxy](/docs/connect/proxies). See [Terminating Gateway](/docs/connect/terminating_gateway)
...
- `Services` `(array<LinkedService>: <optional>)` - A list of services to link
    ...
    must be registered with Consul for the gateway to discover their addresses.
