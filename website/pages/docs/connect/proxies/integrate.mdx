...
client certificate from the caller, the proxy must authorize the connection.

Connection authorization can be performed one of two ways. The first is by
calling the [`/v1/agent/connect/authorize`](/api/agent/connect) endpoint.

~> **Note:** This endpoint is only suited for networking
layer 4 (e.g. TCP) integration. The endpoint will always treat intentions with
Permissions defined (i.e., layer 7 criteria) as deny intentions during evaluation.

Alternatively, proxies may listing intentions that match the destination by
querying the [intention match API](/api/connect/intentions#list-matching-intentions)
endpoint, and represent them in the native configuration of the proxy itself
(such as RBAC for Envoy). For performance and reliability reasons this is the
desirable method for implementing intention enforcement. These cached intentions
can then be consulted for each incoming connection to determine if the should be
...
intend to find a suitable format to document the behavior differences between
