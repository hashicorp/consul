...
Mesh gateways are defined very similarly to other typical services, with two exceptions.
The first is that the [service kind](/api/agent/service#kind) must be "mesh-gateway". The other
is that a mesh gateway service definition may contain a `Proxy.Config` entry just like a
