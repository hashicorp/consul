...
defined as a its address. The address registered with Consul for services the gateway will route to **must** be a resolved IP address.
...
1. On a host dedicated to the gateway and Consul client, start an Envoy proxy using the [envoy subcommand](/docs/commands/connect/envoy#terminating-gateways) and
...
Terminating gateways are defined in service configuration files and registered with Consul like other services, with two exceptions.
...
~> [Configuration Entries](/docs/agent/config_entries) are global in scope. A configuration entry for a gateway name applies
