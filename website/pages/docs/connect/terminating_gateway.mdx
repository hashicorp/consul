...
We recommend that terminating gateways not be exposed to the WAN or open internet. This is because terminating gateways
...
from the gateway will be encrypted using one-way TLS authentication. If a path to a
...
from the gateway will be encrypted using mutual TLS authentication.
