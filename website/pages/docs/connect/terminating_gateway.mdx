...
1. Start an Envoy proxy using the [envoy subcommand](/docs/commands/connect/envoy#terminating-gateways), specifying the `terminating` gateway type:
    ```shell
    $ consul connect envoy -gateway=terminating -register -address '0.0.0.0:8443'
    ```
...
3. Ensure that [Consul intentions](/docs/commands/intention#consul-intention) are set up between the source services in the mesh 
...
The services that the terminating gateway will proxy for must be registered with Consul for the terminating gateway
to discover their addresses. These services can be registered with a local Consul agent, or if there is no agent present,
