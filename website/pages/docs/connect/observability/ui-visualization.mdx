...
To configure Consul's UI to fetch metrics two required configuration settings.
...
(function () {
  var provider = {
     ...
     *    headers. Otherwise it functions like the browsers native fetch.
     *
...
functions like the browsers native fetch and will forward your request on to the
