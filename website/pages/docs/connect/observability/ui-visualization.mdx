...
To configure Consul's UI to fetch metrics there are several
...
We assume that the UI is already enabled by
...
The UI must query the metrics provider through a proxy endpoint. This
simplifies deployment where Prometheus is not exposed externally to UI users'
browsers.

To set this up, provide the URL that the _Consul agent_ should use to reach the Prometheus
server in
...
In many cases the metrics backend may be inaccessible to UI users' browsers or
...
**This is intended simplify setup in test and demo environments. Careful
...
enabling it anyone with network access to the Consul agent's API port may use
it to access metrics from the backend.
...
It's typical for most authenticated users to have this level of access in
Consul as it's required for viewing the catalog or discovering services. If you
...
`["/api/v1/query_range", "/api/v1/query"]`.
...
ui_config {
  ...
  metrics_proxy {
    base_url = "https://example-apm/api/v1/metrics"
    add_headers = [
      {
        name = "Authorization"
        value = "Bearer <token>"
...
[Prometheus](https://prometheus.io). To enable the UI visualization feature to
work with other existing metrics stores and hosted services, we created a
"metrics provider" interface in Javascript. A custom provider may be written
and the javascript file served by the Consul agent.
...
(function () {
  var provider = {
     ...
     * Consul will provide:
     *
     * 1. A boolean field options.metrics_proxy_enabled to indicate whether the agent
...
A fetch-like function is provided to the metrics provider javascript during
`init` as `options.fetch`. This is a thin fetch wrapper that prefixes any url
with the url of Consul's metrics proxy endpoint and adds your current Consul
ACL token to the request headers. Otherwise it functions like the browsers
native fetch and will forward your request on to the metrics backend. The
response will be returned without any modification to be interpreted by the
provider and converted into the format as described in the interface above.
