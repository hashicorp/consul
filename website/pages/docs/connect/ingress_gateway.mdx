...
service mesh to services inside the Consul service mesh. These gateways act as
an entrypoint for outside traffic and allow users to define what services
should be exposed and on what port. They allow for defining a set of
...
2. Create an `ingress-gateway` [configuration
entry](/docs/agent/config-entries/ingress-gateway) that defines a set of listeners that expose the desired backing services.
The config entry can be applied via the [CLI](/docs/commands/config/write) or [API](/api/config#apply-configuration).
...
Ingress gateways are defined very similarly to other typical services, with two exceptions.
The first is that the [kind](/api/agent/service#kind) must be "ingress-gateway". The other
is that a ingress gateway service definition may contain a `Proxy.Config` entry just like a
