...
1. Start an Envoy proxy using the [envoy subcommand](/docs/commands/connect/envoy#ingress-gateways), specifying the `ingress` gateway type.
1. Create an `ingress-gateway` [configuration
...
1. Ensure that [Consul intentions](/docs/connect/intentions) are setup between the ingress gateway and the backing services.
1. **Optionally** use the `<service>.ingress.<domain>` [DNS subdomain](/docs/agent/dns#ingress-service-lookups) to discover the ingress
