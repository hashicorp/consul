...
1. Start an Envoy proxy using the [envoy subcommand](/docs/commands/connect/envoy), specifying the `ingress` gateway type:
    ```shell
    $ consul connect envoy -gateway=ingress -register -address '{{ GetInterfaceIP "eth0" }}:8888'
    ```
...
3. Ensure that [Consul intentions](/docs/connect/intentions) are setup between the ingress gateway and the backing services.
