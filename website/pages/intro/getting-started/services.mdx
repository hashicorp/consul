...
description: >
  ...
  making the appropriate calls to the HTTP API. A configuration file is the most
  common, so we will use this approach to register a service, and then query
  that service using the REST API and DNS interfaces.
---
...
[service definition](/docs/agent/services) or by making the appropriate
calls to the [HTTP API](/api).
...
we covered in the [previous step](/intro/getting-started/agent).
...
[this is configurable](/docs/agent/options#domain). The `service`
...
with [health checks](/intro/getting-started/checks) you'll typically want
...
[automatically encrypt and authorize service-to service communication](/intro/getting-started/connect) with Consul Connect.
