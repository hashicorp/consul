...
is used to map values which can be interpolated while `ListClaimMappings` is
used to map to map lists of values that cannot be interpolated.
...
**TODO** If a claim is configured in claim_mappings, it must existing in the
JWT or else the authentication will fail.
