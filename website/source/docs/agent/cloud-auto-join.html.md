...
description: |-
  Consul supports automatically joining a Consul deployment using cloud metadata on various providers.
---
...
automatically joining a Consul deployment using cloud metadata. To use `retry-join` with a
...
recommended that you make a dedicated key used only to auto-join the deployment. If the
...
It is recommended you make a dedicated key used to auto-join the Consul deployment.
