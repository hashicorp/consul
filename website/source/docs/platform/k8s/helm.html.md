...
* <a name="v-global" href="#v-global">`global`</a> - These global values affect multiple components of the chart.
  ...
  * <a name="v-global-gossip" href="#v-global-gossip">`gossipEncryption`</a> - Provides the ability to enable [Gossip encryption](/docs/agent/encryption.html#gossip-encryption). We highly recommend that you enable gossip encryption in all production environments. You must specify both `secretName` and `secretKey` to enable gossip encryption.
