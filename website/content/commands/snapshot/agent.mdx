...
- `-azure-blob-account-name` - The Azure storage account name. Always required for Azure Blob storage.
...
- `-azure-blob-auth-type` - Authentication type to use for Azure Blob storage. Valid options are `msi` (Managed Service Identity), `service-principal`, or `shared-key`. Defaults to `msi`.

#### Authentication Methods

Azure Blob storage supports three authentication methods:

1. **Shared key authentication:** Include the `-azure-blob-account-key` parameter. This value is the shared access key for the storage account. It is required when using shared key authentication.
...
1. **Managed Service Identity (MSI) authentication:** MSI authentication uses the Azure instance's managed identity and requires no additional configuration beyond the account name and container. This authentication method is used by default, and we recommend using it when running on Azure infrastructure that supports managed identities. This option was introduced in Consul v1.22.0+ent.
