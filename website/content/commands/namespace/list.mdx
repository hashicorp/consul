...
This `namespace list` command lists all namespace configurations.
The results will be filtered based on the ACL token and therefore it is possible to
see a partial list.
This command has the following characteristics:

| Characteristic | Value |
| -------------- | ----- |
| [Required ACLs] | `operator:read` or `namespace:*:read`<sup>1</sup> |
