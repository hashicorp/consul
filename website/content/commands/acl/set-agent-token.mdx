...
- `config_file_service_registration` - This is the token that the agent uses to
  ...
  set, the default token is used. If a service or check definition contains a
  'token' field, then that token is used to register that service or check
  instead of the `config_file_service_registration` token.
