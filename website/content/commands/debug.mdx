...
| `pprof`   | Golang heap, CPU, goroutine, and trace profiling captured for `-duration` time. This information is not retrieved unless [`enable_debug`](/docs/agent/options#enable_debug) is set to `true` on the target agent or ACL are enable and an ACL token with `operator:read` is provided. |
