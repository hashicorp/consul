...
| -------------- | ----- |
| [Required ACLs] | `agent:read` and `session:write` on the local agent;<br />`key:write` and `event:write` on the `"_rexec"` prefix;<br />additional privileges on the agent token<sup>1</sup> |
| Corresponding HTTP API endpoint | None |

<sup>1</sup> In addition to the above, the policy associated with the [agent token](/docs/security/acl/acl-tokens#acl-agent-token) should have `write` on `"_rexec"` key prefix. This is for the agents to read the `exec` command and write its output back to the KV store.
