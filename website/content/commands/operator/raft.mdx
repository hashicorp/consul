...
This command transfer the Consul leader to another server agent. If an `id` is provided, raft leadership will be transferred
to the server with that id. It provides a way to transition leadership without restarting the leader node, maintaining quorum and workload capacity.
...
If empty, leadership will be transferred to a random server agent.
