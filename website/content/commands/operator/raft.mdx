...
Corresponding HTTP API Endpoint: [\[POST\] /v1/operator/raft/transfer-leader](/api-docs/operator/raft#transfer-leader)

This command transfer the Consul leader to another server. If an `id` is provided, raft leadership will be transferred
to the server with that id.
...
- `-id` - ID of the server to transfer leadership to.
