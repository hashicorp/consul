...
| -------------- | ----- |
| [Required ACLs] | Varies<sup>1</sup> |
| Corresponding HTTP API endpoint | N/A |

[Service identity](/docs/security/acl#service-identities) for the [service this proxy represents](#service)

<sup>1</sup> The actual ACL required depends on the whether the Envoy instance is acting
[as a sidecar proxy](#sidecar-for) or
[as a gateway](#gateway).

| Envoy proxy type       | Required ACL       |
| ---------------------- | ------------------ |
| Sidecar proxy          | [Service identity](/docs/security/acl#service-identities) for the [service name this proxy represents](#service)    |
| Ingress gateway        | Refer to [Ingress Gateway Configuration](/docs/connect/gateways/ingress-gateway#ingress-gateway-configuration) |
| Terminating gateway    | Refer to [Terminating Gateway Configuration](/docs/connect/gateways/terminating-gateway#terminating-gateway-configuration) |
| Mesh gateway           | Refer to [Mesh Gateway Configuration](/docs/connect/gateways/mesh-gateway/service-to-service-traffic-datacenters#configuration) |
| API gateway (k8s only) | Handled automatically if [`managedSystemACLs: true`](/docs/k8s/helm#v-global-acls-managesystemacls) |
