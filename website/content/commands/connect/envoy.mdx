...
- `-bootstrap` - If present, the command will simply output the generated
  ...
  allowed to access by [service mesh intentions](/consul/docs/secure-mesh/intentions).
...
- `-enable-config-gen-logging` - This flag enables debug message logging when generating the Envoy bootstrap configuration to help troubleshoot issues. Logs output to `stderr`. For more information about generating the Envoy bootstrap configuration, refer to [Envoy proxy configuration](/consul/docs/reference/proxy/envoy#bootstrap-configuration).
...
registration](/consul/docs/reference/proxy/sidecar) helper):
