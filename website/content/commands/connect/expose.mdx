...
This subcommand is used to expose a service in the service mesh
through an Ingress gateway by modifying the gateway's configuration and adding
...
Usage: consul connect expose [options]
  ...
  Exposes a service in the service meshthrough the given ingress gateway,
  using the given protocol and port.
```
