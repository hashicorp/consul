...
- `Check` `(Check: nil)` - Specifies to register a check. The register API
  ...
  You can provide defaults for TCP and HTTP health checks to the `Definition` field. Refer to [Health Checks](/consul/docs/register/service/vm/health-checks) for additional information.
...
- `merge-central-config` - Include this flag in a request for `connect-proxy` kind or `*-gateway` kind
  ...
  [service-defaults/:service](/consul/docs/reference/config-entry/service-default) config entries.
