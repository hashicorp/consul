...
-> **1.17.2+:** This API is available in Consul versions 1.17.2 and later.

This endpoint returns all the exported services along with their consumers

| Method | Path                                 | Produces           |
| ------ | ------------------------------------ | ------------------ |
| `GET`  | `/catalog/exported-services` | `application/json` |
...
| Blocking Queries | Consistency Modes | Agent Caching | ACL Required   |
| ---------------- | ----------------- | ------------- | -------------- |
| `YES`            | `all`             | `none`        | `mesh:read or operator:read` |

### Sample Request

```shell-session
$ curl \
    http://127.0.0.1:8500/v1/catalog/exported-services
```
