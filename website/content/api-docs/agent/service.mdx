...
- `Kind` `(string: "")` - The kind of service. Defaults to `""`, which is a
  ...
  - `"mesh-gateway"` for instances of a [mesh gateway](/consul/docs/link/mesh-gateway/#service-mesh-proxy-configuration)
...
- `Connect` `(Connect: nil)` - Specifies the
  [configuration for service mesh](/consul/docs/connect/configuration-entries). The connect subsystem provides Consul's service mesh capabilities. Refer to the
...
- `SidecarService` `(ServiceDefinition: nil)` - Specifies an optional nested
  ...
  [Deploy sidecar services](/consul/docs/reference/proxy/sidecar) for additional information.
