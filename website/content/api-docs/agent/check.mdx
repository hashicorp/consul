...
- `UDP` `(string: "")` - Specifies a `UDP` IP address, hostname, port, or combination thereof. The check sends datagrams to the value specified at the interval specified in the `Interval` configuration. If the datagram is sent successfully or a timeout is returned, the check is set to the `passing` state.  The check is logged as `critical' if the datagram is sent unsuccessfully.
