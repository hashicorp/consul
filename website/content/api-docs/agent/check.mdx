...
- `UDP` `(string: "")` - Specifies a `UDP` to send datagrams against the value of `UDP`
  (expected to be an IP or hostname plus port combination) every `Interval`. If
  the datagram sending is successful or gets a timeout, the check is `passing`. 
  Any other failing result is unsuccessful, the check is `critical`.
