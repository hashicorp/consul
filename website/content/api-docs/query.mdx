...
## Fetch Prepared Query By Name

This endpoint fetches an existing prepared query given its name. If no query exists by the
given name, an error is returned.

| Method | Path           | Produces           |
| ------ | -------------- | ------------------ |
| `GET`  | `/query/:name` | `application/json` |
...
- `uuid` `(string: <required>)` - Specifies the name of the query to read.
  This is required and is specified as part of the URL path.
...
```shell-session
$ curl \
  http://127.0.0.1:8500/v1/query/my-query-name
```
