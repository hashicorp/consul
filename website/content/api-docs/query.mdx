...
- `Service` `(Service: <required>)` - Specifies the structure to define the query's behavior.
  ...
  to query. If not provided the query will use Consul default partition for resolution. When combined with
  `SamenessGroup` this will specify the partition in which the `SamenessGroup` exists
