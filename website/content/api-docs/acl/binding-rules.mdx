...
- `BindType` `(string: <required>)` - Specifies the way the binding rule
  ...
  - `BindType=templated-policy` - The bind name value is used as an
...
- `BindVars` `(<ACLTemplatedPolicyVariables>)` - Specifies the templated policy variables when
`BindType` field is set to `templated-policy`. The computed `BindVars` values are used for 
`ACLTemplatedPolicy.TemplateVariables` field in the token that is created. Similar to `BindName`, 
it can either be a plain string or lightly templated using [HIL syntax](https://github.com/hashicorp/hil) 
to interpolate the same values that are usable by the `Selector` syntax. For example:
