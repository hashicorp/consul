...
- `TemplatedPolicies` `(array<TemplatedPolicy>)` - The list of [templated policies](/consul/docs/security/acl#templated-policies)
  that should be applied to the role. Added in Consul 1.17.

    - `TemplateName` `(string: <required>)` - The templated policy name.
    
    - `TemplateVariables` `(ACLTemplatedPolicyVariables: <optional>)` - The templated policy variables. Not all templated policies
      require variables.
