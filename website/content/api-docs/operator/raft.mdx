...
This endpoint transfer the raft leadership from the current raft leader to a new node or
the node with the provided id.
...
The corresponding CLI command is [`consul operator raft tranfer-leader`](/commands/operator/raft#tranfer-leader).

### Query Parameters

- `id` `(string: "")` - Specifies the ID of the raft peer to transfer leadership to, if empty the leadership
will be transferred to a random server.
