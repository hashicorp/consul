...
new root certificate being used, the [Root Rotation](/consul/docs/secure-mesh/certificate#root-certificate-rotation) process will be triggered.
...
~> **If currently using Vault CA provider:**
   ...
   or to change the Vault provider's [`RootPKIPath`](/consul/docs/secure-mesh/certificate/vault#rootpkipath),
   you must temporarily elevate the privileges of the Vault token
   or auth method in use as described in the
   [Vault CA provider documentation](/consul/docs/secure-mesh/certificate/vault#additional-vault-acl-policies-for-sensitive-operations).
...
- `ForceWithoutCrossSigning` `(bool: false)` - Indicates that the CA change
  ...
  Refer to [Forced Rotation Without Cross-Signing](/consul/docs/secure-mesh/certificate#forced-rotation-without-cross-signing)
  for more detail.
