...
By default Docker will replace host files that use localhost addresses with its own DNS settings. This can cause issues with `systemd-resolved` and Docker containers.
To prevent Docker from replacing the host DNS settings, a Stub resolver address can be configured in systemd-resolved and the `dns` option in the `/etc/docker/daemon.json` file should be set to the corresponding address.
...
After making these changes, restart the `systemd-resolved` and `docker` services.
