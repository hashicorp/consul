...
Consul's service mesh features are not enabled by default. To enable the service mesh, you must change the configuration of your Consul servers. You do not need to change client agent configurations in order to use the service mesh.

To enable Consul's service mesh, set `connect.enabled` to `true` in a new or existing [agent configuration file](/consul/docs/agent/config/config-files). On Kubernetes deployments, update the cluster's [Helm chart configuration](/consul/docs/k8s/helm) instead.

<CodeTabs heading="Enable Consul service mesh" tabs={[ "HCL", "JSON", "YAML" ]}>
...
On Kubernetes deployments, you can run `kubectl apply -f values.yaml` to apply the configuration to your deployment. If you use the Consul on Kubernetes CLI, you can run `consul-k8s upgrade -config-file values.yaml`. For information about the `consul-k8s` CLI and how to install it, refer to [Install Consul on Kubernetes from Consul K8s CLI](/consul/docs/k8s/installation/install-cli)
