...
To enable Consul's service mesh, set `connect.enabled` to `true` in a new or existing [agent configuration file](/consul/docs/agent/config/config-files). Service mesh is enabled by default on Kubernetes deployment. Refer to the [Helm chart configuration](/consul/docs/k8s/helm) to verify that service mesh is enabled.
