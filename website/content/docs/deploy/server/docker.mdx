...
To avoid mounting volumes or copying files to the container, you can also save [configuration JSON](/consul/docs/reference/agent/configuration-file#configuration-files) to that directory via the environment variable `CONSUL_LOCAL_CONFIG`, which will be covered later in the tutorial.
