...
If you would like to interact with your containerized Consul agents using a local install of Consul, follow the instructions [here](/consul/docs/install) and install the binary somewhere on your PATH.
...
To avoid mounting volumes or copying files to the container, you can also save [configuration JSON](/consul/docs/agent/config/config-files#configuration-files) to that directory via the environment variable `CONSUL_LOCAL_CONFIG`, which will be covered later in the tutorial.
