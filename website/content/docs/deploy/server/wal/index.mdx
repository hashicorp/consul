...
This topic provides an overview of the WAL (write-ahead log) LogStore backend. The WAL backend is an experimental feature. Refer to [Requirements](/consul/docs/agent/wal-logstore/enable#requirements) for supported environments and known issues.

We do not recommend enabling the WAL backend in production without following [our guide for safe testing](/consul/docs/agent/wal-logstore/enable).
