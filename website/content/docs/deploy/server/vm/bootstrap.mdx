...
1. Create a cluster with bootstrap-expect set
1. Join the servers either automatically or manually

### Initiate cluster

Use the [`-bootstrap-expect`](/consul/docs/agent/config/cli-flags#_bootstrap_expect) flag on the command line or configure `bootstrap_expect` in the agent configuration. This option declares the expected number of servers and automatically bootstraps them when that many servers are available. To prevent inconsistencies and "split-brain" deployments where multiple servers consider themselves leader, you should either specify the same value for each server's `bootstrap_expect` parameter or specify no value at all on all the servers. When `-bootstrap-expect` is omitted, Consul assumes a default value of `1` for the number of expected servers.

The following command bootstraps a simple datacenter that consists of three Consul servers:
...
Consul should produce a warning message in the service output when the number of servers in a cluster is less than the expected bootstrap number:
...
After you start the servers, you must join them in a cluster to initiate the Raft election. To join servers automatically, specify network addresses or [cloud auto join](/consul/docs/deploy/server/vm/cloud-auto-join) tags for supported cloud environments using either the [-retry-join CLI flag](/consul/docs/agent/config/cli-flags#_retry_join) or the [`retry_join` configuration option](/consul/docs/agent/config/config-files#retry_join).
