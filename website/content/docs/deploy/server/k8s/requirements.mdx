...
You can enable support for Red Hat OpenShift by setting `enabled: true` in the `global.openshift` stanza. Refer to the [Deploy Consul on RedHat OpenShift tutorial](/consul/docs/deploy/server/k8s/platform/openshift) for instructions on deploying to OpenShift. 
