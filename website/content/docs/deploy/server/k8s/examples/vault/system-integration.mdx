...
Complete the following steps once:
  ...
  - Enabling Vault PKI Engine if you are choosing to store and manage either [Consul Server TLS credentials](/consul/docs/deploy/server/k8s/examples/vault/data-integration/server-tls) or [Service Mesh and Consul client TLS credentials](/consul/docs/deploy/server/k8s/examples/vault/data-integration/service-mesh-certificate)
...
As a next step, please proceed to Vault integration with Consul on Kubernetes' [Data Integration](/consul/docs/deploy/server/k8s/examples/vault/data-integration).
