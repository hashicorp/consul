...
This topic describes how to configure the Consul Helm chart to use an ACL partition token stored in Vault when using [Admin Partitions](/consul/docs/segment/admin-partition) in Consul Enterprise.

## Overview

Complete the steps outlined in the [Data Integration](/consul/docs/deploy/server/k8s/examples/vault/data-integration) section to use an ACL partition token stored in Vault.
...
1. Read and completed the steps in the [Systems Integration](/consul/docs/deploy/server/k8s/examples/vault/system-integration) section of [Vault as a Secrets Backend](/consul/docs/deploy/server/k8s/examples/vault).
2. Read the [Data Integration Overview](/consul/docs/deploy/server/k8s/examples/vault/data-integration) section of [Vault as a Secrets Backend](/consul/docs/deploy/server/k8s/examples/vault).
