...
Before starting the installation, ensure that the Consul Servers are configured to have the gRPC port enabled `8502/tcp` using the [`ports.grpc = 8502`](/consul/docs/reference/agent#grpc) configuration option.
...
As noted in the [Security Model](/consul/docs/secure-consul/security-model),
...
so that the Consul servers can validate a Kubernetes service account token when using the [Kubernetes auth method](/consul/docs/secure-consul/acl/auth-methods/kubernetes)
with `consul login`.
