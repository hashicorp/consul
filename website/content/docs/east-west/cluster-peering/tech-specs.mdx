...
By default, cluster peering connections use mesh gateways in [remote mode](/consul/docs/east-west/mesh-gateway/federation#remote). Be aware of these additional requirements when changing a mesh gateway's mode.
...
Refer to [mesh gateway modes](/consul/docs/east-west/mesh-gateway/#modes) for more information.
...
The `exported-services` configuration entry is required in order for services to communicate across partitions with cluster peering connections. Basic guidance on using the `exported-services` configuration entry is included in [Establish cluster peering connections](/consul/docs/east-west/cluster-peering/establish-connection/vm#export-services-between-clusters).
