...
For additional information about specific Consul secrets that you can store in Vault, refer to [Data Integration](/consul/docs/deploy/server/k8s/vault/data) in the [Vault as a Secrets Backend](/consul/docs/deploy/server/k8s/multi/vault) documentation.
