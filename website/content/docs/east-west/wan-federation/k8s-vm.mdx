...
1. If ACLs are enabled you must create an ACL replication token with the following rules:
   ...
   ```
1. If ACLs are enabled you must also modify the [anonymous token](/consul/docs/secure/acl/tokens#anonymous-token) policy to have the following permissions:
...
by the primary datacenter. By setting the API URL, the secondary cluster will configure a [Consul auth method](/consul/docs/secure/acl/auth-methods)
