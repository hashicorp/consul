...
This topic covers the advanced form of federating Consul datacenters using the network areas capability of Consul Enterprise. For the basic form of federation available in the open source version of Consul, check the [basic federation documentation](/consul/docs/east-west/wan-federation/vms).

<Note title="Enterprise Only">

 The network area functionality demonstrated here requires [Consul Enterprise](https://www.hashicorp.com/products/consul/pricing/). If you've purchased or wish to try out Consul Enterprise, refer to [how to access Consul Enterprise](/consul/docs/enterprise#access-consul-enterprise).
...
Consul's WAN gossip federation support relies on all Consul servers in all datacenters having full connectivity via server RPC (`8300/tcp`) and Serf WAN (`8302/tcp` and `8302/udp`). Securing this setup requires TLS in combination with managing a gossip keyring. With massive Consul deployments, it becomes tricky to support full connectivity between all Consul servers, and to manage the keyring.

Traffic between network areas is routed via server RPC (`8300/tcp`) so it can be secured with just TLS. Also network areas do not require full connectivity across all servers in all the datacenter, but only between the servers in the two datacenter that are being federated. 

Network areas are the suggested approach when your architecture layout does not permit connectivity between all Consul servers across all datacenters, or when the data in one datacenter only needs to be accessible from one or few other datacenters. 

To help migrating across the two federation solutions, networks areas can be used alongside the Consul's WAN gossip federation model and the WAN gossip pool. A peer datacenter can be connected via the WAN gossip pool and a network area at the same time, and RPC requests will be forwarded as long as servers are available in either.
...
To get started, follow the [Deployment Guide](/consul/tutorials/production-deploy/deployment-guide) to start each datacenter. After bootstrapping, you should have two datacenters now which will be referred to as `dc1` and `dc2`. Note that datacenter names are opaque to Consul; they are simply labels that help human operators reason about the Consul datacenters.

A compatible pair of areas must be created, one in each datacenter.
...
That, until servers are not joined in a network area, will show only local servers.
...
To complete the federation, join with at least one Consul server in the other datacenter. Consul will automatically make sure that all servers within the datacenter where the area was created are joined to the area using the LAN information.
...
This time the command will output all the servers present in the network area.
...
With network area enabled, you can route RPC commands in both directions. Here's a sample command to set a KV entry in `dc2` from `dc1`.
...
The command will output `true` on success
...
Consul does not provide a command to leave a previously joined network area. To remove the federation between two datacenters the suggested approach is to remove the network area from both.
...
Once the command is executed from one of the datacenters, the federation is already removed and the servers are shown as `left` from the other datacenters.
...
Once the command is execute from one of the datacenters, the federation is already removed.
