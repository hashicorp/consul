...
This page describes the process to install Consul on bare-metal machines, virtual machines (VMs), and Kubernetes clusters. To find the most recently available releases for each supported runtime and CPU architecture, refer to [install Consul](/consul/install).
...
We recommend using a precompiled binary, and we provide downloads over TLS along with SHA256 sums to verify the binary.

We also distribute a PGP signature with the SHA256 sums that can be verified. For guidance, refer to [verify HashiCorp binary downloads](/well-architected-framework/operational-excellence/verify-hashicorp-binary).
...
To verify your installation, run `consul version` on your system.
