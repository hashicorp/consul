...
Use the `-dev` flag to start the Consul agent in development mode.

```shell-session
$ consul agent -dev
```

The terminal should output the server agent's logs, which resemble the following example:
...
When Consul runs in development mode, you can also query the datacenter using the [Consul HTTP API](/consul/api-docs/).

```shell-session
$ curl localhost:8500/v1/catalog/nodes
```

The HTTP API returns a payload that resembles the following example:
...
You can use [Consul DNS](/consul/docs/services/discovery/dns-overview) to discover nodes in the datacenter. Address nodes using the following syntax: 

```plaintext hideClipboard
<node-name>.node.consul
```

By default, Consul uses port `8600` for DNS requests. Address your command to the DNS port with the `-p` flag.

```shell-session
$ dig @127.0.0.1 -p 8600 boruszak-LFH0JH12XD.node.consul
```

The output for this command resembles the following example:
...
Run Consul in development mode to help you learn about other Consul fundamentals. The following pages use development mode to demonstrate Consul's user interfaces:
