---
layout: docs
page_title: Consul CLI fundamentals
description: >-
  Consul includes a built-in command line interface.
---

# Consul CLI fundamentals
...
By default, the `consul` command targets the local instance. To target a specific cluster, provide the address of a Consul server in the `-http-addr` command line flag or from the `CONSUL_HTTP_ADDR` environment variable.
