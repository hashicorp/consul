...
When a node leaves a cluster, it communicates its intent and the cluster marks the node as `left`. Unlike when a node fails, all of the services are immediately deregistered if they were provided by a node that leaves gracefully.
...
Reaping occurs on a configurable interval of 72 hours. We do no recommend changing the reap interval due to its consequences during outage situations. For failed nodes, reaping desregisters all of the node's services.

## Agent requirements

You should run one Consul agent per server or host. Instances of Consul can run in separate VMs or as separate containers. At least one server agent per Consul deployment is required, but three to five server agents are recommended.
...
When a server exits gracefully, the server is not marked as _left_. This is to minimally impact the consensus quorum. Instead, the server is marked as _failed_. To remove a server from the cluster, the [`force-leave`](/consul/commands/force-leave) command is used. Using `force-leave` puts the server instance in a _left_ state as long as the server agent is not alive.
...
Configurations specified later are be merged into configuration specified earlier. In most cases, "merge" means that the later version overrides the earlier one. In some cases, such as event handlers, merging appends the handlers to the existing configuration. The exact merging behavior is specified for each option.
