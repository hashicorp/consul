...
| `ProxyType`    | `string` | <nobr>`connect-proxy`</nobr> | Determines the proxy type the extension applies to. The only supported value is `connect-proxy`. |
...
For a full list of parameters for `EnvoyExtensions`, refer to the [`service-defaults`](/consul/docs/reference/config-entry/service-default#envoyextensions) and [`proxy-defaults`](/consul/docs/reference/config-entry/proxy-defaults#envoyextensions) configuration entries reference documentation.
