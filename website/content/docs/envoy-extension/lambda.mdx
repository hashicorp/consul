...
Unlike [manual lambda registration](/consul/docs/lambda/registration/manual#supported-meta-fields), the Envoy extension infers the region from the ARN.
...
For a full list of parameters for `EnvoyExtensions`, refer to the [`service-defaults`](/consul/docs/connect/config-entries/service-defaults#envoyextensions). 
