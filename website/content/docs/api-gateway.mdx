...
1. Create a `values.yaml` file for your Consul API Gateway deployment that contains the following configuration through the Consul Helm chart. See [Helm Chart Configuration - apigateway](https://www.consul.io/docs/k8s/helm#apigateway) for more available options on how to configure your Consul API Gateway deployment on Kubernetes 
