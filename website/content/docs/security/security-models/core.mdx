...
- **Consul agent-to-CA communication** - Communication between the Consul server and the configured Aertificate
  Authority provider for the service mesh is always encrypted.
...
- **Service mesh communication between services** - Communications between two services in the service mesh
  (through native integration or via sidecar proxy) are secured from eavesdropping and provide authentication
  through mutual TLS.
...
- **Memory access to a running Consul client agent** - The blast radius of this is much smaller than a server agent but
  ...
  API including services, KV, and connect subsystem information may be compromised. If a particular set of data on the server was
...
- **Network access to a local service mesh proxy or service** - Communications between a service in the mesh and its sidecar proxy
  ...
  namespaces. This also requires that external processes cannot communicate to the service in the mesh or its sidecar proxy (except on
  the inbound port). Therefore, applications in the mesh should only bind to non-public addresses if using a sidecar proxy.
