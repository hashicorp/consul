...
By default, tokens created by the JWT authentication method are scoped to the local datacenter in which they were created.
...
Either a global token, or a local token from the target datacenter, is required to perform a cross-datacenter request.
