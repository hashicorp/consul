...
therefore only be valid for the datacenter in which they were created.

In the primary DC, the Auth Method can be created with [`-token-locality=global`](/commands/acl/auth-method/create#token-locality). With globaly token locality, the tokens will be replicated to all secondary datacenters.

Either a global token, or a local token in the target datacenter, is required to perform a cross-datacenter request.
