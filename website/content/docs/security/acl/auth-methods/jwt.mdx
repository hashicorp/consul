...
In the primary DC, the Auth Method can be created with [`-token-locality=global`](/commands/acl/auth-method/create#token-locality). All tokens with global locality will be replicated to all secondary datacenters.
