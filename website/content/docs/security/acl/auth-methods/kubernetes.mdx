...
For cases requiring global token scope, the primary Consul datacenter must have
access to all Kubernetes clusters in order for tokens to be validated. This
property is often prohibitive so using [`-token-locality=global`](/commands/acl/auth-method/create#token-locality)
with the Kubernetes Auth method is not recommended. As an alternative, configuring
the anonymous token policy to allow specific cross-datacenter access is recommended.
