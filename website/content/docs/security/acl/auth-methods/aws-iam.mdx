...
-> **1.12.0+:** This feature is available in Consul versions 1.12.0 and newer.
...
- `EnableIAMEntityDetails` `(bool: <false>)` - This enables the auth method to fetch the IAM role or
  ...
  login must have `iam:GetRole` or `iam:GetUser` permission, respectively, to retrieve itself. This
  is required in order to fetch the full path and tags of the IAM user or role, and it is required
  in order to use wildcards in the `BoundIAMPrincipalARNs`.
- `IAMEntityTags` `(array<string>: [])` - The list of tag keys retrieved from the IAM role or user
  ...
  Only the tags in this will be available in binding rules as `entity_tags.<key>`. For example, if
  you include a `service-name` tag on an IAM role and specify the tag key in this list, then the you
  can reference the tag value using `entity_tags.service-name` in binding rules.
...
- If the authenticating client is an IAM role, the client must have an `iam:GetRole` permission to
  fetch its own role. The following shows an example of an IAM Policy document which grants this
  permission:
...
If `EnableIAMEntityDetails=false`, a client must log in with `consul login -type aws-iam
-aws-auto-bearer-token ...`. This command does the following:
...
Otherwise, if `EnableIAMEntityDetails=true`, the client must log in with `consul login -type aws-iam
-aws-auto-bearer-token -aws-include-entity ...` in order to inclue a signed `iam:GetRole` or
`iam:GetUser` request in the bearer token. This command does the following:
...
- Optionally, it sends the `iam:GetRole` or `iam:GetUser` request to AWS. This request is pre-signed
  by the client, so no other credentials or permisssions are required to make the request. Only the
  client must have `iam:GetRole` or `iam:GetUser` permissions. AWS validates the client's signature
