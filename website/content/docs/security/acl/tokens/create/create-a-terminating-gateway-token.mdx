...
Gateways must present a token linked to policies that grant the appropriate set of permissions in order to be discoverable and to forward traffic out of the mesh.
...
The terminating gateway token must be linked to policies that grant the following permissions:
