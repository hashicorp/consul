...
Templated policies are a configuration block that you can add to a token/role configurations. You can use them during the authorization process to automatically generate a policy for specific use-cases. The policy is linked to the role or token, allowing token/role to have required to access to a particular resource.

Templated policies enable you to quickly construct policies for common use cases, rather than creating multiple identical policies or digging through the permission system to understand permissions.
