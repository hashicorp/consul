...
Looking for an policy example for your use case? Refer to the [Policy Examples](/docs/security/acl/policy-examples) topic.
...
The following are the policy rules for the `global-management` policy.

<Tabs>

<Tab heading="OSS">

The `global-management` policy provides write permissions on all resource types.
...
In Consul Enterprise, the `global-management` policy contains all of the OSS rules and
adds additional partition and namespace scoped rules.
...
The `namespace-management` policy will be injected into all namespaces you create. The policy will be assigned a randomized UUID and can be managed as a normal, user-defined policy within the namespace. 

The following are the policy rules for the namespace management policy. Because this
policy resides in a non-default namespace, these permissions are implicitly limited to 
