...
Refer to the [Policy Examples](/consul/docs/security/acl/acl-policy-examples) for example policy configurations.
...
The following policy rules define the `global-management` policy.
...
Consul injects the `namespace-management` policy into all namespaces you create. Consul also assigns the policy a randomized UUID that you can use to manage the policy within the namespace. 

The following policy rules define the namespace management policy. Because this
