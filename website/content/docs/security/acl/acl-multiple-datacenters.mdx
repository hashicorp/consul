...
In a [federated Consul deployment](https://www.consul.io/docs/k8s/installation/multi-cluster), one of the datacenters is marked as the primary datacenter.
...
- `policy = "read"` and `intentions = "read"` in the `service_prefix` field: These permissions enable `service-default` configuration entries, CA, and intention data to be replicated.
...
When ACLs are enabled, client agents need a special token known as the `agent token` for ACL-related operations. Agent tokens need to have the right policies for node related actions, including
