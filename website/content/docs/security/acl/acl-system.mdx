...
| [`mesh`](/docs/security/acl/acl-rules#mesh-rules)            | Access to ingress gateways, terminating gateways and mesh configuration entries [Config API](/api/config)
...
A full table of what functions/endpoints are controlled by each resource is [here](/docs/security/acl/acl-endpoints-by-resource).:
