...
- **ACL Service Identities** - Service identities are a policy template for
  expressing a link to a policy suitable for use in the [service mesh](/docs/connect).
  At authorization time this acts like an
...
suitable for use in the [service mesh](/docs/connect). They are usable
...
3. The [service mesh CA roots endpoint](/api/connect/ca#list-ca-root-certificates) exposes just the public TLS certificate which other systems can use to verify the TLS connection with Consul.
