...
| `peering` &nbsp; &nbsp; &nbsp;        | Controls access to cluster peerings in the [Cluster Peering API](/api-docs/peering). See [Peering Rules](#peering-rules) for details.                                                                           | No     |
...
Mesh rules are scoped to an Admin Partition in Consul Enterprise. Therefore they can be nested in an
...
Peering rules are scoped to an Admin Partition in Consul Enterprise. Therefore they can be nested in an
[admin partition rule](#admin-partition-rules) but not a [namespace rule](#namespace-rules).
...
See [Admin Partition Rules](#admin-partition-rules) for another example rule that uses the `peering` resource.
