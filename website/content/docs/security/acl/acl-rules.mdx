...
When using prefix-based rules, the most specific prefix match determines the action. In a real-world scenario, a combination of rules would be combined to create a flexible policy. Each team or business unit would use tokens based on polcies that enforce several rules, for example:
...
In the following example, the agent has write access to the `ex-namespace` namespace, as well as namespaces prefixed with `exns-` in the `example` partition.
