...
| --------------------------------------------------------------------- | ---------- | ---------- | ----------------------------------------------------------------------------------------- |
| [`acl_datacenter`](/docs/agent/options#acl_datacenter)                | `REQUIRED` | `REQUIRED` | Root control that enables ACLs by defining the authoritative Consul datacenter for ACLs |
...
| ----------------------------------------------------------------------------- | ---------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`acl_agent_root_token`](/docs/agent/options#acl_agent_root_token_legacy) | `OPTIONAL` | `OPTIONAL` | Special token that can be used to access [Agent API](/api/agent) when the ACL datacenter isn't available, or servers are offline (for clients); used for setting up the cluster such as doing initial join operations, see the [ACL Agent Root Token](#acl-agent-root-token) section for more details |
...
| [`acl_root_token`](/docs/agent/options#acl_root_token_legacy)             | `REQUIRED` | `N/A`      | Special token used to bootstrap the ACL system, see the [Bootstrapping ACLs](#bootstrapping-acls) section for more details                                                                                                                                                                                |
...
#### ACL Agent Root Token

Since the [`acl_agent_root_token`](/docs/agent/options#acl_agent_root_token_legacy) is designed to be used when the Consul servers are not available, its policy is managed locally on the agent and does not need to have a token defined on the Consul servers via the ACL API. Once set, it implicitly has the following policy associated with it (the `node` policy was added in Consul 0.9.0):
...
2. An ACL root token of "b1gs33cr3t"; see below for an alternative using the [/v1/acl/bootstrap API](/api/acl/acl#bootstrap-acls)
...
The [`acl_root_token`](/docs/agent/options#acl_root_token) will be created
as a "management" type token automatically. The
[`acl_root_token`](/docs/agent/options#acl_root_token) is only installed when
a server acquires cluster leadership. If you would like to install or change the
[`acl_root_token`](/docs/agent/options#acl_root_token), set the new value for
[`acl_root_token`](/docs/agent/options#acl_root_token) in the configuration
...
to make the initial root token, so a token never needs to be placed into a configuration
file. To use this approach, omit `acl_root_token` from the above config and then call the API:
...
It's only possible to bootstrap one time, and bootstrapping will be disabled if a root
...
ACL API, and the ACL root token we set in the previous step:
...
configured with [`acl_agent_root_token`](/docs/agent/options#acl_agent_root_token) to allow
...
- [`acl_agent_root_token`](/docs/agent/options#acl_agent_root_token) is used as
...
need to use the [`acl_agent_root_token`](/docs/agent/options#acl_agent_root_token) to
...
[`acl_root_token`](/docs/agent/options#acl_root_token) configured; it is not
