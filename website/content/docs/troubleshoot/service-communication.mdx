...
When communication between upstream and downstream services in a service mesh fails, you can diagnose the cause manually with one or more of Consul’s built-in features, including [health check queries](/consul/docs/services/usage/checks), [the UI topology view](/consul/docs/observe/telemetry/vm), and [agent telemetry metrics](/consul/docs/agent/telemetry#metrics-reference).
