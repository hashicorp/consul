...
Provide filters on the query string, such as a name and health check status. For example, to query all instances of service `counting` with a healthy passing status, query the following endpoint `/v1/health/service/counting?passing`.
...
For further information related to troubleshooting, check the [Frequently Asked Questions](/consul/docs/troubleshoot/faq) article.

To learn how to troubleshoot service mesh issues, refer to the [Debug service mesh article](/consul/docs/troubleshoot/mesh). If you are experiencing connectivity issues within the service mesh services, check the [Troubleshoot service-to-service communication](/consul/docs/troubleshoot/service-communication) article. Finally, if you are operating Consul at scale, review [our best practices](/consul/docs/manage/scale) to help you fine-tune operations for your networking needs.