...
We recommended practitioners to use the following approach to troubleshooting:
...
The first thing to do would be to gather data. Consul and other tools generate log files, status messages, process lists, data feeds, and other useful information, which you can use when gathering data.

Next, verifying what works is important because Consul sits at the center of other highly complex systems. Making sure that core systems, networking, and services are working as expected can help you narrow down the problem space, and prevent you from spending time troubleshooting the wrong issues. 

Furthermore, Consul is highly configurable. Therefore you'll find it easier if you solve one problem at a time, verify successful operation, and then proceed to the next issue. If necessary, build a smaller system where you can test the specific configuration options or features that seem to be operating incorrectly.
...
These Consul-specific tools help you gather data and verify what is working correctly.
...
You monitor the health check status by visiting the Consul Web UI, or by querying the HTTP API. The `/v1/agent/services` endpoint will return a list of all services registered in the catalog.

```shell-session
$ curl "http://localhost:8500/v1/agent/services"
```

Filters can be provided on the query string, such as a name and status. For example, to query all instances of service `counting` with a healthy passing status, query the following endpoint `/v1/health/service/counting?passing`.
...
The `dig` tool is a command line application that interacts with DNS servers. To retrieve details about a DNS record in Consul, pass the `-p` flag with the port number of the Consul DNS server (default is `8600`); as well as the address of the Consul DNS server, preceeded by the `@` symbol. The following example queries directly the Consul DNS server running on the current host about the `counting` service.
...
If you have configured [DNS forwarding](/consul/tutorials/networking/dns-forwarding), you may be able to use Consul-specific domain names to communicate to the service. For example, `curl http://counting.service.consul/`.
...
The `ping` command is a simple tool that verifies network connectivity to a host, if the specific host is configured and allowed to respond to `ping` requests.
...
- Use `consul members` to list the members of the Consul datacenter
- Use `consul operator raft list-peers` to view the status of the Consul datacenter
- Use `consul monitor` to view log messages from the Consul agent
- Use `consul validate` to validate configuration files
- Use `consul debug` to gather detailed information about the Consul agent
- Use the health check HTTP API endpoint `/v1/agent/services` to view the status of services in the Consul catalog
