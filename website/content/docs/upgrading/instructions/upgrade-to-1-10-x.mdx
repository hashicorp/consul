...
version of Consul prior to 1.8, please follow the link for the version you are on from 
[here](/docs/upgrading/instructions). If you are already on major versions 1.8 or 1.9
...
page. There are notes on that paged about other changes that might cause issues during an upgrade as well.
...
~> When any of the steps below call for upgrading components, we assume you follow
the normal safe upgrade procedure you normally use for upgrades. The basic server
upgrade procedure is documented [here](/docs/upgrading/instructions/general-process).
