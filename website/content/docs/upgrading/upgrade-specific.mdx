...
1.13.0](https://www.envoyproxy.io/docs/envoy/v1.13.0/api-v3/api) so this should
be doable on most earlier versions of Consul+Envoy in advance of the Consul
1.10.0 upgrade.
...
1. Upgrade Envoy sidecars to the latest version of Envoy that is
   [supported](/docs/connect/proxies/envoy#supported-versions) by the current
   running version of Consul and 1.10.0.

1. Determine if you are using the [escape hatch](/docs/connect/proxies/envoy#advanced-configuration)
   feature. If so, rewrite the escape hatch to use the xDS v3 syntax and
   reconfigure/restart to purge v2 elements from any configs.

1. Upgrade Consul (both servers and clients) in the typical manner to 1.10.0.
   At this point the existing Envoy instances will continue to speak the v2
   State of the World protocol to the new Consul instances without issue.

1. Once a Consul client is upgraded, use an updated CLI binary to re-bootstrap
   Envoy using [`consul connect envoy`](/commands/connect/envoy) and restart
   that Envoy on the new config so that it switches to the v3 Incremental
   protocol.
