...
[ACL templated policies](/consul/docs/security/acl#templated-policies) were added to 1.17.0 to simplify obtaining the right permissions for ACL tokens. When performing [rolling upgrades](/consul/tutorials/datacenter-operations/upgrade-federated-environment#server-rolling-upgrade), if an older consul version (prior to 1.17.x) is presented with a token 
that was created with a newer consul version (1.17.x and above) that contains templated policies, the templated policies field would not be recognized hence your token might not have the necessary permissions expected on the older version of consul.
