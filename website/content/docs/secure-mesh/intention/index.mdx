...
If the destination service uses TCP or any non-HTTP-based protocol, then intentions can control traffic based on identities encoded in mTLS certificates. Refer to [Mutual transport layer security (mTLS)](/consul/docs/architecture/data-plane/connect#mutual-transport-layer-security-mtls) for additional information.
