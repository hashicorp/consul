...
Consul supports prepared queries for services registered with a single port. You cannot use prepared queries if you register a service with multiple ports using [the `ports` parameter in a service definition](/consul/docs/reference/service#port).
