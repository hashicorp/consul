...
@include 'feature/consul-dns.mdx'
...
- [`ports.dns`](/consul/docs/reference/agent#dns_port): By default, Consul does not use port `53`, which is typically reserved for the default port for DNS resolvers, because it requires an escalated privilege to bind to. 
...
- [advertise-wan](/consul/docsreference-cli/agent#_advertise-wan)
...
Responses to pointer record (PTR) queries, such as `<ip>.in-addr.arpa.`, always use the [primary domain](/consul/docs/reference/agent#domain) and not the alternative domain.
