...
Consul Enterprise includes the admin partition concept, which is an abstraction that lets you define isolated administrative network areas. Refer to [Admin Partitions](/consul/docs/segment/admin-partition) for additional information.
...
- The `cluster-peer` name is optional, and specifies the [cluster peer](/consul/docs/link/cluster-peering) whose [exported services](/consul/docs/connect/config-entries/exported-services) should be the target of the query.
