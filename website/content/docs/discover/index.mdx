...
Consul's service discovery features are enabled by default.

## Introduction

Service discovery is one of the core Consul use cases. When a service registers with Consul, the catalog records the address of each service's node. Consul then updates an instance's catalog entry with the results of each health check it performs. Consul agents replicate catalog information between each other using the [Raft consensus protocol](/consul/docs/architecture/consensus), enabling high availability service networking through any Consul agent.

Consul's service discovery operations use [Consul DNS addresses](consul/docs/discover/dns) to route traffic to healthy service instances and lookup information about service nodes registered to Consul.
...
- [Consul service discovery on virtual machines (VMs)](/consul/docs/discover/vm)
