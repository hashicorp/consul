...
The policies will be defined using a [`service-resolver`](/consul/docs/connect/config-entries/service-resolver) configuration entry.
...
-> Configuration Info: When using an `http` listener in the ingress gateway configuration, Consul will accept requests to the ingress gateway service only if the hostname matches with the internal naming convention `<service_name>.ingress.*`. If you need the service to be accessible using a different hostname or an IP, please refer to the ingress gateway [configuration entry](/consul/docs/connect/config-entries/ingress-gateway#hosts) documentation.
