...
Auto scaling groups based on automatic removal of Consul server nodes might cause severe outages to your Consul datacenter. To mitigate the risks associated with automatic scaling of Consul nodes refer to [Prevent 'No cluster leader' issues](#prevent-no-cluster-leader-issues).
...
## Prevent 'No cluster leader' issues

Consul servers rely on the [Raft](/consul/docs/concept/consensus) consensus algorithm to ensure consistency of the cluster's state. 

Consensus algorithms are generally only correct if the server group is reconfigured one node at a time, in a rolling fashion. This means that after every node is added or removed permanently, the cluster should be allowed to get back to a healthy state, ideally with all nodes healthy but at least a with a valid quorum, before any other action is taken.

Consul's [autopilot server health](/consul/docs/manage/scale/autopilot#server-health-checking) makes use of those rules to determine the server cluster's health.

Unexpected events or automations can break those rules and lead to different issues, from relatively innocuous temporary unavailability, to quorum loss (requiring manual recovery) or even to data loss/corruption.

Example of these events and automation include:
- VM(s) failure in your datacenter
- Node(s) failure in K8s cluster
- AWS [Auto Scaling Group](https://docs.aws.amazon.com/autoscaling/ec2/userguide/auto-scaling-groups.html) or [Spot Instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-spot-instances.html)
- Azure [VM Scale Sets](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/) or [Spot VMs](https://learn.microsoft.com/en-us/azure/virtual-machines/spot-vms)
- GCP [Managed Instance Group](https://cloud.google.com/compute/docs/instance-groups#managed_instance_groups) or [SpotVms](https://cloud.google.com/compute/docs/instances/spot)/[Preemptible VMs](https://cloud.google.com/compute/docs/instances/preemptible)

In all these cases, the sudden removal of Consul server agents, might result in having the servers moved into `failed` states and to jeopardize the Raft peer set.

### Improve resiliency

While outages can always happen there are a few best practices that might help making your Consul datacenter more resilient.
...
Having automated scaling of the Consul server nodes based on load or usage is, in general, not a good idea if not backed by some logic that prevents the cluster to lose the leader or the quorum.

A way to improve your datacenter resiliency, and to leverage automatic scaling up and down, is to use autoscaling for [read replicas](/consul/docs/manage/scale/read-replica) and [redundancy zones](/consul/docs/manage/scale/redundancy-zone).

This will provide the ability to undergo read-heavy workload periods without risking the stability of the overall cluster.

#### Set `leave_on_terminate`

Setting [`leave_on_terminate`](/consul/docs/reference/agent/configuration-file/general#leave_on_terminate) to `true` for server nodes causes the agent that receives a `TERM` signal to send a leave message to the rest of the cluster and gracefully leave.

While this setting does not fully shield your cluster from outages due to sudden failure of nodes, with no opportunity to graceful leave, it makes the automatic scaling less impactful.

<Note>

Setting `leave_on_terminate=true`, all server instances, including those that are simply restarting the Consul agent process to make a configuration change, will need to leave the cluster, rejoin it, and download the snapshot from leader before being able to be functional. This might lead to slower restarts.

</Note>

#### Set `min_quorum`

Production datacenter should always set [`min_quorum`](/consul/docs/reference/agent/configuration-file/bootstrap#min_quorum) to allow Consul Autopilot to stop pruning dead servers when this minimum is reached.

### Recover from outages

Even with all the mitigation settings in place is always possible to end up in a quorum loss situation where manual intervention is required.

Refer to [manual recovery using peers.json](/consul/tutorials/operate-consul/recovery-outage#manual-recovery-using-peers-json) to learn how to recover a Consul datacenter from an inconsistent state.