...
- If [token persistence was enabled on client agents](/consul/docs/reference/agent/configuration-file#acl_enable_token_persistence) when the snapshot was captured, then the client agents will resume function after you restore the cluster's server agents.
- If [the ACL token is specified directly in the client agent configuration](/consul/docs/reference/agent/configuration-file#acl_tokens_agent) when when the snapshot was captured, then the client agents resume function after you restore the cluster's server agents.
