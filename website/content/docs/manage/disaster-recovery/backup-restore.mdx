...
In addition, there are some situations where restoring from a backup is the only method to restore a Consul datacenter's previous functionality. As a result, we recommend creating backups on a regular basis to limit the Consul datacenter's potential downtime.
...
The snapshot restore process needs to run only one time. If the target Consul server is not the cluster leader, it streams the snapshot data contents to the leader. The Raft consensus protocol ensures that all servers restore the same state.
