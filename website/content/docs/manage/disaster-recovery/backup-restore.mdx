...
When restoring a snapshot on a new datacenter make sure node names are unique and different from those of the datacenter you saved the snapshot from. Otherwise you might incur in restore issues. Read more at [Snapshot restore error](/consul/docs/error-messages/consul#snapshot-restore-error).
