...
- [Automated backups](/consul/docs/manage/automated-backup). Run an automated Consul snapshot agent to regularly backup your datacenter to a cloud storage bucket.
- [Read replicas](/consul/docs/manage/read-replica). Deploy additional Consul server agents to improve catalog response time at scale.
- [Redundancy zones](/consul/docs/manage/redundancy-zone). Deploy Consul servers across availability zones in a single cloud region in case a single physical data center fails.
...
If a datacenter goes down, you can use snapshots to restore a Consul cluster's operations and registered services. For more information, refer to [disaster recovery](/consul/docs/disaster-recovery).