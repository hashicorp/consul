...
We recommend you create a dedicated [ACL token with DNS permissions](/consul/docs/secure/acl/token/dns) for the Consul DNS proxy. The Consul DNS proxy requires these ACL permissions.
