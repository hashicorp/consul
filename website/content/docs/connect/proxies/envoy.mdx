...
Consul servic emesh has first class support for using
...
Consul's first-class configuration over time, however some advanced users will
...
- `protocol` - The protocol the service speaks. Consul's Envoy integration
...
- `envoy_extra_static_listeners_json` - Similar to
  ...
  Can be used to setup limited access that bypasses service mesh mTLS or
