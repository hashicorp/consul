...
You can configure Consul Dataplane to automatically configure and manage an Envoy process that generates the bootstrap configuration file on startup. To configure how Consul Dataplane starts Envoy, refer to the [Consul Dataplane CLI reference](/docs/connect/dataplane/consul-dataplane).

### Control bootstrap configuration from proxy configuration 
Consul service mesh can only specify some Envoy configuration options, such as metrics and tracing sinks, can through the bootstrap configurations. You can control some parts of the bootstrap configuration by specifying Envoy proxy configuration options. This requires Consul 1.5.0 or later.
...
When connected to a Consul client agent, you can place the configuration in the `proxy.config` field of the
