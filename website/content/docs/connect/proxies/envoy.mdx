...
Envoy requires an initial bootstrap configuration file that is created in one of the two following
ways:

- When using a Consul Client agent, the easiest way to create the Envoy bootstrap configuration is
  using the [`consul connect envoy` command](/commands/connect/envoy). The command can either output
  the bootstrap configuration directly to stdout, or generate the configuration and issue an `exec`
  command to the Envoy binary as a convenience wrapper. For more information about using `exec` to
  bootstrap Envoy, refer to [Exec Security
  Details](/consul/commands/connect/envoy#exec-security-details).
- When using Consul Dataplane, an Envoy process is automatically configured and managed by Consul
  Dataplane. To configure how Consul Dataplane starts Envoy, refer to the [Consul Dataplane CLI
  reference](/docs/connect/dataplane/consul-dataplane).
...
field of a [proxy service definition](/docs/connect/registration/service-registration). When using a
Consul Client agent, this configuration can be placed in the `proxy.config` field of the
