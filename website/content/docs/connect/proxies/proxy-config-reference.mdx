...
| `destination_sameness_group` | String value that specifies the name of the sameness group containing the upstream service. | Required to make requests to within a sameness group when `DefaultForFailover` is set to `false` in the [sameness group configuration entry](/consul/docs/connect/config-entries/sameness-group). | None |
...
<CodeTabs heading="Example of dialing remote upstreams in a sameness group">

```hcl
destination_name = "api"
destination_sameness_group = "product-group"
