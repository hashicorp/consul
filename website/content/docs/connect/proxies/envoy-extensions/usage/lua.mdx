...
~> **Note:** Although you can apply `EnvoyExtensions` to `ProxyDefaults`, this may have unintended consequences. We recommend enabling `EnvoyExtensions` with `ServiceDefaults` in most cases.
