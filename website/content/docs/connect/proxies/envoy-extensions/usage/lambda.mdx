...
The LambdaEnvoy extension has the following arguments:
...
To use the Lambda Envoy extension, you must configure and apply a `service-defaults` configuration entry. The name of this entry will be used as the Consul service name for the Lambdas in the catalog. Downstream services will use this name to invoke the Lambda.

The following example configures the Lambda Envoy extension to create a service “`lambda` in the mesh that can invoke the associated Lambda function.
...
~> **Warning:** You can only enable the Lambda extension through `service-defaults`.
