...
You can use the asterisk (`*`) wildcard to specify multiple destination services. Refer to [Precedence and match order](/consul/docs/connect/intentions/intentions#precedence-and-match-order) for additional information.  
...
### Define the service defaults configuration entry
...
[service defaults configuration entry](/docs/connect/config-entries/service-defaults) 
or globally in a [proxy defaults configuration entry](/docs/connect/config-entries/proxy-defaults).
