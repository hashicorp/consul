...
| `inbound_worker_load_balancing` | Boolean value that describes whether inbound connections for the service should be evenly distributed across proxy worker threads. <br/>This typically helps performance when long-lived high-traffic connections are traversing the proxy. The tradeoff is that establishing sockets will cost more CPU time. <br/>Supported when using Envoy for the proxy. | Optional | `false` |
...
| `outbound_worker_load_balancing` | Boolean value that describes whether outbound connections for the upstream should be evenly distributed across proxy worker threads. <br/>This typically helps performance when long-lived high-traffic connections are traversing the proxy. The tradeoff is that establishing sockets will cost more CPU time. <br/>Supported when using Envoy for the proxy. | Optional | `false` |
