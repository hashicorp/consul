...
You can limit request rates for all traffic to a service, as well as set rate limits for specific URL paths on a service. When multiple rate limits are configured on a service, Consul applies the limit configured for the first matching path. As a result, the  maximum RPS for a service is potentially the rate limit configured for that service or path multiplied by the number instances deployed for that service. 

## Requirements

Consul v1.17.x or later
