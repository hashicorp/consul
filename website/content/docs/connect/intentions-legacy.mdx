...
description: >-
  Intentions define access control for services in the Consul service mesh and are used to
...
Intentions define access control for services in the Consul service mesh and are used
...
default ACL policy is "deny all", then all service mesh connections are denied by
default.
...
service registered that wants to join the service mesh needs `intentions:read`
