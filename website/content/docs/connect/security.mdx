---
layout: docs
page_title: Consul Service Mesh - Security
description: |-
  Consul service mesh enables secure service-to-service communication over mutual TLS.
  This provides both in-transit data encryption as well as authorization.
  This page will document how to secure the service mesh.
---
...
documents how to secure the service mesh. To try the service mesh locally, complete the
...
setting up the service mesh in production, review the
[Service Mesh Production Checklist tutorial](https://learn.hashicorp.com/tutorials/consul/service-mesh-production-checklist?utm_source=consul.io&utm_medium=docs).
...
affect the service mesh since requests must also always contain a valid ACL token.
...
### Prevent Non-Service Mesh Traffic to Services ((#prevent-non-connect-traffic-to-services))

For services that are using
[sidecar proxies](/docs/connect/proxies)
...
will not be encrypted or authorized via Consul service mesh.
