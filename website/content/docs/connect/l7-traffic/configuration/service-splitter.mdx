...
The `service-splitter` configuration entry kind (`ServiceSplitter` on Kubernetes) splits incoming requests across different subsets of a single service (like during staged canary rollouts), or across different services (like during a v2 rewrite or other type of codebase migration).  Since L7 traffic splitting is only meant for HTTP, you cannot use request headers with a TCP listener.

## ACLs

Configuration entries may be protected by [ACLs](/docs/security/acl).

If you want to read a `service-splitter` configuration entry, you must present an ACL token with read permissions (`service:read`).

If you want to create, update, or delete a `service-splitter` configuration entry, you must present an ACL token with write permissions (`service:write`) for the configuration entry and read permissions (`service:read`) for all services in the [`Splits[].Service`](#splits-service).

For more information, refer to the [`consul config`](/consul/commands/config) page.
...
- [`Splits`](#splits): array of [`ServiceSplit`](#servicesplit) | optional
...
The Kubernetes equivalent is `name` and is defined in the `metadata` field. In the following example, the configuration entry is named `web`:
...
The Kubernetes equivalent is `namespace` and is defined in the `metadata` field. In the following example, the configuration entry is in the `main` namespace:
...
- Type: array of `ServiceSplit`
- Required: optional

The Kubernetes equivalent is `splits` and is defined in the `spec` field.
...
The Kubernetes equivalent is `namespace`.
...
The Kubernetes equivalent is `partition`.

#### Splits.RequestHeaders

The `RequestHeaders` object contains a set of HTTP-specific header modification rules that are applied to requests routed through the split.

The Kubernetes equivalent is `requestHeaders`.
...
The `ResponseHeaders` object contains a set of HTTP-specific header modification rules that are applied to responses routed through the split.

The Kubernetes equivalent is `responseHeaders`.
...
The Kubernetes equivalent is `service`.
...
The Kubernetes equivalent is `serviceSubset`.
...
The Kubernetes equivalent is `weight`.
