...
description: >-
  A service mesh-aware proxy enables unmodified applications to use Consul service mesh. A
...
Consul service mesh is the component shipped with Consul that enables service mesh functionality. The terms _Consul Connect_ and _Consul service mesh_ are used interchangeably throughout this documentation. 
...
Begin by enabling Consul service mesh for your Consul
cluster. By default, Consul service mesh is disabled. Enabling Consul service mesh requires changing
the configuration of only your Consul _servers_ (not client agents). To enable
Consul service mesh, add the following to a new or existing
...
This will enable Consul service mesh and configure your Consul cluster to use the
...
Services and proxies may always register with Consul service mesh settings, but they will
fail to retrieve or verify any TLS certificates. This causes all service mesh-based
connection attempts to fail until Consul service mesh is enabled on the server agents.

Other optional Consul service mesh configurations that you can set in the server
...
If you would like to use Envoy as your Consul service mesh proxy you will need to [enable
...
!> **Security note:** Enabling Consul service mesh is enough to try the feature but doesn't
automatically ensure complete security. Please read the [Consul service mesh production
