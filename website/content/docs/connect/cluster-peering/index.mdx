...
description: >-
  Cluster peering establishes east-west communication between independent clusters in Consul, allowing services to interact across datacenters, regions, and cloud providers. Learn how cluster peering works, its differences with WAN federation for multi-datacenter deployments, and how to troubleshoot common issues.
---

# Cluster peering overview

This topic provides an overview of Consul's cluster peering features. Cluster peering is a strategy to extend Consul operations in an east-west direction by connecting two or more independent Consul clusters so that services deployed to different datacenters can communicate. Cluster peering is enabled in Consul by default.

## Introduction

Consul supports cluster peering connections between two [admin partitions](/consul/docs/enterprise/admin-partitions) _in different Consul datacenters_. Even though Consul requires an Enterprise license to configure server tenancy, deployments without an Enterprise license can still use cluster peering because every datacenter automatically includes a `default` partition. Admin partitions _in the same Consul datacenter_ do not require cluster peering connections because you can export services between them without generating or exchanging a peering token.

The following diagram describes Consul's cluster peering architecture.

![Diagram of cluster peering with admin partitions](/img/cluster-peering-overview.png#light-theme-only)
![Diagram of cluster peering with admin partitions](/img/cluster-peering-overview-dark.png#dark-theme-only)
...
WAN federation and cluster peering are different ways to connect services through mesh gateways so that they can communicate in an east-west direction across datacenters. WAN federation connects multiple datacenters to make them function as if they were a single cluster, while cluster peering treats each datacenter as a separate cluster. As a result, WAN federation requires a primary datacenter to maintain and replicate global states such as ACLs and configuration entries, but cluster peering does not.
...
The following usage documentation describes processes to establish and manage cluster peering connections, manage L7 traffic across datacenters with established cluster peering connections, and create sameness groups across peered clusters. Refer to the documentation for your specific runtime.
...
The following reference documentation is available to help you use Consul's cluster peering features.

- [Cluster peering best practices](/consul/docs/connect/cluster-peering/best-practices)
...
- To manage intentions that specify services in peered clusters, use [configuration entries](/consul/docs/connect/config-entries/service-intentions). The `consul intention` CLI command does not support cluster peering connections.
...
- Consul does not support accessing key/value stores across cluster peers.
