...
Cluster peering is enabled in Consul by default. You can use either the API or CLI, or the Consul UI for most cluster peering operations. However, be aware that the UI does not support exporting services between clusters or creating service intentions, which are required steps for establishing new cluster peering connections.

If you use the HTTP API, refer to the [`/peering` endpoint reference](/consul/api-docs/peering). If you use the CLI, refer to the [`peering` command reference](/consul/commands/peering).
