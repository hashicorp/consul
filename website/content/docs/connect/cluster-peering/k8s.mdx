...
1. Install Consul on Kubernetes on each Kubernetes cluster by applying `values.yaml` using the Helm CLI. With these values,
...
```shell-session
$ helm install ${HELM_RELEASE_NAME} hashicorp/consul --create-namespace --namespace consul --version "0.47.1" --values values.yaml --kube-context $CLUSTER_CONTEXT
```
...
1. Apply the `PeeringDialer` resource to the second cluster.
    ...
    ```shell-session
    $ kubectl apply --filename dialer.yaml
    ```
