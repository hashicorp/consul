...
1. For the service in `cluster-02` that you want to deploy onto the mesh and export, ensure that you add the `"consul.hashicorp.com/connect-inject": "true"` annotation to your service's pods prior to deploying. Below is an example workload with the annotation highlighted that allows the workload to join the mesh. 
