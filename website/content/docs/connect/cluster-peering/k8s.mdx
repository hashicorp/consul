...
1. For the service in `cluster-02` that you want to deploy onto the mesh and evautally export, ensure that you add the `"consul.hashicorp.com/connect-inject": "true"` annotation to your service's pods prior to deploying. Below is an example workload with the annotation highlighted that allows the workload to join the mesh. 
...
1. For the services in `cluster-01` that you want like to deploy and dial `backend` in `cluster-02`, ensure that you add the `"consul.hashicorp.com/connect-inject": "true"` annotation to your service's pods before deploying the workload. To dial the upstream service from an application, ensure that the requests are sent to the correct DNS name as specified in [Service Virtual IP Lookups](/docs/discovery/dns#service-virtual-ip-lookups). Below is an example workload with the annotation highlighted that allows the workload to join the mesh, and config provided to the workload that enables the workload to dial the upstream service using the correct DNS name. 

    <CodeBlockConfig filename="frontend.yaml" hightlight="34">
