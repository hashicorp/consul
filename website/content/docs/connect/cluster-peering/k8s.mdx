...
1. For the service in `cluster-02` that you want to deploy onto the mesh and evautally export, ensure that you add the `"consul.hashicorp.com/connect-inject": "true"` annotation to your service's pods. Below is an example workload with the annotation highlighted that allows the workload to join the mesh. 
