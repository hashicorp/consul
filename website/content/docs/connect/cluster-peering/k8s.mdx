...
1. After provisioning a Kubernetes cluster and setting up your kubeconfig file to manage access to multiple Kubernetes clusters, export the Kubernetes context names for future with `kubectl`. 

  You can discover the context names using either `kubectl config current-context` for the current cluster you are on, or by issuing `kubectl config get-contexts` to get all configured contexts for your kubeconfig file.  
...
1. To establish cluster peering through Kubernetes, create a values.yaml file with the following Helm values. 
  ...
  When generating a peering token from one of the clusters, the address(es) of the load balancer will be used in the peering token, so the peering stream will go through the load balancer in front of the servers. For customizing the addresses used in the peering token, see [`global.peering.tokenGeneration`](/docs/k8s/helm#v-global-peering-tokengeneration).
...
1. For the services in `cluster-01` that you want to access the "backend-service," add the following annotations to the service file. In order dial the upstream service from an application, ensure that the requests are sent to the correct DNS name as specified in [Service Virtual IP Lookups](https://www.consul.io/docs/discovery/dns#service-virtual-ip-lookups). 
