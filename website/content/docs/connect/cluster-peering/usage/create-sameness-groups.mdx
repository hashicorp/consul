...
This topic describes how to create a sameness group, which designates a set of admin partitions as functionally identical in your network. Adding an admin partition to a sameness group enables Consul to recognize services registered to different clusters as instances of the same service when they share a name and namespace.
...
The workflow to create a sameness group requires you to write and apply a set of three configuration entries for each partition that is a member of the group:
...
If you want all services to failover to other instances in the sameness group by default, set `DefaultForFailover=true` and list the group members in the order you want to use in a failover scenario. Refer to [failover with sameness groups](/consul/docs/connect/failover/sameness) for more information.
...
If you separately follow the steps in this documentation to create a cluster peering connection between two partitions and then add those partitions to a sameness group, you write and apply two separate exported services configuration entries: one to export services to the peer, and a second to export services to other members of the group.

If you are peering clusters with the goal of creating a sameness group, it is possible to combine these workflows by using a single exported services configuration entry.

Configure the `Services[].Consumers` block with the `SamenessGroup` field instead of the `Peer` field. Be aware that this scenario requires you to write the `sameness-group` configuration entry to Consul before you apply the `exported-services` configuration entry that references the sameness group.
...
If you separately follow the steps in this documentation to create a cluster peering connection between two partitions and then add those partitions to a sameness group, you write and apply two separate service intentions configuration entries: one to authorize traffic to the peer, and a second to authorize traffic to other members of the group.
...
When `DefaultForFailover=true` in a sameness group configuration entry, additional upstream configuration is not required. If the sameness group is not configured as the default for failover, you must specify `destination_sameness_group` in a connect-proxy upstream configuration to enable the sameness group to function as a service upstream. Refer to [upstream configuration reference](/consul/docs/connect/proxies/proxy-config-reference#upstream-configuration-reference) for more information.
