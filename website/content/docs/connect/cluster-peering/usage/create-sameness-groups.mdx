...
This topic describes how to create a sameness group, which designates admin partitions as functionally identical in your network. Adding an admin partition to a sameness group enables Consul to recognize services registered to different clusters as instances of the same service when they share a name and namespace.
...
Sameness groups are a user-defined set of partitions with established cluster peering connections between each other. These partitions should have identical configurations, including configuration entries.
...
Sameness groups are defined at the partition level. Consul assumes these partitions have identical configurations, including identical service names and identical namespaces. This behavior occurs even when two partitions in the group contain functionally different services that share a common name and namespace. For example, distinct services both named `api` registed to members of a sameness group would lead to errors, as requests will be sent to the incorrect service.
