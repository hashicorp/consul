...
If you are using cluster peering for the first time, the following sections contain steps to configure environmental variables, update the Helm chart, and install the updated Helm chart on each cluster with Consul on Kubernetes.
