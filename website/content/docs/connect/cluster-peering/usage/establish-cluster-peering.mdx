...
Consul's default configuration supports cluster peering connections directly between clusters. In production environments, we recommend using mesh gateways to securely route service mesh traffic between partitions with cluster peering connections. Refer to  [cluster peering best practices: mesh gateway recommendations](/consul/docs/connect/cluster-peering/best-practices#mesh-gateway-recommendations) for more information.

## Create a peering token

To begin the cluster peering process, generate a peering token in one of your clusters. The other cluster uses this token to establish the peering connection. Use `<dc-name>-<partition-name>` as your naming convention, as described in [cluster peering best practices](/consul/docs/connect/cluster-peering/best-practices).
...
1. In `dc1`, use the [`consul peering generate-token` command](/consul/commands/peering/generate-token) to issue a request for a peering token.

  ```shell-session
  $ consul peering generate-token -name dc2-default
  ```

  The CLI outputs the peering token, which is a base64-encoded string containing the token details.

1. Save this value to a file or clipboard to use in the next step.

</Tab>
<Tab heading="HTTP API" group="api">

1. In `dc1`, use the [`/peering/token` endpoint](/consul/api-docs/peering#generate-a-peering-token) to issue a request for a peering token.

  ```shell-session
  $ curl --request POST --data '{"Peer":"dc2-default"}' --url http://localhost:8500/v1/peering/token
  ```

  The CLI outputs the peering token, which is a base64-encoded string containing the token details.

1. Create a JSON file that contains the first cluster's intended peer name and the peering token.
...
1. In the Consul UI for `dc1`, click **Peers**.
1. Click **Add peer connection**.
1. In the **Generate token** tab, enter `dc2-default` in the **Name of peer** field.
...
1. In one of the client agents deployed to `dc2-default`, issue the [`consul peering establish` command](/consul/commands/peering/establish) and specify the token generated in the previous step.

  ```shell-session
  $ consul peering establish -name dc1-default -peering-token <token-from-generate>
  "Successfully established peering connection with dc1-default"
  ```

When you use this CLI command, servers peer their default partitions. To establish peering connections for other partitions through server agents, you must add the `-partition` flag to the `establish` command and specify the partitions you want to peer. For additional configuration information, refer to [`consul peering establish` command](/consul/commands/peering/establish).
...
1. In one of the client agents in `dc2-default` use `peering_token.json` and the [`/peering/establish` endpoint](/consul/api-docs/peering#establish-a-peering-connection) to establish the peering connection. This endpoint does not generate an output unless there is an error.
...
1. In the Consul UI for `dc2`, click **Peers** and then **Add peer connection**.
1. Click **Establish peering**.
1. In the **Name of peer** field, enter `dc1-default`. Then paste the peering token in the **Token** field.
1. Click **Add peer**.
...
An `exported-services` configuration entry makes services available to another admin partition. While it can target admin partitions either locally or remotely, cluster peers always export services between remote partitions. Refer to [exported service consumers](/consul/docs/connect/config-entries/exported-services#consumers-1) for more information.

You must use the Consul CLI or HTTP API to complete this step. The Consul UI does not support `exported-services` configuration entries.

<Tabs>
<Tab heading="Consul CLI" group="cli">

1. Create a configuration entry and specify the `Kind` as `"exported-services"`. The following example exports services between `default` admin partitions. To export services between non-default admin partitions, you must include a `Partition` parameter that contains the name of the local partition the configuration entry applies to.
...
1. Add the configuration entry to your cluster. If necessary, include the `-partition` flag to write the configuration entry to a non-default admin partition.
...
1. Create a configuration entry and specify the `Kind` as `"exported-services"`. The following example exports services between `default` admin partitions. To export services between non-default admin partitions, you must include a `"Partition"` filed that contains the name of the local partition the configuration entry applies to.
  ...
  <CodeBlockConfig filename="peering-config.json" hideClipboard>
...
1. Add the configuration entry to your cluster. If necessary, include the `partition` query parameter to write the configuration entry to a non-default admin partition.

  ```shell-session
  $ curl --request PUT --data @peering_config.json http://127.0.0.1:8500/v1/config
  ```
...
1. Create a configuration entry and specify the `Kind` as `"service-intentions"`. Declare the service on `dc2-default` that can access the service in `dc1-default`. In the following example, the service intentions configuration entry authorizes the `backend-service` to communicate with the `frontend-service` that is hosted on remote peer `dc2-default`:
...
1. Create a configuration entry and specify the `Kind` as `service-intentions`. Declare the service on `dc2-default` that can access the service in `dc1-default`. In the following example, the service intentions configuration entry authorizes the `backend` service to communicate with the `frontend` service that is hosted on remote peer `dc2-default`:
  ...
  <CodeBlockConfig filename="peering-intentions.json" hideClipboard>
...
1. Add the configuration entry to your cluster.
  ...
  ```shell-session
  $ curl --request PUT --data @peering-intentions.json http://127.0.0.1:8500/v1/config
  ```
