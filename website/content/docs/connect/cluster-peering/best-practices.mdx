...
By default, cluster peering connections use mesh gateways in [remote mode](/consul/docs/connect/gateways/mesh-gateway#modes). The `none` mode is invalid for mesh gateways with cluster peering connections.
...
You must configure the Envoy proxies that function as sidecars in your service mesh in order to properly route traffic to peers. You can configure sidecar proxy behavior in either a [service definition](/consul/docs/services/configuration/services-configuration-references) or a [`connect-proxy` CRD](/consul/docs/connect/proxies/proxy-config-reference).
