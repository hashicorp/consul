...
description: >-
  Intentions define access control for services in the mesh and are used to
...
Intentions define access control for services in the service mesh.
They control which services may establish connections or make requests.
Intentions can be managed via the API, CLI, or UI.
...
If the configuration is set `allow`, then all service mesh connections will be allowed by default.
...
Note that `intentions:read` is required for a token that a service in the mesh
uses to register itself or its proxy. If the token used does not
