...
referred to as _north-south_ network traffic because it refers to the flow of
...
1. Create an API gateway configuration entry. The configuration entry includes
   listener configurations and TLS certificates.
...
listener on port `8080` that routes can use to connect external traffic to
