...
* Configure the `exported-services` configuration entry to make services from one admin partition available to one or more other datacenter-local partitions. Refer to the [Exported Services documentation](/consul/docs/connect/config-entries/exported-services) for more details.
  * A service must be exported to a consuming partition for the consuming partition to have access to that service.
  * The mesh gateway service must also be exported to each consuming partition. Otherwise, the consuming partition will not be able to access other services through the mesh gateway.
