...
In addition to the [ACL Configuration](/consul/docs/connect/cluster-peering/best-practices#acl-recommendations) necessary for service-to-service traffic, mesh gateways that route peering control plane traffic must be granted `peering:read` access to all peerings.
...
In addition to the [ACL Configuration](/consul/docs/connect/cluster-peering/best-practices#acl-recommendations) necessary for service-to-service traffic, mesh gateways that route peering control plane traffic must be granted `peering:read` access to all peerings in all partitions.
