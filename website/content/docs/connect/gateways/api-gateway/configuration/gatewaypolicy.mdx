...
This topic provides reference information for the API gateway policy resource, which defines API gateway policies for Consul service mesh on Kubernetes-orchestrated networks.
...
- [`apiVersion`](#apiversion): string | required | must be set to `gateway.networking.k8s.io/v1beta1`
...
- [`spec`](#spec): map | required
   - [`targetRef`](#spec-targetref): map | required 
      ...
      - [`group`](#spec-targetref): string | required | must be set to `gateway.networking.kuberenetes.io`
...
spec:
  targetRef:
    ...
    sectionName: <name of a specific section the policy applies>
  override:
    jwt:
      providers:
  ...
  default:
    jwt:
      providers:
      - name: "<name of the default provider>"
        verifyClaims:
        - path:
          - "roles"
          - "perm"
...
| `group` | Must be set to `gateway.networking.kuberenetes.io` <TODO need help providing a more complete description/>| String | None |
| `sectionName` | Specifies a part of the gateway that the policy applies to. | String | None |

### `spec.override`

Map that contains configurations to apply to listeners when the policy is attached to the gateway. The override settings have precedence over the [`spec.default` configurations](#spec-default).
...
Map that contains JWT verification configurations to apply to listeners when the policy is attached to the gateway. The override settings have  precedence over the [`default` configurations](#listeners-default), as well as any route-specific JWT configurations defined in [`HTTPRouteAuthFilter` configurations](/consul/docs/connect/gateways/api-gateway/configuration/httprouteauthfilter).
...
Specifies a list of JWT provider configurations to apply to listeners when the policy is attached to the gateway. A provider configuration contains the name of the provider and claims. The override settings have precedence over defaults as well as any listener specific configuration. 
...
| `verifyClaim` | Specifies a list of paths and a value that define the claim. Consul verifies requests that match the claims declared in the listener JWT configuration and allow the request through the gateway. The `VerifyClaims` map specifies the following settings: <ul><li>`path`: List of paths to the claims</li><li>`value`: ???</li></ul> | Map | None |
...
Specifies default JWT configurations to apply to listeners when the policy is attached to the gateway. You can specify override configurations that have precedence over default configurations. Refer to the [`spec.override.jwt`](#spec-override-jwt) for details.
...
Specifies default JWT provider configurations to apply to the listener when the policy is attached to the gateway. A provider configuration contains the name of the provider and claims. You can specify override configurations that have precedence over default configurations. Refer to the [`spec.override.jwt.providers`](#spec-override-jwt-providers) for details.
...
In the following exampl, all requests through the gateway must have the `api.apps.organization.com` audience. Additionally, requests through the gateway must have a `user` role by default. 
