...
This topic provides reference information for the gateway routes configuration entry. Refer to [Consul API Gateway Configuration Overview]() for information about configuring API gateways in Kubernetes environments.
...
- [`Filters`](#rules-filters): map | no default
  - [`Headers`](#rules-filters-headers): list | no default
    - [`Add`](#rules-filters-headers): map | no default
    - [`Remove`](#rules-filters-headers): list | no default
    - [`Set`](#rules-filters-headers): map | no default
  - [`URLRewrites`](#rules-filters-urlrewrites): list | no default
    - [`Path`](#rules-urlrewrites-path): map | no default
- [`Matches`](#rules-matches): list | no default
  - [`Path`](#rules-matches-path): map | no default
    - [`Match`](#rules-matches-path): string | no default
    - [`Value`](#rules-matches-path): string | no default
...
Specifies the list of gateways that this route should be bound to.
...
Specifies the name of the listener on the api-gateway. If left empty, this will bind this route to all listeners on the parent gateway
...
Specifies the list of HTTP-based routing rules that this route should use for constructing a route table.
...
Specifies the matching criteria used in the routing table. If an incoming request matches the given HTTPMatch configuration, then traffic is routed to services specified in the [`Rules.Services`](#rules-services) field.
...
Specifies how a match is done on a request’s headers
...
Specifies the name of the header to match on
...
Specifies the value of the header to match on
...
Specifies the http method to match on
...
- Default: none
- This field is not required.
- Data type: map which contains the following fields
  `Match`
  `Value`
...
Specifies the value of the path to match on
...
Specifies how a match is done on a request’s query parameters
...
Specifies type of match for query parameters, `exact`, `present`, or `regex`
...
Specifies the name of the query parameter to match on
...
Specifies the value of the query parameter to match on
...
Specifies the name of an HTTP-based service this should route to.
...
Specifies the proportion of requests forwarded to the specified service. The proportion is determined by dividing the value of the weight by the sum of all weights in the service list. For non-zero values, there may be some deviation from the exact proportion depending on the precision an implementation supports. Weight is not a percentage and the sum of weights does not need to equal 100.
