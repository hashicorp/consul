---
layout: docs
page_title: HTTPRouteAuthFilter configuration reference
description: Learn how to configure the `HTTPRouteAuthFilter` resource, which defines behaviors for gateway listeners that are attached to specific HTTP routes.
---

# HTTPRouteAuthFilter configuration reference
...
- [`kind`](#kind): string | required | must be set to `HTTPRouteAuthFilter`
...
Specifies the type of configuration entry to implement. Must be set to `HTTPRouteAuthFilter`.

#### Values

- Default: None
- This field is required.
- Data type: String value that must be set to `HTTPRouteAuthFilter`.
...
Map that contains JWT verification configurations to apply to listeners when the filter is attached to the route. Route-specific settings have precedence over the `default` configurations, but not `override` JWT configurations, defined in the [`GatewayPolicy`](/consul/docs/connect/gateways/api-gateway/configuration/gatewaypolicy) resource.
