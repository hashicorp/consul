...
- `spec.targetRef.group`: Specifies ??
- `spec.targetRef.sectionName`: Specifies ??
...
Create an `HTTPRouteAuthFilter` values file and configure the following fields. Refer to [`HTTRouteAuthFilter` configuration reference](/consul/docs/connect/gateways/api-gateway/configuration/httprouteauthfilter) for details.  
...
- `extensionRef.group`: Specifies ??
- `extensionRef.kind`: Specifies the type of extension reference to attach to the route. Must be `HTTPRouteAuthFilter`
