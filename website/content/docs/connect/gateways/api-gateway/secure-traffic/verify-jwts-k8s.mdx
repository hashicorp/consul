...
- Listener defaults: Define basic defaults that apply to all routes attached to a listener via GatewayPolicy attachment.
...
Below is an HTTPRoute that references the `auth-filter` RouteAuthFilter for the `/admin` path. With this route the effective JWT configuration is that every request to `api-gateway-fqdn:3002/admin` must be signed
