---
layout: docs
page_title: How Consul Service Mesh Works
description: >-
  This page details the internals of Consul service mesh: mutual TLS, agent caching
  and performance, intention and certificate authority replication.
---

# How Consul Service Mesh Works

This page details the inner workings of some of Connect service mesh's core features.
Understanding how these features work isn't a prerequisite for using the service mesh,
but will help you build a mental model of what's going on under the hood, which
may help you reason about the service mesh's behavior in more complex deployment
scenarios.

To try using the service mesh locally, complete the [Getting Started with Consul service
...
This enables services in the mesh to establish and accept connections with
...
All API calls used by the service mesh typically respond in microseconds
and impose minimal overhead to existing services. To accomplish this,
these API calls to the [agent connect endpoints](/api/agent/connect)
are made to the local Consul agent over a loopback interface, and the endpoints
implement local caching, background updating, and support blocking queries.
Most API calls will operate on purely local in-memory data.

## Agent Caching and Performance

To enable fast responses on endpoints such as the service mesh's [agent connect
API](/api/agent/connect), the Consul agent locally caches most service mesh-related
...
the service mesh has not been used at all, the cache will have no data. On first request,
...
With service mesh enabled, you'll likely see increased memory usage by the
local Consul agent. The total memory is dependent on the number of intentions
related to the services registered with the agent participating in the service mesh.
The other data (leaf certificates and public CA certificates)
...
The primary datacenter also acts as the root Certificate Authority (CA) for the service mesh.
...
this intermediate to sign new service mesh certificates in the secondary datacenter
