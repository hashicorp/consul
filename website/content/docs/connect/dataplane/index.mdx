...
- Dataplanes on AWS Elastic Container Services (ECS) requires Consul ECS v1.0.0 and newer. 
...
As a result, small deployments require fewer resources, overall. For especially large deployments or deployments that expect to experience high levels of churn, consider the following impacts to your network's performance:
...
**Additional environment and runtime support**: Consul on Kubernetes versions *prior* to 1.0 (Consul 1.14) require the use of hostPorts and DaemonSets for client agents, which limits Consul’s ability to be deployed in environments where those features are not supported.
...
- [Deploy Consul with the Terraform module](/consul/ecs/install-kubernetes)
...
- Consul API Gateway is supported on Kubernetes.
