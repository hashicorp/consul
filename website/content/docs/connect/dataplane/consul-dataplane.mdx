...
- `-consul-dns-bind-addr` - The address that will be bound to the consul dns proxy. Default is `"127.0.0.1"`.
- `-consul-dns-bind-port` - The port the consul dns proxy will listen on. By default -1 disables the dns proxy Default is `-1`.
...
- `-telemetry-prom-merge-port` - The port to serve merged Prometheus metrics. Default is `20100`.
- `-telemetry-prom-retention-time` - The duration for Prometheus metrics aggregation. Default is `1m0s`.
- `-telemetry-prom-scrape-path` - The URL path where Envoy serves Prometheus metrics. Default is `"/metrics"`.
- `-telemetry-prom-service-metrics-url` - Prometheus metrics at this URL are scraped and included in Consul Dataplane's main Prometheus metrics.
