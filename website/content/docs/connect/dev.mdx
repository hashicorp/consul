---
layout: docs
page_title: Service Mesh - Development and Debugging
description: >-
...
# Developing and Debugging Services in the Mesh
...
Restricting access to services only via the service mesh ensures that the only way to
...
## Connecting to Services in the Mesh ((#connecting-to-connect-only-services))

As an example, let's assume that we have a PostgreSQL database running that
we want to connect to via `psql`, but the only non-loopback listener is the
service mesh listener. Let's also assume that we have an ACL token to identify as
