...
traffic for services in the mesh are directed through the Envoy proxy. This makes it more likely that traffic is secure
and only reaches intended destinations since the proxy can enforce security and policy like TLS and Service Intentions.

Previously, service mesh users would need to explicitly define upstreams for a service as a local listener on the Envoy
...
registration](https://www.consul.io/docs/connect/registration/service-registration) on VMs and via the
[annotation](https://www.consul.io/docs/k8s/connect#consul-hashicorp-com-connect-service-upstreams) in Kubernetes.

To support transparent proxying, Consul now supports a command `consul connect redirect-traffic` to redirect traffic
through an inbound and outbound listener on Envoy. It also watches Service Intentions and configures the Envoy proxy
with the appropriate upstream IPs. In Consul on Kubernetes, the traffic redirection command is automatically set up via
an init container.
...
* Service Intentions should be set up to allow intended services to connect to each other.
...
* For a service on a VM to be a part of the service mesh, it needs to run a Connect Envoy proxy.
* The `consul connect redirect-traffic` command needs to be run on the VM to set it up to redirect all inbound and
outbound traffic to that VM through the Envoy proxy.
* Services need to be registered with Consul and Service Intentions should be set up to allow intended services to
...
* "direct" - Explicitly disables configuring Envoy as a transparent proxy, falling back to only configuring explicit upstreams.
* "transparent" - Explicitly enables configuring Envoy as a transparent proxy.
...
* For services on VMs, transparent proxy is only supported if there is one service per VM. This is because the traffic
redirection rules are applicable to the whole VM and are responsible for directing traffic from one service to it’s
sidecar proxy.
...
inbound and outbound listener on the Envoy proxy. The proxy will be configured to know how to route traffic to the
appropriate upstream services based on [Service
Intentions](https://www.consul.io/docs/connect/config-entries/service-intentions). This means Connect services no longer
...
[ServiceIntention](https://www.consul.io/docs/connect/config-entries/service-intentions) allowing it to talk to
...
Envoy connect proxy. Then, to enable transparent proxy mode to reach this service, you can set apply a service defaults
...
~> **Note** Only one service is supported per VM. See [Known Limitations](#known-limitations)
