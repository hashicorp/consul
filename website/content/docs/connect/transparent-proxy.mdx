...
Consul configures `iptables` rules for each Pod bound to the Kubernetes Service to redirect all inbound and outbound traffic through an inbound and outbound listener on the sidecar proxy. Consul configures the proxy to route traffic to the appropriate upstream services based on [service
