...
* When dialing headless services the request will be proxied using a plain TCP
 proxy with a connection timeout configured with the [`ConnectTimeout`
 field](/docs/connect/config-entries/service-resolver#connecttimeout) of a
 `service-resolver` configuration entry (with a default of 5s). Currently the
 upstream's protocol and [upstream defaults](/docs/connect/config-entries/service-defaults#upstreamconfig)
 connection timeout are not considered.
