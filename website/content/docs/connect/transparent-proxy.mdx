...
* In a deployment configuration with [federation between Kubernetes clusters](/docs/k8s/installation/multi-cluster/kubernetes),
  services in one Kubernetes cluster must explicitly dial a service in another Kubernetes cluster (Consul datacenter) using the
  ...
  annotation. You cannot use transparent proxy. For example, an annotation of
  `"consul.hashicorp.com/connect-service-upstreams": "my-service:1234:dc2"` will reach an upstream service called `my-service`
  on port `1234` in the datacenter `dc2`.
 
* In the deployment configuration with a
  [single Consul datacenter spanning multiple Kubernetes clusters](/docs/k8s/installation/deployment-configurations/single-dc-multi-k8s),
  ...
  will reach an upstream service called `my-service` in another Kubernetes cluster and on port `1234`.
  Although transparent proxy is enabled, Kubernetes DNS is not utilized when communicating between services existing on separate Kubernetes clusters.
  
* In a deployment configuration with [cluster peering](/docs/connect/cluster-peering), you can use transparent proxy.
...
[Kubernetes DNS](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/) address or via Pod IPs, and that
...
~> Note: In order to use Kubernetes DNS, the Kubernetes service name will need to match the Consul service name. This will be the
