...
It also hardens application security by preventing direct inbound connections that bypass the mesh.

#### Without Transparent Proxy

![Diagram demonstrating that without transparent proxy, applications must "opt in" to connecting to its dependencies through the mesh](/img/consul-connect/without-transparent-proxy.png)
...
1. Upstreams are inferred from service intentions, so no explicit configuration
   is needed.
1. Outbound connections to KubeDNS names "just work" — rules redirect them
   through the proxy.
