...
* Consul 1.10.0+
* Consul Helm chart 0.32.0+. If you want to use the Consul CNI plugin to redirect traffic, Helm chart 0.47.0+ is required. Refer to [Enable the Consul CNI plugin](#) for additional information.
...
The Consul Helm chart installs the CNI plugin, but it is disabled by default. Refer to the Consul on Kubernetes installation instructions for information about enabling the Consul CNI plugin. 
