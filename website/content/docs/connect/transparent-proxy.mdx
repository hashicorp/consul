...
* Consul Helm chart 0.32.0 or later 
* Consul on Kubernetes 0.26.0 or later
* Consul 1.10.0 or later
* If your default ACLs policy is `deny`, you must configure service intentions to explicitly allow services to connect to each other.
Services in the mesh inherit the default ACL policy and will not be able to communicate if the default policy is to deny.
* All worker nodes within a Kubernetes cluster must have the `ip_tables` kernel module running, e.g. `modprobe ip_tables`. 
