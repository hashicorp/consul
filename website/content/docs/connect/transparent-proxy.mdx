---
layout: docs
page_title: Service Mesh - Transparent Proxy
...
Otherwise, all services in the mesh can talk to all other services.
...
traffic will be transparently sent through the proxy. Services in the mesh in Kubernetes are required to have a Kubernetes
...
Intentions](/docs/connect/config-entries/service-intentions). This means services in the mesh no longer
...
for service mesh applications, i.e:
...
When this mode is enabled on the upstream, service mesh certificates will be presented for mTLS and
