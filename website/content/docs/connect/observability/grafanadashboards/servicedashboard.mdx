---
layout: docs
page_title: Dashboard for Consul service mesh metrics and monitoring
description: >-
...
- **Grafana query:** `sum(envoy_server_live{app!="traffic-generator"})`
...
- **Grafana query:** `sum(irate(envoy_cluster_upstream_rq_xx{envoy_response_code_class!="5", envoy_response_code_class!="4", consul_destination_service=~"$service"}[10m])) / sum(irate(envoy_cluster_upstream_rq_xx{consul_destination_service=~"$service"}[10m]))`
...
- **Grafana query:** `sum(irate(envoy_cluster_upstream_rq_xx{envoy_response_code_class=~"4|5", consul_destination_service=~"$service"}[10m])) / sum(irate(envoy_cluster_upstream_rq_xx{consul_destination_service=~"$service"}[10m]))`
...
- **Grafana query:** `sum(rate(envoy_cluster_upstream_rq_time_sum{consul_destination_service=~"$service"}[10m])) / sum(rate(envoy_cluster_upstream_rq_total{consul_destination_service=~"$service"}[10m]))`
...
- **Grafana query:** `sum(increase(envoy_cluster_upstream_rq_xx{envoy_response_code_class=~"4|5", consul_destination_service=~"$service"}[10m])) by(local_cluster)`
...
- **Grafana query:** `sum(rate(envoy_tcp_downstream_cx_rx_bytes_total{}[10m])) by (local_cluster)`
...
- **Grafana query:** `sum(rate(envoy_tcp_downstream_cx_rx_bytes_buffered{}[10m])) by (local_cluster)`
...
- **Grafana query:** `sum(rate(envoy_tcp_downstream_cx_total{}[10m])) by(local_cluster)`
...
- **Grafana query:** `sum(envoy_cluster_upstream_cx_active{app=~"$service"}) by (app)`
...
- **Grafana query:** `sum(envoy_http_downstream_cx_active{app=~"$service"}) by (app)`
...
- **Grafana query:** `sum by(namespace,app,envoy_response_code_class) (rate(envoy_cluster_upstream_rq_xx[5m]))`
...
- **Grafana query:** `sum(rate(envoy_http_downstream_rq_xx{envoy_http_conn_manager_prefix="public_listener"}[5m])) by (namespace, app, envoy_response_code_class)`
