...
When a sameness group is configured as the failover default, sameness group failover takes precedence over all other failover configurations in the cluster.
...
You can specify a sameness group in a prepared query to return service instances from the first memberthat has healthy instances. When a member does not have healthy instances, Consul queries group members following the order of the list of members in the sameness group configuration entry.
