...
- `RootPKIPath` / `root_pki_path` (`string: <required>`) - The path to
  ...
  root certificate and is currently not configurable. When enabling WAN Federation
  over Mesh Gateways, each secondary datacenter must share the same `root_pki_path`
...
- `IntermediatePKIPath` / `intermediate_pki_path` (`string: <required>`) -
  ...
  automatically. When enabling WAN Federation over Mesh Gateways, each secondary 
