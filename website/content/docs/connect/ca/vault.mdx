...
- `RootPKIPath` / `root_pki_path` (`string: <required>`) - The path to
  ...
  of 8760 hours (1 year). 
The root certificate will expire at the end of the specified period. The period is not configurable. When WAN Federation
...
- `IntermediatePKIPath` / `intermediate_pki_path` (`string: <required>`) -
  ...
  automatically. When WAN Federation is enabled, every secondary 
