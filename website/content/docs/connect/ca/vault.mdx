...
- `Token` / `token` (`string: ""`) - A token for accessing Vault.
  ...
  duration has expired. You can also provide this token through an environment variable `VAULT_TOKEN`. Token has to be provided either through the configuration file or through the environment variable. Both cannot be used at the same time.
