---
layout: docs
page_title: Vault as a Consul Service Mesh CA
description: >-
...
# Vault as a Consul Service Mesh CA

Consul can be used with [Vault's PKI secrets engine](https://www.vaultproject.io/docs/secrets/pki) to
manage and sign service mesh certificates.

This page documents the specifics of the Vault CA provider for Consul service mesh.
...
-> **NOTE**: A Learn [tutorial](https://learn.hashicorp.com/tutorials/consul/vault-pki-consul-connect-ca?in=consul/vault-secure) is available to help you configure Vault as the Consul service mesh Certification Authority.
...
- **Vault 0.10.3 or later.** Consul uses URI SANs in the PKI engine which
  ...
  compatible with Consul service mesh.
...
for managing service mesh certificates.
...
granted full control of the Intermediate or Leaf CA for service mesh participants.
...
control of the Root and Intermediate or Leaf CA for service mesh participants.
