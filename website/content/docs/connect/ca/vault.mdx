...
- `Token` / `token` (`string: ""`) - A token for accessing Vault.
  ...
  duration has expired. You can also provide the token through the `VAULT_TOKEN` environment variable. The token must be provided either through the configuration file or through the environment variable, but you cannot provide the token using both methods to at the same time.
