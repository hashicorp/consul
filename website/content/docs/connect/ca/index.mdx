---
layout: docs
page_title: Service Mesh - Certificate Management
description: An overview of the Consul Service Mesh Certificate Authority mechanisms.
---

# Connect Certificate Management

Certificate management in Consul service mesh is done centrally through the Consul
...
as long as the connect subsystem that provides Consul's service mesh capabilities
...
If no specific provider is configured when the connect subsystem is enabled, the built-in
...
updating the connect subsystem to use Vault instead of the built-in CA.
