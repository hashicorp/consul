---
layout: docs
page_title: Built-In CA as a Consul Service Mesh CA
description: >-
  Consul ships with a built-in CA system so that Consul service mesh can be easily enabled
...
Consul ships with a built-in CA system so that Consul service mesh can be
...
If the connect subsystem that provides Consul's service mesh capabilities
[is enabled](/docs/connect/configuration#agent-configuration),
the built-in CA is the default provider used if no CA provider is specified.
The provider can be
...
The built-in CA provider has no required configuration. Enabling the connect subsystem
...
the connect subsystem is enabled - the PrivateKey and RootCert fields have not been set, so those have
