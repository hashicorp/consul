---
layout: docs
page_title: Consul Service Mesh - Native Application Integration - Go
description: >-
  We provide a library that makes it drop-in simple to natively integrate most Go applications
  with the connect subsystem that provides Consul's service mesh capabilities.
  Native integration eliminates the need to deploy a sidecar proxy with the service.
---

# Native Integration into the Consul Service Mesh with Go
...
We provide a library that makes it drop-in simple to natively integrate
most [Go](https://golang.org/) applications with the connect subsystem that provides
Consul's service mesh capabilities. Native integration eliminates the need to deploy
a [sidecar proxy](/docs/connect/proxies) with the application.
This page shows examples
of integrating this library for accepting or establishing Consul service mesh
connections. For most Go applications, native integration requires
just a single line of code excluding imports and struct initialization.

In addition to this, please read and understand the
[overview of Consul service mesh native integrations](/docs/connect/native).
In particular, after integrating applications with Consul service mesh, they must declare
that they accept service mesh connections via the
[connect stanza](/docs/discovery/services#connect) of their service definitions.

## Accepting Connections

Any server that supports TLS (HTTP, gRPC, net/rpc, etc.) can begin
accepting service mesh connections in just a few lines of code. For most
existing applications, converting the server to accept service mesh
connections will require only a one-line change excluding imports and
structure initialization.

The Go library exposes a `*tls.Config` that _automatically_ communicates with
...
`*http.Client`. This client will automatically establish service mesh
