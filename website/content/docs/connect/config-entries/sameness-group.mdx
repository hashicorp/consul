...
When [querying Consul](/consul/docs/services/discovery/dns-static-lookups) DNS using sameness groups, `DefaultForFailover` must be set to `true`.  Otherwise, Consul DNS will return an error.
