...
This page provides reference information for service resolver configuration entries. Configure and apply service resolvers to create named subsets of service instances and their behavior when satisfying upstream requests.
...
- [`Kind`](#kind): string | `service-resolver` | required
...
- [`RequestTimeout`](#requesttimeout): string | `0s`
- [`Subsets`](#subsets): map
...
- [`apiVersion`](#apiversion): string | required | `consul.hashicorp.com/v1alpha1`
- [`kind`](#kind): string | required | `serviceResolver`
...
- [`spec`](#spec): map | required
  ...
  - [`requestTimeout`](#spec-requesttimeout): string | `0s`
...
Subsets = {
 <subset01-name> = {
   Filter      = "<expression.to.match.on == subset01-name>"
   OnlyPassing = true
 }
 <subset02-name> = {
   Filter      = "<expression.to.match.on == subset02-name>"
   OnlyPassing = true
 }
}
...
spec:
  ...
  subsets:
    <subset01Name>:
      filter: <expression.to.match.on == subset01Name>
      onlyPassing: false
    <subset02Name>:
      filter: <expression.to.match.on == subset02Name>
      onlyPassing: false
...
Specifies names for service subsets and the conditions under which service instances belong to each subset. Subsets are defined in the form of a key/value pair, where the key is the subset’s name and the value is a map that contains a filtering expression. If empty, only the unnamed default subset is usable.
...
- Data type: Map containing a key/value pair.
  ...
  - `<VALUE>`: Map that can contain the following fields:
...
- Default: None
- Data type: Map that can contain the following paramters:
    | Parameter                              | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Data type | Default |
    | :------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ------- |
    | `Service`                              | Specifies the name of a service at the redirect’s destination that resolves local upstream requests.                                                                                                                                                                                                                                                                                                                                                                                                                                                    | String    | None    |
    | `ServiceSubset`                        | Specifies the name of a subset of services at the redirect’s destination that resolves local upstream requests. If empty, the default subset is used. If specified, you must also specify at least one of the following in the same `Redirect` map: `Service`, `Namespace`, `Datacenter`                                                                                                                                                                                                                                                                | String    | None    |
...
This map is keyed by the name of the local service subset whose traffic should redirect when it fails. The `"*"` wildcard can be used to apply failovers to any subset.
...
- Default: None
- Data type: Map that can contain the following objects:
- Data type: Map that can contain the following paramters:
    | Parameter                              | Description                                                                                                                                                                                                                                                                                 | Data type | Default |
    | :------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ------- |
...
- Default: None
- Data type: Map containing the following object:
  | Parameter     | Description                                                                                        | Data type | Default |
  | :------------ | :------------------------------------------------------------------------------------------------- | --------- | ------- |
...
- Default: None
- Data type: List that can contain the following maps:
  | Parameter     | Description                                                    | Data type | Default |
  | :------------ | :------------------------------------------------------------- | --------- | ------- |
...
- Data type:  List of maps for the following parameters:
  ...
  | `Terminal`     | Determines if Consul should short circuit the computation of the hash when multiple hash policies are present. If a hash is computed when a terminal policy is evaluated, then that hash is used and subsequent hash policies are ignored. | Boolean  | `false`  |
...
Expose a set of services to another datacenter as a virtual service:
...
#### Failover for federation and cluster peering
...
#### Failover for all service subsets
