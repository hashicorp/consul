  ...
  keys={[
    ...
    },
    {
      ...
      description:
        ...
        'are`random_sampling` (default) or `client_sampling`.'
    },
    {
      ...
      yaml: false,
      description:
        'Target percentage of requests received by the gateway that should be traced. When `TracingStrategy` is set ' +
        'to `client_sampling`, this value is applicable if the `x-client-trace-id` header is set. With ' +
        '`random_sampling` strategy, this value is applicable when `x-client-trace-id` header is not set.'
    },
    {
      name: 'Meta',
