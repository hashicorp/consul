...
description: >-
  The cluster config entry kind allows for configuring global config
  defaults across all services for service mesh proxy configuration. 
  Settings in this config entry apply across namespaces and federated datacenters.
...
The `cluster` config entry kind allows for configuring global config
that applies to all services for service mesh proxy configuration. 
Settings in this config entry apply across namespaces and federated datacenters.
...
- `TransparentProxy` `(TransparentProxyConfig: <optional>)` - Controls configuration specific to proxies in 
  ...
  - `CatalogDestinationsOnly` `(bool: false)` - Determines whether transparent sidecar proxies should 
