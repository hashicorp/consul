...
- [`JWT`](#jwt): map 
  - [`Providers`](#jwt-providers): list of maps
    - [`Provider`](#jwt-providers-provider): string
...
- [`spec`](#spec): map | no default 
  ...
  - [`providers`](#spec-jwt-providers): list of maps
    - [`provider`](#spec-jwt-providers-provider): string
...
Specifies the names of one or more previously configured [JWT provider configuration entries](/consul/docs/connect/config-entries/jwt-provider), which include the information necessary to calidate a JSON web token.
...
- Default: None
- Data type: List of strings that can contain one of the following values:
