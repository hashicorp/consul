...
The following example configures Consul to export the `payments` and `refunds` services from the `billing` namespace of the `finance` admin partition to that partition's `web-shop` peer.
...
When exporting to datacenter-local admin partitions rather than a cluster peer,
the exporter's mesh gateway service must also be exported to the consuming partitions.
If the mesh gateway service is not exported, the consuming partition will not
be able to access other exported services through the mesh gateway.
For more information, refer to [Enabling Service-to-Service Traffic Across Admin Partitions using Mesh Gateways](/consul/docs/connect/gateways/mesh-gateway/service-to-service-traffic-partitions).
...
The exporter's mesh gateway service is also exported to the consuming partitions
by this configuration, enabling the consuming partitions to access other exported services
through the mesh gateway.
