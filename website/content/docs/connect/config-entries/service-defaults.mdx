...
This topic describes how to configure service defaults configuration entries. Configure and apply service defaults to specify common configurations, such as upstreams and gateways, for all services in your cluster. Refer to [Define service defaults](/consul/docs/services/usage/define-services#define-service-defaults) for usage information.
...
- [`service-splitter` configuration entry](/docs/connect/config-entries/service-splitter) 
- [`service-router` configuration entry](/docs/connect/config-entries/service-router) 
...
| Parameter | Description | Data type | Default |
| ---   | ---         | ---       | ---     |
| `Address` <br/> `address` (Kubernetes) | Specifies a list of addresses for the destination. You can configure a list of hostnames and IP addresses. Wildcards are  not supported.  | array | none |
...
<!-- OLD CONTENT - KEEPING FOR NOW JUST IN CASE
