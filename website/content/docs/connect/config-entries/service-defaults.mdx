...
- [`LocalIdleTimeoutMs`](#localidletimeoutms): integer | `0`
...
Specifies the idle timeout for HTTP requests to the local application instance. Applies to HTTP-based protocols only. If not specified, inherits the Envoy default for route idle timeouts.
...
<ConfigEntryReference
  keys={[
    ...
    {
      name: 'LocalIdleTimeoutMs',
      description: ' In milliseconds, the idle timeout for HTTP requests to the local application instance. Applies to HTTP-based protocols only. If not specified, inherits the Envoy default for route idle timeouts (15s).',
      type: 'int: 0',
      yaml: false,
    },
    {
