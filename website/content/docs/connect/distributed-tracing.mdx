...
be implemented in each application, it can't be added by Consul. Once implemented in your applications, adding
distributed tracing to Consul will add the sidecar proxies as spans in the request path.

## Application Changes

Consul alone can't implement distributed tracing for your applications. Each application must propagate the required
headers. Typically this is done using a tracing library.
...
-> On Kubernetes this is a matter of restarting your deployments, e.g. `kubectl rollout restart deploy/deploy-name`.
