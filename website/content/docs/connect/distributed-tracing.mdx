...
1. Requests through [Ingress Gateways](/docs/connect/gateways/ingress-gateway) will not be traced unless the header
   `x-client-trace-id: 1` is set (see [hashicorp/consul#6645](https://github.com/hashicorp/consul/issues/6645)) and
   if `TracingStrategy` is also not set on the config entry for the Ingress Gateway. For more information about the
   latter case, see the documentation for [Ingress Gateway Config Entries](/docs/connect/config-entries/ingress-gateway#available-fields) 
