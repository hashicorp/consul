...
You must enable log verification on any voting server in Enterprise and all servers in OSS becaise the leader writes verification checkpoints.
...
1. Restart each server to apply the changes. The `consul reload` is not sufficient to apply `raft_logstore` configuration changes. 
...
Whenc omplete, log entries for the servers should resemble the following status:
...
- If you wish to permanently enable `wal` on all servers, repeat the steps described in this topic for each server. Even if `backend = "wal"` is set in logs, servers continue to use BoltDB if they find an existing raft.db file in the data dir.