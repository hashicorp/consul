...
This topic describes how to safely configure and test the WAL backend in your Consul deployment. The following steps describe the general process for enabling the WAL `LogStore` backend for a single server. Repeat the steps for each Consul server.
...
- Consul 1.15 or later is required for all servers in the datacenter. Refer to the [standard upgrade procedure](/consul/docs/upgrading/general-process) and the [1.15 upgrade notes](/consul/docs/upgrading/upgrade-specific#consul-1-15-x) for additional information.
...
- Monitor Consul server metrics and logs and sett an alert on specific log events that occur when WAL is enabled.
- Enable WAL in a pre-production environment and run it for a several days before enabling it in production.

## Risks

The likelihood of the following potential risks is low to very low:
...
You must enable log verification on any voting server in Enterprise and all servers in OSS because the leader writes verification checkpoints.
...
1. Restart the server to apply the changes. The `consul reload` is not sufficient to apply `raft_logstore` configuration changes. 
...
When complete, log entries for the servers should resemble the following status:
