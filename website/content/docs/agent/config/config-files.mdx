...
- `acl` ((#acl)) - This object allows a number of sub-keys to be set which
  ...
  - `tokens` ((#acl_tokens)) - This object holds all of the configured
    ...
    - `config_file_registration` ((#acl_tokens_config_file_registration)) - The ACL token this agent
      uses to register services and checks from [service definitions](/docs/discovery/services) and
      ...
      used to register that service or check instead. If the `config_file_registration` token is not
      ...
      This token needs permissions for all services and checks this agent needs to register. For
      example, if there are multiple service definitions in configuration files, then the
      `config_file_registration` token needs multiple `service:write` permissions in order for
