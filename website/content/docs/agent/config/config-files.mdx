...
- `limits`: This block specifies various types of limits that the Consul server agent enforces. Configuring limits does not affect the behavior of Consul agents in client mode.
  ...
  The `kv_max_value_size` and `txn_max_req_len` fields parse values as bytes. For example, specify `524288` to configure a limit of `512KB`.
