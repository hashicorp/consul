...
- `enable_debug` (Defaults to `false`) When set, enables some additional debugging features. Currently, this is only used to
  capture runtime profiling (pprof) data. This setting is only required for clusters without ACL [enabled](#acl_enabled), and requires agent restart.
