...
- `connect` This object allows setting options for the Connect feature.
  ...
  - `enable_serverless_plugin` ((#connect_enable_serverless_plugin)) Determines whether the serverless plugin
    is enabled. Currently, the serverless plugin only supports [AWS
    Lambda](https://aws.amazon.com/lambda/). The [Lambda pages](/docs/lambda) provide
    additional information on invoking Lambda functions from mesh services.
