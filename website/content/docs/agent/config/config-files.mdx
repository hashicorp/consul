...
- `performance` Available in Consul 0.7 and later, this is a nested object that allows tuning the performance of different subsystems in Consul. See the [Server Performance](/consul/docs/install/performance) documentation for more details. The following parameters are available:
  ...
  - `enable_xds_load_balancing` - Load balancing of xDS Sessions on the server. If true, the total number of xds sessions are divided among the available servers. If the number of sessions reaches a threshold, it rejects the connection; hence the XDS client can retry on different server. If false, the server will accept all xDS sessions received on it. This is useful for cases where there is load balancer in front of consul servers, and it is distributing the load across the servers.
