...
- `limits`: This block specifies various types of limits that the Consul server agent enforces. Declaration differs between Consul Agent Server and Consul Agent Client. For Server, use `limits { ... }` and for Client, use `limits = { ... }`. Not adding the `=` sign for the stanza in the Client config could show the error `expected a map, got slice` upon reloading or restarting the service.
  ...
  ~> **Warning:** Both `kv_max_value_size` and `txn_max_req_len` values need to be declared in bytes, e.g. for `512KB` the value would be `524288`.
