...
- `disable_primary_gateway_fallback` Disables the fallback mechanism that
  ...
  This should only be set to true if network connectivity between servers in a
  secondary datacenter and the mesh gateways of the primary datacenter are
  deliberately severed.

  This was added in Consul 1.10.0.
