...
- `connect` This object allows setting options for the Connect feature.
  ...
  - `ca_config` ((#connect_ca_config)) An object which allows setting different
    ...
    - `leaf_cert_ttl` ((#ca_leaf_cert_ttl)) The upper bound on the lease
      ...
      This value cannot be lower than 1 hour or higher than 1 year. Must use hour
      units or smaller, for example, `1d` is not valid but `24h` is.
    ...
    - `intermediate_cert_ttl` ((#ca_intermediate_cert_ttl)) The time to live (TTL) for the
