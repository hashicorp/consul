...
- `tls` Added in Consul 1.12, for previous versions see
  ...
  - `defaults` ((#tls_defaults)) Provides default settings that will be applied
    ...
    - `tls_min_version` ((#tls_defaults_tls_min_version)) This specifies the
      minimum supported version of TLS. Accepted values as of Consul 1.12.0 are
      "TLSv1_0", "TLSv1_1", "TLSv1_2", or "TLSv1_3". This defaults to "TLSv1_2".
      Deprecated values of "tls10", "tls11", "tls12" and "tls13" are currently
      accepted only when using the [deprecated top-level `tls_min_version`
      config](#tls_deprecated_options), will emit a warning during configuration,
      and will be removed in a future release. **WARNING: TLS 1.1 and lower are
      generally considered less secure; avoid using these if possible.**
