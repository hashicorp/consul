...
1. **Consul Servers:** Currently, Consul servers must run inside ECS. In the future, we will support Consul servers running in HashiCorp Cloud Platform and on EC2 VMs.
...
The easiest way to get started is to use the [`dev-server-fargate` example module](https://registry.terraform.io/modules/hashicorp/consul/aws-ecs/latest/examples/dev-server-fargate)
