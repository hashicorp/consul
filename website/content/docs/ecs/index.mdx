...
Consul can be deployed on AWS ECS (Elastic Container Service) using our official
Terraform modules.

## Use Cases

Currently the only supported use case is Service Mesh. ECS Tasks can join the Consul Service Mesh and take advantage
of service mesh features such as zero trust networking and routing policy.

As we iterate and get feedback from users and customers
we will look at supporting additional use cases such as Service Discovery.

## Supported Configurations

- **Infrastructure As Code:** The tasks that you want to add to the service mesh must first be modeled in Terraform.
- **Launch Type:** ECS Tasks can run on either Fargate or EC2.
- **Subnets:** ECS Tasks can run in private or public subnets.
- **Consul Production Readiness:** Currently, Consul production features such as TLS, ACLs, and Gossip Encryption are not supported. This will be added in the future.
- **Consul Servers:** Currently, Consul servers must run inside ECS. In the future, we will support Consul servers running in HashiCorp Cloud Platform and on EC2 VMs.

## Getting Started

The easiest way to get started is to use the [`dev-server` example module](https://registry.terraform.io/modules/hashicorp/consul/aws-ecs/latest/examples/dev-server)
to install a Consul server and two example service mesh tasks onto an ECS cluster.

## Adding Tasks To The Mesh

Adding existing tasks to the mesh is a multi-part process:
