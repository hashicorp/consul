...
# Technical specifications for Consul on AWS Elastic Container Service (ECS)

This topic describes the general requirements and specifications for using Consul service mesh for your ECS workloads. For requirements associated with using the Terraform mesh-task module to deploy Consul service mesh, refer [Deploy Consul with the Terraform module](/consul/docs/ecs/install-terraform). For requirements associated with manually deploying Consul service mesh to your ECS cluster, refer [Deploy Consul manually](/consul/docs/ecs/install-manual). 
...
- **Consul servers**: You can use your own Consul servers running on virtual machines or use HashiCorp Cloud Platform Consul to host the servers for you. 
...
The following table describes the maximum resource usage we observed for each container:
...
Refer to [Architecture](/consul/docs/ecs/reference/architecture) for details about each component.