...
| `timeout` | `string` | optional | Specifies a timeout for outgoing connections in the case of a Script, HTTP, TCP, UDP, or gRPC check. Must be a duration string, such as `10s` or `5m`.  |
