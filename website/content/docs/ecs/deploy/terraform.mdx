...
- Enable [TLS encryption](https://developer.hashicorp.com/consul/docs/security/encryption#rpc-encryption-with-tls) on your Consul servers so that they can communicate securely with Consul containers over gRPC.
...
Add a `module` block to your Terraform configuraiton and specity the following fields:

- `source`: Specifies the location of the `mesh-task` module. This must be set to `hashicorp/consul-ecs/aws//modules/mesh-task`. The `mesh-task` automatically adds the Consul service mesh infrastructure when you apply the Terraform configuration.
...
- `container_definitions`: Specifies a list of [container definitions](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definitions) for the task definition. This is where you include your application containers.

Refer to the [`mesh-task` module reference documentation](https://registry.terraform.io/modules/hashicorp/consul-ecs/aws/latest/submodules/mesh-task?tab=inputs) for information about all options you can configure.
...
For additional information about the `config.tf` file, refer to the [JSON schema reference documentation](/consul/docs/ecs/reference/consul-server-json). 
...
Refer to  [`aws_ecs_service`](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/ecs_service) for a complete configuration reference.
...
Mesh gateways require TLS and gossip encryption to be enabled. Refer to the [Requirements](#requirements) for additional information. 
...
1. Specify the following required inputs:
    ...
    - `kind`: Should be set to `mesh-gateway`
...
Refer to the [module reference documentation](https://registry.terraform.io/modules/hashicorp/consul-ecs/aws/latest/submodules/gateway-task?tab=inputs) for a complete reference.
...
1. On the Consul server, create a policy that grants `acl:write` and `operator:write` access for the controller. The policy allows Consul to generate a token linked to the policy. Refer to [Create a service token](/consul/docs/security/acl/tokens/create/create-a-service-token) for instructions.
...
The following Terraform passes the generated CA certificate ARN to the `mesh-task` module and ensures that the CA certificate and PEM variable are set for both HTTPS and gRPC communication.
...
module "my_task" {
  ...
  acls                      = true
  consul_server_hosts          = "https://consul-server.example.com:8501"
