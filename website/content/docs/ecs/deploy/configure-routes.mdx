...
Add the `upstreams` block to your application configuration ad specify the following fields:

- `destinationName`: Specifies the name of the upstream service as it is registered in the Consul service catalog.  
- `localBindPort`: Specifies the port that the proxy forwards requests to. This must be an unused port but does not need to match the upstream service port.
...
Use an appropriate environment variable in your container defintion to configure your application to call the upstream service at the loopback address. 
