...
[Audit logging](/docs/enterprise/audit-logging) is supported when ACLs are enabled and tasks are configured to use a Consul Enterprise client.
To enable audit logging you must update the startup script to configure the Consul client config file with an `audit` stanza.

The following example shows the additional content to include in the `consul-client` startup script to configure audit logs to be written to `stdout` of the `consul-client` container.
