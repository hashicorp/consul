...
Consul OSS clients can run with Consul Enterprise servers without issue as long as you are
not using features that require Consul Enterprise client support.
Admin Partitions and Namespaces require Consul Enterprise clients.

## Feature Support

Consul on ECS supports the following Enterprise features.
If you are only using features that run on Consul servers, then you can use an OSS client in your mesh-tasks.
If client support is required for any of the features, then you must use an Enterprise client in your mesh-tasks.
...
| Namespaces                        | Yes           | This feature requires Consul Enterprise servers and clients.                                                                                                            |
| Admin Partitions                  | Yes           | This feature requires Consul Enterprise servers and clients.                                                                                                            |
...
* That ACLs are enabled.
* The ACL Controller is running in the ECS cluster.
* Mesh-tasks are using a Consul Enterprise client image.
...
Services are assigned to admin partitions and namespaces through the use of task tags.
...
To support admin partitions and namespaces, a `mesh-task` must use a Consul Enterprise image.
By default the `mesh-task` module uses a Consul OSS image. The module provides
a `consul_image` field that can be used to set the Consul client image to an Enterprise version.
