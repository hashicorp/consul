...
Secure Consul cluster should include the following:
...
and CA certificate for the Consul server in AWS Secrets Manager.
...
To deploy the controller we will use the [`acl-controller` terraform module](https://registry.terraform.io/modules/hashicorp/consul-ecs/aws/latest/submodules/acl-controller?tab=inputs):
...
Once ACL controller is up and running, you can deploy services on the mesh using the [`mesh-task` module](https://registry.terraform.io/modules/hashicorp/consul-ecs/aws/latest/submodules/mesh-task).
Please make sure that you already have the basic configuration for the [Task Module](/docs/ecs/get-started/install#task-module).
We will add additional configuration to that basic configuration to make it production-ready.
