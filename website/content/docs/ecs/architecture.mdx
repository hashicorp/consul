...
The ACL controller is responsible for:
* Automatically provisioning Consul ACL tokens for Consul clients and service mesh services.
* Automatically managing Consul admin partitions and namespaces.
...
The ACL controller automatically performs the following:
* Create its configured admin partition at startup if it does not exist.
* Inspect ECS task tags to discover a task's intended partition and namespace.
  The ACL controller ignores tasks which do not have a matching partition tag.
* Create namespaces when tasks startup. Namespaces are only created if the do not exist.
* Provision ACL tokens and ACL policies that are scoped to the applicable admin partition and namespace.
