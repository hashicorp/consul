...
1. Create a federation Secret in Kubernetes.
1. Update Helm charts of additional clusters with federation Secret.
...
1. Enable mesh gateways in the cluster's Helm chart.
1. Create a `ProxyDefaults` CRD that configures the mesh gateway mode.
1. Apply the `ProxyDefaults` CRD to each cluster.
...
### Cluster peering on Kubernetes usage documentation
