---
id: 033a78cd-f9fb-4f24-a3ed-48265efca6b9
name: Federate multiple datacenters with network areas
short_name: Network areas
products_used:
  - product: consul
    min_version: 0.8.0
description: >-
...
edition: enterprise
read_time: 12
default_collection_context: consul/datacenter-operations
---
...
This tutorial covers the advanced form of federating Consul datacenters using the new network areas capability added in Consul Enterprise version 0.8.0. For the basic form of federation available in the open source version of Consul, check the [basic federation tutorial](/consul/tutorials/networking/federation-gossip-wan) for more details.
...
The [`translate_wan_addrs`](/consul/docs/agent/config/config-files#translate_wan_addrs) configuration provides a basic address rewriting capability.
...
Networks areas can be used alongside the Consul's WAN gossip federation model and the WAN gossip pool. This helps ease migration, and datacenters like the [primary datacenter](/consul/docs/agent/config/config-files#primary_datacenter) are more easily managed via the WAN because they need to be available to all Consul datacenters.
...
In case you are trying to setup ACL replication using the [1.4 ACL system](/consul/docs/security/acl) or enabling Consul service mesh with CA replication, we recommend you to use WAN gossip federation to leverage all Consul's latest functionalities.
