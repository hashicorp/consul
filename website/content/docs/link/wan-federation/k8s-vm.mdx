...
You need to set up additional ACL tokens as needed by the ACL system. Refer to the [ACLs](/consul/docs/secure-consul/acl) docs
...
1. If ACLs are enabled you must create an ACL replication token with the following rules:
   ...
   ```
1. If ACLs are enabled you must also modify the [anonymous token](/consul/docs/secure-consul/acl/tokens#anonymous-token) policy to have the following permissions:
