...
[Federate Multiple Datacenters tutorial](/consul/tutorials/networking/federation-gossip-wan).
...
Using the techniques in this section you will develop prepared queries with failover policies where simply changing application configurations to look up "banking-app.query.consul" instead of "banking-app.service.consul" via DNS will result in automatic geo failover to the next closest [federated](/consul/tutorials/networking/federation-gossip-wan) Consul datacenters, in order of increasing network round trip time.
