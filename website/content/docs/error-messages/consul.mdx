...
If a host does not properly implement half-close, you may see an error message when saving snapshots.
...
This should not affect saving and restoring snapshots.
...
When restoring a snapshot on a new Consul datacenter, if Consul throws the following errors there is a conflict between the `node_name` used in the new datacenter and the `node_id` value for those nodes. 
...
Specifically this means that in the new datacenter, there is at least one node with the same `node_name` of a node in the snapshotted datacenter but with a different `node_id`. This represents a consistency issue for the restore commands that throws the above errors.

Possible workarounds:

1. Always use unique node names for your Consul datacenters so that there is no risk of conflicts (e.g., `consul-server-<random_chars>`). You can configure node names for your Consul agent nodes using the [`node_name`](/consul/docs/reference/agent/configuration-file/node#_node) configuration parameter.

1. Save the UUID from the previous node’s data directory. Re-use that same UUID when bringing up the agent on the new node. You can configure node IDs for your Consul agent nodes using the [`node_id`](/consul/docs/reference/agent/configuration-file/node#_node_id) configuration parameter.
