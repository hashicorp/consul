...
CTS uses Consul's service catalog as a source of truth for all applications running in a given environment. When it detects services are registered or removed from Consul, CTS dynamically applies the necessary changes to your infrastructure using Terraform, in this case, the NGFW's dynamic address group.
