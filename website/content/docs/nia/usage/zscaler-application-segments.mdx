...
1. A Consul datacenter for service networking
...
## Configure ZPA segment group

The following example Terraform configuration shows a minimum segment group required for the ZPA application segment and NIA integration. When your Consul services change, CTS will automatically modify the ZPA application segment's domain entries.
...
- `module` defines the path to the ZPA Terraform NIA module. CTS automatically applies this module whenever it detects changes to the specified Consul service to dynamically create and update ZPA application segments to reflect the service's new status.
...
Run the `consul-terraform-sync start` command to start CTS. This command performs the following operations:
...
Check out the following resource to learn more about network infrastructure automation with CTS and ZScaler.
