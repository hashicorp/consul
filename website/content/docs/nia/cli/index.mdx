...
- `-port=<port>` - (int: 8558) The port that the Consul-Terraform-Sync daemon serves its API. This port is used with the address `http://localhost`. If a different scheme or address is required, use `-http-addr` instead.
- `-http-addr=<address>` - (string: http://localhost:8558) The address and port of the Consul-Terraform-Sync API. The value can be an IP address or DNS address. This can also be specified via the `CTS_HTTP_ADDR` environment variable.
- `-ssl-verify` - (bool: true) Whether to verify TLS/SSL connections to the Consul-Terraform-Sync API. The default value is `true`, as disabling verification is a potential security vulnerability. This can also be specified using the `CTS_SSL_VERIFY` environment variable.
- `-ca-cert=<ca_file>` - (string) The path to a PEM-encoded certificate authority file used to verify TLS/SSL connections. This can also be specified using the `CTS_CACERT` environment variable.
- `-ca-path=<ca_directory>` - (string) The path to a directory of PEM-encoded certificate authority files used to verify TLS/SSL connections. This can also be specified using the `CTS_CAPATH` environment variable.
- `-client-cert=<cert>` - (string) The path to a PEM-encoded client certificate used when the Consul-Terraform-Sync API has `verify_incoming` enabled. This can also be specified using the `CTS_CLIENT_CERT` environment variable.
- `-client-key=<key>` - (string) The path to a PEM-encoded client key for the certificate specified by `-client-cert`. Used when the Consul-Terraform-Sync API has `verify_incoming` enabled. This can also be specified using the `CTS_CLIENT_KEY` environment variable.
