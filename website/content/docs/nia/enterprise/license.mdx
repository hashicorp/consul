...
~> Enabling automatic license retrieval is recommended when using HCP Consul, as HCP Consul licenses expire more frequently than Consul Enterprise licenses. Without auto-retrieval enabled, you will have to restart CTS to load a new license.
...
1. Configure the [`license`](/docs/nia/configuration#license) path option in the configuration file to point to the file containing the license.
...
1. Configure the [`license_path`](/docs/nia/configuration#license_path) option in the configuration file to point to the file containing the license. **Deprecated in CTS 0.6.0 and will be removed in a future release. Use [license block](/docs/nia/configuration#license) instead.**
...
If automatic license retrieval is disabled, then use the following procedure to update the license when it expires or is near the expiration date:
