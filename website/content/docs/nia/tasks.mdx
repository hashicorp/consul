...
Because scheduled tasks trigger on a configured cadence, buffer periods are disabled for scheduled tasks. Any configured `buffer_period` at the global-level or task-level will only apply to dynamic tasks and not to scheduled tasks.
