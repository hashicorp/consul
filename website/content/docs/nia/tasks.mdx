...
A task can also monitor, but not execute on, other variables that provide additional information to the task's module. For example, a task with a catalog-services condition may execute on registration changes, as well as monitor service instances for IP information.

A source input can be specified that implicitly includes variables to be provided to the task’s module. For example, a task can specify a Consul KV source input. The specified keys or key paths would be monitored for changes. Any changes detected would be included as input information for the modules. The details of what values are monitored and what values can execute the task are determined by the module.

-> **The source input block is only supported when using a schedule condition.** Adding a source input block alongside any other type of condition will result in an error. To accomplish a similar behaviour with other condition blocks, use the `source_includes_var` field.
...
- [Services source input](/docs/nia/terraform-modules/#services-source-input): configure through [`task.services`](/docs/nia/configuration#services) or [`source_input "services"`](/docs/nia/configuration#source_input)
- [Catalog services source input](/docs/nia/terraform-modules/#catalog-services-source-input): configure through [`source_input "catalog-services"`](/docs/nia/configuration#source_input)
- [Consul KV source input](/docs/nia/terraform-modules/#consul-kv-source-input): configure through [`source_input "consul-kv"`](/docs/nia/configuration#source_input)
...
* [Long-running mode](/docs/nia/cli#long-running-mode): At the beginning of long-running mode, Consul-Terraform-Sync first passes through a once-mode phase in which all tasks are executed once. Scheduled tasks will trigger once during this once-mode phase even if not on schedule. After once-mode completes, scheduled tasks subsequently trigger on schedule.

* [Inspect mode](/docs/nia/cli#inspect-mode): For inspect mode, the inspected task will display the proposed changes for the tasks in the terminal and exit. Scheduled tasks will display proposed changes that would be made if the task was to run at that time even if not on schedule.

* [Once mode](/docs/nia/cli#once-mode): During once mode, all tasks are triggered once. Scheduled tasks will execute during once mode even if not on schedule.
...
Because scheduled tasks trigger on a configured cadence, buffer periods are disabled for scheduled tasks. Any configured `buffer_period` at the global-level or task-level will only apply to dynamic tasks and not to scheduled task.
