...
To configure a task to trigger on services, you can configure one of two options for a given task:
1. task block's  [`services`](/docs/nia/configuration#services) field: configure a list of services that will trigger the task
2. task block's [service `condition`](/docs/nia/configuration#services-condition) block: configure a regular expression to match on services that will trigger the task

When the `services` condition is configured, Consul monitors the [/health/service/:service](/api-docs/health#list-nodes-for-service) API endpoint for changes. Only healthy and passing service instances are monitored by default, but you can configure the [`filter`](/docs/nia/configuration#filter) parameter in the `services` block to include unhealthy service instances. 

The task executes when the value changes for one or more configured services, such as IP address or new/removed values in the service instance `tags` or `status` attributes. The following table describes all attributes that can trigger task execution:
...
| `status`                | The overall status for the service instance is determined by aggregating all node and service health checks and returning the least healthy status according to the following order: <br/> `maintenance` (least healthy)<br/> `critical`<br/> `warning`<br/> `passing` (healthiest) |
