...
The services condition operates by monitoring the [List Service Instances for Service Health API](/api-docs/health#list-nodes-for-service) for healthy, passing service instances. Unhealthy service instances are excluded by default but can be included using the `services` block [`filter`](/docs/nia/configuration#filter) configuration. When there is any change of information for configured services, the task executes. These changes include one or more changes to service values, like IP address, added or removed service instance, tags, status. A complete list of values that would cause a task to run are expanded below:
...
| `status`                | Overall status for the service instance based on an aggregate of all its health checks <sup>1</sup>|
...
<p>
  <sup>1</sup> Overall status is determined by aggregating all (node and service) health checks and returning the "worst" status from the following order: maintenance, critical, warning, passing - where maintenance is the worst.
</p>
