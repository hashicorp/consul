...
For the CTS Enterprise binary, download a zip archive with the `+ent` metadata. [CTS Enterprise requires a Consul Enterpise license](/docs/nia/enterprise/license) to run.
...
The CTS OSS binary is available in the HashiCorp tap, a repository of all our Homebrew packages.
