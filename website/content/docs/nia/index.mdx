...
CTS has an open source and enterprise offering, which is available with Consul Enterprise. Get started with the [Network Infrastructure Automation introduction tutorial](https://learn.hashicorp.com/tutorials/consul/consul-terraform-sync-intro?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) or read more on [CTS Enterprise](/docs/nia/enterprise).
