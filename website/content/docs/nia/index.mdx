...
**Services** - A service in CTS represents a service that is registered with Consul for service discovery. Services are grouped by its service name that may have one or more instances of that service with unique ids. CTS monitors services based on service names, and can provide service instance details to a Terraform module for network automation.
