...
- `Source Input` - A source input defines objects that provide values or metadata to the Terraform module. See [source input](/docs/nia/terraform-modules#source-input) for the supported metadata and values. For example, a user can configure a Consul KV source input to provide KV pairs as variables to their respective Terraform Module.

  The source input can be configured in a couple of ways:
  - Setting the `condition` block's `use_as_module_input` field to true
    - Field was previously named `source_includes_var` (deprecated)
  - Configuring `source_input` block(s)
