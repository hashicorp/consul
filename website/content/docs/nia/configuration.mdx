...
This is the default type of condition when no `condition` block is configured for a task. This condition will trigger the task on changes in services configured in [`task.services`](#services). The task can be optionally configured with a services condition block. However, this is not necessary since this is the default type of condition and there is currently no additional configuration options available for services condition.
...
task {
  ...
  // configure depending on module. provides additional services information but
  // does not execute task. refer to module docs on how to configure.
  services = ["web-api"]
