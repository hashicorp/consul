...
A task monitors Consul objects that are defined by the task's configuration. The Consul objects can be used for the source input that satisfies the requirements defined by the task's Terraform module's [input variables](https://www.terraform.io/docs/language/values/variables.html).
