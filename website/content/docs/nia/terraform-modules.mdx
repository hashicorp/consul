...
A source input provides a mechanism for satisfying the requirements defined by the Terraform Module’s [input variables](https://www.terraform.io/docs/language/values/variables.html) and is included alongside a condition in a task. Whereas the condition defines a resource or Consul-Terraform-Sync supported objects whose changes result in triggering a given task, the source input defines objects whose values or metadata are provided to the Terraform Module. These objects can be the same, such as when `task.services` is provided without a source input block and condition block, but they can also be defined using separate blocks. In this way the source input does not need to be tied to the provided condition in order to satisfy the Terraform Module.
...
Tasks with the services source input monitor for changes to a list of configured services or changes to any services that match a given regex.

In order to configure a task with the services source input, the list of services which will be used for the input must be configured in one of the following ways:
...
The services source input operates by monitoring the [Health List Nodes For Service API](/api-docs/health#list-nodes-for-service) and provides latest service information to the input variables. A complete list of service information that would be provided to the module is expanded below:
...
Tasks with a Consul KV source input monitor Consul KV for changes to KV pairs that satisfy the provided configuration. The Consul KV source input operates by monitoring the [Consul KV API](/api-docs/kv#read-key) and provides these key values to the task’s module.

In order to configure a task with the Consul KV source input, the KVs which will be used for the input must be configured in one of the following ways:
...
Below is a similar example to that provided in the [Consul KV Condition](docs/nia/tasks#consul-kv-condition) section. The difference in this example however, is that instead of triggering based on a change to Consul KV, this task will instead execute on a schedule. Once execution is triggered, Consul KV information is then provided to the task’s module.
...
Tasks with a Catalog Services source input monitors for service and tag information provided by the [Catalog List Services API](/api-docs/catalog#list-services). The source input is a map of service names to a list of tags.

Sample catalog-services input:
...
In order to configure a task with the Catalog Services source input, the catalog services which will be used for the input must be configured in one of the following ways:
