...
The source input, however, monitors defined objects with the intent of providing values or metadata about these objects to the Terraform module. The monitored source input and condition objects can be the same object, such as a task configured with a `condition "services"` block and `use_as_module_input` set to `true`. The source input and condition can also be different objects and configured separately, such as a task configured with a `condition "catalog-services` and `source_input "consul-kv"` block. As a result, the monitored source input is decoupled from the provided condition in order to satisfy the Terraform module.
...
- **`condition` block's `use_as_module_input` field** - When set to true, the condition's objects are used as source input.
...
Example of a catalog-services condition which supports source input through `use_as_module_input`:
