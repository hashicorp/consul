...
~> **NOTE:** Admin partition names are static, and an admin partition cannot be renamed once they have been created as admin partition names are utilized for DNS names for all its services. If an admin partition needs to be renamed, the recommendation is to deploy a new admin partition and redeploy services onto it. 
