...
* If deploying both Consul servers on Kubernetes, Consul servers must be deployed wihtin a single Kubernetes cluster. Consul servers may be deployed external to Kubernetes and configured using the `externalServers` stanza. 
