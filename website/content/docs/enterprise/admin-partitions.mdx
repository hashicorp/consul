...
- All Consul clients must be able to communicate with the Consul servers in the `default` partition, and all servers must be able to communicate with the clients. 
  ...
  - Mesh Gateways are deployed as Kubernetes `LoadBalancer` services on port 443, on all Kubernetes clusters to allow for cross partition networking between Kubernets services. 
