...
Refer to the instructions on [upgrading to 1.10.x](/consul/docs/upgrade/instructions/1-10-x) for details.
...
Once you have the license then create a Kubernetes secret containing the license as described in [Kubernetes - Consul Enterprise](/consul/docs/deploy/server/k8s/examples/enterprise).
...
!> **Warning:** If you are upgrading the Helm chart but **not** upgrading the Consul version, you must set `server.enterpriseLicense.enableLicenseAutoload: false`. See [Kubernetes - Consul Enterprise](/consul/docs/deploy/server/k8s/examples/enterprise) for more details.
