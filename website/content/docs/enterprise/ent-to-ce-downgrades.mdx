...
During the downgrade as the raft replication logs start flowing into the ce server we either drop certain requests or filter certain data with reference to enterprise data.

For example :-

Register requests - register requests having non default namespace or having services or health checks in non default namespace or partition are dropped.
