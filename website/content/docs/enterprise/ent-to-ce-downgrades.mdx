...
Following examples describe scenarios where the server may filter out data from requests:
...
1. Restart or redeploy all Consul clients with a CE version of the binary. You can use a service management system, such as `systemd`, `upstart` to restart the Consul service. If you are not using a service management system, you must restart the agent manually. The following example uses `systemctl` to restart the Consul service:
