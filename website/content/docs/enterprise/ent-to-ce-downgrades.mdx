...
2. Update server binaries to use the CE version

3. Restart or redeploy the servers one at a time with an env var named `CONSUL_ENTERPRISE_DOWNGRADE_TO_CE` set to `true` on the servers.

3. Validate that all servers are on CE and healthy using `consul members` command
