...
server    192.168.4.159:8301    alive   server   1.13.1+ent  2         dc1  default    <all>
client1   192.168.4.159:8447    alive   client   1.13.1+ent  2         dc1  default    alpha
```
...
Node          Address           Status  Type    Build      Protocol  DC   Segment
server1      10.20.10.11:8301   alive   server  1.6.2+ent  2         dc1  alpha
client1      10.20.10.21:8303   alive   client  1.6.2+ent  2         dc1  alpha
```
...
You can call the [`/agent/members` API endpoint](/consul/api-docs/agent#list-members) to view members that the agent sees in the cluster gossip pool. 

<CodeBlockConfig highlight="23">
