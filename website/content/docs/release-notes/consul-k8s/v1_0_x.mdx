...
- ** Simplified Service Mesh Deployments with Consul Dataplane **: Consul client agents are no longer deployed by default, and Consul service mesh no longer uses Consul clients to operate. This change affects several main areas listed below.
  ...
  - Helm:
    ...
    * Consul snap-shot agetn runs as a sidecar to Consul servers, results in the following changes to Helm values:
...
- Consul Dataplane v1.0.x. Refer to [Envoy Integration](/docs/connect/proxies/envoy) for details to see more details about the versions of Consul Dataplane and the packaged Envoy version that is available. 
- Kubernetes 1.21.x - 1.25.x,  and `kubectl` 1.21.x - 1.25.x 
- Helm 3.6+
