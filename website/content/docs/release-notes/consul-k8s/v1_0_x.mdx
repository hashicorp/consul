...
- ** Simplified Service Mesh Deployments with Consul Dataplane **: Consul client agents are no longer deployed by default, and Consul service mesh no longer uses Consul clients to operate. This change affects several main areas listed below.
  - Control plane: 
...
- **Consul snapshot-agent now runs as a sidecar with Consul servers.** Running the snapshot-agent as a sidecar to Consul servers, results in the following changes to Helm values:
...
- Pod Security Standards that are configured for the [Pod Security Admission controller](https://kubernetes.io/blog/2022/08/25/pod-security-admission-stable/) are currently not supported by Consul K8s. Support will be added in a future Consul K8s 1.0.x patch release. 
