...
- **TLS Certificates for Ingress Gateways via an SDS source:**: Inress Gateways can now be configured to retrieve TLS certificates from an external SDS Service for operators to load the TLS certificates for Ingress listeners. This configuration is set using the `ingress-gateway` configuration entry via the [TLS.SDS](docs/connect/config-entries/ingress-gateway#sds) stanza. 
