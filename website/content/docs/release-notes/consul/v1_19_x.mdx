...
- **Argo Rollouts Plugin for Progressive Delivery**: A new Argo Rollouts plugin is now available for `consul-k8s`, which allows users to manage service subset-based routing for Canary deployments. Refer to [Argo Rollouts Progressive Delivery with Consul on Kubernetes](/consul/docs/k8s/deployment-configurations/argo-rollouts-configuration) for more information. 
