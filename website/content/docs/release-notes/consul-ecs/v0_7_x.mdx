...
- **Consul Dataplane:** Consul on ECS 0.7.x adopts the [Dataplane architecture](/consul/docs/architecture/control-plane/dataplane) to simplify connecting your ECS workloads to Consul. Refer to the documentation to learn more about the updated [ECS components](/consul/docs/reference/ecs/architecture) and how to [deploy Consul to ECS using the Terraform module](/consul/docs/register/service/ecs/).
...
- Add the `go-discover` binary to the Consul ECS image to better support [cloud auto-join](/consul/docs/deploy/server/vm/cloud-auto-join).
