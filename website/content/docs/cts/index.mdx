...
- `Scheduled Tasks` - A scheduled task is a type of task that is triggered only on a schedule. It is configured with a [schedule condition](/consul/docs/reference/cts/configuration#schedule-condition).
...
- `Module Input` - A module input defines objects that provide values or metadata to the Terraform module. See [module input](/consul/docs/reference/cts/terraform-module#module-input) for the supported metadata and values. For example, a user can configure a Consul KV module input to provide KV pairs as variables to their respective Terraform Module.
...
- [Build integrations](/consul/docs/reference/cts/terraform-module) for CTS
