...
This topic describes how to run Consul-Terraform-Sync (CTS). The CTS daemon monitors your service mesh for changes and applies a Terraform configuration when it detects a change. Refer to [Network infrastructure automation](/consul/docs/cts) for additional information.


## Overview

You can operate CTS in regular or high-availability (HA) mode. HA mode is an enterprise feature and requires an enterprise license. 

## Requirements

Refer to [Requirements](/consul/docs/cts/requirements) for information about 

### Use a private module with the Terraform network driver
...
2. Create the config.hcl file and configure the options for your use case. Refer to the [configuration reference](/consul/docs/reference/cts/configuration) for details about all CTS configurations.
...
You can start CTS in [inspect mode](/consul/docs/reference/cts/cli/start#modes) to review and test your configuration before applying any changes. Inspect mode allows you to verify that the changes work as expected before running them in an unsupervised daemon mode.
...
- If using [Terraform driver](/consul/docs/nia/network-drivers/terraform), the task may complete depending on the cause of the failover. The new leader starts and attempts to run each task in [once-mode](/consul/docs/reference/cts/cli/start#modes). Depending on the module and provider, the task may require manual intervention to fix any inconsistencies between the infrastructure and Terraform state.
...
### ACL requirements
The following table describes the ACL policies required by CTS. 
...
> Complete the [Secure Consul-Terraform-Sync for Production tutorial](/consul/tutorials/network-infrastructure-automation/consul-terraform-sync-secure?utm_source=docs#configure-acl-privileges-for-consul-terraform-sync) to get hands-on experience configuring CTS.

### High availability mode requirements
...
Add the `high_availability` block in your CTS configuration and configure the required settings to enable high availability. Refer to the [Configuration reference](/consul/docs/reference/cts/configuration#high-availability) for details about the configuration fields for the `high_availability` block.
...
The `session` and `keys` resources in your Consul environment must have `write` permissions. Refer to the [ACL documentation](/secure-consul/acl) for details on how to define ACL policies.
...
1. Create an HCL configuration file that includes the settings you want to include, including the `high_availability` block. Refer to [Configuration Options for Consul-Terraform-Sync](/consul/docs/reference/cts/configuration) for all configuration options.
1. Issue the startup command and pass the configuration file. Refer to the [`start` command reference](/consul/docs/reference/cts/cli/start#modes) for additional information about CTS startup modes.
  ```shell-session
...
1. Identify the leader CTS instance by either making a call to the [`status/cluster` API endpoint](/consul/docs/reference/cts/api/status#cluster-status) or by checking the logs for the following entry:
  ```shell-session
...
1. Identify the leader CTS instance by either making a call to the [`status/cluster` API endpoint](/consul/docs/reference/cts/api/status#cluster-status) or by checking the logs for the following entry:
  ...
  [INFO] ha: acquired leadership lock: id=<ID-OF-CTS-INSTANCE>
  ```
1. Send a `DELETE` call to the  [`/task/<task-name>` endpoint](/consul/docs/reference/cts/api/tasks#delete-task) to delete the task. In the following example, the leader instance is at `localhost:8558`:

  ```shell-session
  $ curl --request DELETE  localhost:8558/v1/tasks/task_a
  ```

 You can also use the [`task delete` command](/consul/docs/reference/cts/cli/task#task-delete) to complete this step.
...
1. Send a `POST` call to the `/task/<task-name>` endpoint and include the updated task in your payload.
  ...
  You can also use the [`task-create` command](/consul/docs/reference/cts/cli/task#task-create) to complete this step.

### Discard data with the `-reset-storage` flag

You can restart the CTS cluster using the [`-reset-storage` flag](/consul/docs/reference/cts/cli/start#options) to discard persisted data if you need to update a task.
...
1. Check the logs printed to the console for errors. Refer to the [`syslog` configuration](/consul/docs/reference/cts/configuration#syslog) for information on how to locate the logs. In the following example output, CTS reported a `401: Bad credentials` error:
  ```shell-session
