...
1. To point to the file containing the license, in the configuration file, configure the [`license`](/consul/docs/reference/cts/configuration#license) path option.
...
1. To point to the file containing the license, in the configuration file, configure the [`license_path`](/consul/docs/reference/cts/configuration#license_path) option i. **Deprecated in CTS 0.6.0 and will be removed in a future release. Use [license block](/consul/docs/reference/cts/configuration#license) instead.**

   ```hcl
   license_path = "<PATH>/<TO>/<FILE>"
   ```

~> **Note**: the [options to set the license and the order of precedence](/consul/docs/enterprise/license#binaries-without-built-in-licenses) are the same as Consul Enterprise server agents.
