...
1. Use the token to join a Consul client. 

The Consul clients then uses the JWTs to securely retrieve security setting changes from Consul servers.
...
Because the `jwt_validation_pub_keys` attribute requires single-line certificates, you must add line escapes `\n` to sections of `secint-pub-key.pem`. In the following example, notice the three instances of line escapes `\n` in the configuration.
