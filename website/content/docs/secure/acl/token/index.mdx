...
Refer to the [ACL API](/consul/api-docs/acl/token) and [ACL CLI](/consul/commands/acl/token/create) documentation for more details on how to create and link tokens. Tokens can also be created dynamically from trusted external system using an [auth method](/consul/docs/secure/acl/auth-method).
