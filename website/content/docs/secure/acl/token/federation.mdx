...
In a [federated Consul deployment](/consul/docs/deploy/server/k8s/multi/federation), one of the datacenters is designated the primary datacenter. You should add the `acl` configuration block to the primary datacenter server's configuration file as shown in the following example.
...
You must configure servers in secondary datacenters to point to the primary datacenter. You need to provide the ACL replication token to the secondary datacenters. Replication tokens are required to replicate ACL tokens and to create [configuration entries](/consul/docs/fundamentals/config-entry) and [auth methods](/consul/docs/secure-consul/acl/auth-methods) in connected secondary datacenters.
