...
In a [federated Consul deployment](/consul/docs/eas), one of the datacenters is designated the primary datacenter. You should add the `acl` configuration block to the primary datacenter server's configuration file as shown in the following example.
