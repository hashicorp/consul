...
Documentation for the commands used in this topic is available at [CEncryption parameters for Consul agent configuration files](/consul/docs/reference/agent/configuration-file/encryption). You can find more information over the gossip protocol used by Consul at [Gossip Protocol](/consul/docs/concept/gossip).
