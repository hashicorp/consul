...
# Enable TLS encryption with built-in CA
...
This page provides instructions for enabling TLS encryption for new Consul datacenters. If you are adding TLS encryption to an existing Consul datacenter, refer to [Enable TLS encryption for existing datacenters](/consul/docs/secure-consul/encryption/gossip/existing.mdx).
...
Repeat this process on the same node where you created the CA, until there is an _individual_ certificate for each server. You can use this command repeatedly since it will automatically increase the certificate and key numbers. Distribute the certificates to the servers.
...
To authenticate Consul servers, you must provide servers with a special certificate - one that contains `server.dc1.consul` in the `Common Name` and in the `Subject Alternative Name`. If you enable [`verify_server_hostname`](/consul/docs/agent/config/config-files#verify_server_hostname), only agents that provide such certificate are allowed to boot as a server. Without `verify_server_hostname = true` an attacker could compromise a Consul client agent and restart the agent as a server in order to get access to all the data in your datacenter. This is why server certificates are special, and only servers should have them provisioned.
...
Earlier, you created the server certificates and distributed them. To use auto-encryption to distribute the client certificates, enable the `auto_encrypt` feature. The following example configuration set the certificate files and enable auto-encrypt.

Since Consul will load all files in the configuration directory, you can add the following configuration as new files. You can also add them to an existing configuration file. Verify that the syntax is valid using [`consul validate`](/consul/commands/validate).
...
You have configured your Consul agents with TLS for encrypted communication. With the auto encryption method, your client certificates are managed by the server. With the operator method, you distributed all the certificates, but have a more flexible configuration.