...
This page provides instructions for enabling TLS encryption for OpenSSL. If you are interesting in using Consul's built in CA, refer to [Enable TLS encryption with built-in CA](/consul/docs/secure-consul/encryption/gossip/builtin.mdx).
...
To authenticate Consul servers, servers are provided with a special certificate - one that contains `server.dc1.consul` in the `Common Name`. If you enable [`verify_server_hostname`](/consul/docs/agent/config/config-files#verify_server_hostname), only agents that provide this certificate are allowed to boot as a server. Without `verify_server_hostname = true`, an attacker could compromise a Consul client agent and restart the agent as a server in order to get access to all the data in your datacenter. This is why server certificates are special and only servers should provision them.

Create a certificate signing request with the private key. The following command will generate a CSR that has Common Name `server.dc1.consul`. Configure the CSR to use the name of your [datacenter](/consul/docs/agent/config/config-files#_datacenter) and your [domain](/consul/docs/agent/config/config-files#_domain).
...
These signing commands are based on default parameters and configurations that might not apply to your infrastructure. If you are issuing production certificates, reach out to your organization's CA admins and/or to your internal processes to get the request signed.
...
This command will create a new file `*.cst`.
...
Repeat this process on the same server where you created the CA until there is an _individual_ certificate for each server.

### Create clients certificate

In this section, you will create a single client certificate that can be shared across multiple clients. If you require a more fine grained certificate policy, repeat the steps below to create multiple client certificates.
...
1. CA public certificate: defined by [`ca_file`](/consul/docs/agent/config/config-files#ca_file) parameter is used to verify the identity of the other nodes. For these instructions, the CA public certificate is named `consul-agent-ca.pem`.
1. Consul agent public certificate: defined by [`cert_file`](/consul/docs/agent/config/config-files#cert_file) parameter.
1. Consul agent private key: defined by [`key_file`](/consul/docs/agent/config/config-files#key_file) parameter.
...
You can add the suggested parameter as a separate file in the ([`-config-dir`](/consul/docs/agent/config/config-files#_config_dir)) directory from where Consul will load all the configuration. This will help you keep configuration separate but you can also include the options into a single configuration file if you prefer that.

The following example agent TLS configuration for Consul servers mentions the distributed certificate files.
...
Consul can use TLS to verify the authenticity of the servers with [`verify_outgoing`](/consul/docs/agent/config/config-files#verify_outgoing) and [`verify_server_hostname`](/consul/docs/agent/config/config-files#verify_server_hostname). It can also optionally verify client certificates when using [`verify_incoming`](/consul/docs/agent/config/config-files#verify_incoming).
...
This example configuration disables the HTTP port to make sure there is only encrypted communication. Existing clients that are unprepared to talk HTTPS will not be able to connect afterwards.

### Start Consul agents

Now that you have configured your Consul agents, start Consul beginning with the servers. The startup logs will confirm that TLS is enabled (`TLS-Outgoing: true` and `TLS-Incoming: true`).
...
If your datacenter is configured to only communicate via HTTPS, you need to create an additional certificate to continue to access the API, including using any of the CLI commands.
...
If you fail to provide the certificate, the Consul CLI will return an error.
...
With HTTPS enabled, you will not be able to access the UI anymore. We recommend that you pick one (or two for availability) Consul agent you want to run the UI on and follow the instructions to get the UI up and running again after creating a new certificate.
...
Use [`addresses.https`](/consul/docs/agent/config/config-files#https) or [client_addr](/consul/docs/agent/config/config-files#client_addr) to update the interface.  Be mindful when updating these values since it also impacts the DNS server.
...
Since your Consul agent is now available to the network, verify that [`enable_script_checks`](/consul/docs/agent/config/config-files#_enable_script_checks) is `false` and [`disable_remote_exec`](/consul/docs/agent/config/config-files#disable_remote_exec) is `true`.
...
If you also want the UI traffic to use mutual TLS, add a client certificate to your browser. This permits to limit UI access only to those browsers where the certificate is present. If you fail to provide the Consul CA, you will not be able to access the Consul UI.

 Trust your Consul CA (`consul-agent-ca.pem`) on your machine to resolve this issue. These instructions will vary based on your operating system. Once you trust the CA, you can access the UI.
...
When `verify_incoming` is enabled, Consul requires that all incoming connections make use of TLS and that the client provides a certificate signed by a CA from the `ca_file` or `ca_path`. This applies to both server RPC and to the HTTPS API.

Since the browser doesn't present a certificate signed by your CA, you cannot access the UI. If you `curl` your HTTPS UI, it will return an error.
...
## Next steps
You have enabled TLS on your Consul datacenter so it will encrypt all RPC, consensus, and HTTP traffic. With TLS encryption enabled, you need reference the [CA cert](/consul/commands/#consul_cacert), [client cert file](/consul/commands/#consul_client_key), and [client key file](/consul/commands/#consul_client_cert) to complete any API or CLI operation since they are HTTP traffic.