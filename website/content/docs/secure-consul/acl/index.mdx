...
There are two workflows for administrating and using ACLs. The ACL administrator configures and manages the ACL system. This includes creating all the artifacts required for ACLs (for example, rules, policies, and tokens), generating the ACL tokens, and distributing the tokens. The ACL consumers are users and services that use the token to authenticate to Consul.
...
Once the ACL consumers receive the ACLs token from the administrator. They present the token when making requests (CLI, API, or UI). Consul evaluates the token to determine if the request has permission to interact with the requested resource.

Refer to the [Troubleshoot](#troubleshooting) page if you do not observe the expected behavior.

## Tokens

ACL tokens are the core method of authentication in Consul. The ACL token (the value of the `SecretID` field) lets your or your service identify the person or system making the request. If ACLs is enabled in Consul, you must include the ACL token to authenticate any requests. The ACLs administrator can also use the token's `AccessorID` for audit logging purposes.
...
An ACL policy is a set of rules that grant or deny access to resources in the network. The ACLs administrator will create policies and link them to tokens. You can assign multiple policies to an ACL token. You can configure policies to grant tokens different scopes.
