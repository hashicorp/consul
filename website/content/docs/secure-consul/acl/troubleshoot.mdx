...
This page describes how to troubleshoot your Consul access control list (ACL) system using several Consul CLI commands and the ACL system reset procedure.

## Requirements

These instructions assumes an existing datacenter running Consul 1.4 or newer. In addition, you will need access to a machine where you can execute Consul CLI commands, either a Consul agent or a local binary configured to control the remote datacenter.
...
The `consul catalog nodes -detailed` [command](/consul/commands/catalog/nodes) will display node information, including "TaggedAddresses". If "TaggedAddresses" is null for any of the agents, that agent's ACLs are not configured correctly. You can start debugging by reviewing the Consul logs on all the servers. If ACLs are enabled correctly, you can investigate the agent's  token.
...
The `consul acl policy list` [command](/consul/commands/acl/policy/list) will output all the available policies. This command is useful when you first set up policies so you can verify your Consul policies and their respective rules. In 
...
The `consul acl token list` [command](/consul/commands/acl/policy/list) will list all the tokens. Verify that this list only includes the tokens in use. It is important for the security of your datacenter and you should check it often. Since tokens do not expire, it is the ACL administrator's responsibility to [delete](/consul/commands/acl/token#delete) tokens that are not in use.

In the following example, there are three tokens. 

1. The first token is created by Consul during the bootstrap process, called bootstrap token.
1. TThe second token is user generated and was created with the `server-one-policy` policy. 
...
The `consul acl token read` [command](/consul/commands/acl/policy/read) will provide information about the token specified. Verify the privileges of the token are expected. This is useful when checking a node or service has the correct privileges to add itself to the catalog.
