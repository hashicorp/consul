...
Refer to the [ACL workflow overview](/consul/docs/secure-consul/acl#workflow-overview) for information about tokens' role in the ACL system.

## Create tokens

The ACLs administrator can use the API or CLI to create and link tokens to entities that enable permissions to resources. 
...
Refer to the [ACL API](/consul/api-docs/acl/tokenscl) and [ACL CLI](/consul/commands/acl/token/create) documentation for more details on how to create and link tokens. Tokens can also be created dynamically from trusted external system using an [auth method](/consul/docs/secure-consul/acl/auth-methods).
...
Refer to the [Token](/consul/docs/reference/acl/token) reference docs for a full list of token attributes.

### Perform Consul operations

If ACLs are enabled, you must pass an ACLs token with your request to interact with Consul.
...
Refer to the [Services](/consul/docs/reference/service) reference docs for more information.
...
Refer to the [Agent](/consul/docs/reference/agent#acl) reference docs for more information.

# Rotate token

Eventually, you will need to rotate your tokens.

You must create a new token, update any configuration that references the old token with the new token, and then delete the old token. You can rotate tokens used to register services and agents, query DNS, and access Consul data.
...
Apply tokens to [the agent](/consul/docs/secure-consul/acl/bootstrap#add-token-to-agents) or to [the services](/consul/docs/secure-consul/acl/bootstrap#add-token-to-service-definition) as needed.

Finally, delete the old token from Consul. Alternatively, you can delete ACL tokens using the [HTTP API](/consul/api-docs/acl/tokens#delete-a-token).
...
| ----------------------------- | ---------- | ---------- | ----------- |
| [`acl.tokens.agent`](/consul/docs/agent/config/config-files#acl_tokens_agent) | `optional` | `optional` | Used for internal agent operations. Refer to [ACL Agent Token](#acl-agent-token) for details. |
...
| [`acl.tokens.initial_management`](/consul/docs/agent/config/config-files#acl_tokens_initial_management) | `optional` | `n/a` | Used to bootstrap the ACL system. Refer to [Initial management token](#initial-management-token). |
...
You can use the [`/v1/agent/token` API](/consul/api-docs/agent#update-acl-tokens) to create or update all reserved tokens except the `initial_management` token.
...
Consul includes a built-in anonymous token and initial management token. Both tokens are injected during when you [bootstrap](/consul/docs/secure-consul/acl/bootstrap) the cluster.
...
1. Bootstrap the ACL system with `consul acl bootstrap`. Consul will automatically generate a token for you. Refer to [Bootstrap ACLs](/consul/docs/secure-consul/acl/bootstrap) for more details.
2. Define an initial management token in the Consul agent configuration ([`acl.token.initial_management`](/consul/docs/reference/agent#acl_tokens_initial_management)). This lets ACL administrators to bootstrap the ACL system with a known value.

Consul will link the management token to the built-in [global management](/consul/docs/secure-consul/acl/policy#global-management) policy. This token will have unrestricted privileges to resources and APIs.
