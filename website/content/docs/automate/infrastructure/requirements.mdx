...
The Consul agent must be running in order to dynamically update network devices. Refer to the [Consul agent documentation](/consul/docs/architecture/control-plane/agent) for information about configuring and starting a Consul agent.
...
Alternatively, you can configure the [`limits.http_max_conns_per_client`](/consul/docs/reference/agent/configuration-file#http_max_conns_per_client) option to set a maximum number of connections to meet your needs.
