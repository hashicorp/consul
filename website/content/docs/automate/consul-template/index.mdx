...
1. _Update configuration files_. The Consul template tool can be used to update service configuration files. A common use case is managing load balancer configuration files that need to be updated regularly in a dynamic infrastructure.

1. _Update configuration secrets_. You can configure Consul Template to use Vault secret engines to generate secrets for your application's configuration, such as encryption keys, TLS certificates, passwords, and render them into files or environment variables where they can be consumed by your application.

1. _Discover data about the Consul datacenter and service_. It is possible to collect information about the services in your Consul datacenter. For example, you could
   collect a list of all services running on the datacenter or you could discover all service addresses for the Redis service. Note, this use case has limited scope for production.
...
In this first use case example, you will render a template that pulls the HashiCorp address from Consul KV. To do this, you will create a simple template that contains the HashiCorp address, run the `consul-template` command, add a value to Consul KV for HashiCorp's address, and finally view the rendered file.

<Tip>

If you don't have a Consul instance you can start a dev instance using `consul agent -dev`. Read more at [Starting the Consul agent](/consul/docs/fundamentals/install/dev).
...
Verify the data was written by viewing the `hashicorp_address.txt` file which will be located in the same directory where Consul Template is running.
...
If you update the key `hashicorp/street_address`, you can see the changes to the file immediately. 

Try change the content of the key in Consul.
...
In this use case example, you will discover all the services running in the Consul datacenter. To follow along, use the same Consul agent from the previous example.
...
If you complete this on your local development agent, the answer will only contain the default `consul` service when viewing `all-services.txt`.
...
The two examples above provide you with a basic idea on how to use Consul Template to render filed dynamically using data in the Consul KV store or in the Consul Catalog.

Extending this simple process you can realize complex configurations.

You can use Consul Template to [automate an NGINX reverse proxy configuration](/consul/tutorials/network-automation/consul-template-load-balancing) using Consul Catalog as a source for the upstream IPs.

You can even use Consul Template to automate Consul operations using Vault as secret management. For example you can:
...
To find additional examples, refer to the examples folder in [GitHub](https://github.com/hashicorp/consul-template/tree/master/examples).

To explore the available functions to use in your templates refer to the [Consul Template Go language](/consul/docs/reference/consul-template/go) reference.