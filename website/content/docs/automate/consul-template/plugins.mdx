...
- Data piped into the plugin is appended after any parameters given explicitly (e.g. `{{ "sample-data" | plugin "my-plugin" "some-parameter"}}` will call `my-plugin some-parameter sample-data`)
