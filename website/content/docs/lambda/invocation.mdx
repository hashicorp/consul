...
The terminating gateway must:
* Have [the appropriate IAM permissions](/docs/lambda/registration#configure-iam-permissions-for-envoy) to invoke the Lambda function.
* Have the Lambda function's Consul service linked to it. This is done by:
  1. Ensuring the Lambda service is linked to the terminating gateway's configuration:
      <CodeBlockConfig filename="terminating-gateway.hcl">
  ...
  1. Issuing `consul config write terminating-gateway.hcl` to update the terminating gateway's configuration.
...
This method is secure because [AWS IAM permissions](/docs/lambda/registration#configure-iam-permissions-for-envoy) is required to invoke Lambda functions. Additionally, all communication is encrypted with Amazon TLS when invoking Lambda resources.
