...
The terminating gateway must have the following configurations:
* [The appropriate IAM permissions](/docs/lambda/registration#configure-iam-permissions-for-envoy) to invoke the Lambda function.
* The Lambda function's Consul service linked to the terminating gateway. To link the Lambda service to the terminating gateway's configuration, use the following configuration file entry:
  ...
  Issue `consul config write terminating-gateway.hcl` to update the terminating gateway's configuration.
...
This method is secure because [AWS IAM permissions](/docs/lambda/registration#configure-iam-permissions-for-envoy) are required to invoke Lambda functions. Additionally, all communication is encrypted with Amazon TLS when invoking Lambda resources.
