...
permissions to invoke the Lambda. In the following example, the IAM policy
...
| `consul_http_token_path` |  Specifies the AWS Parameter Store path to the ACL Token Lambda registrator uses. This parameter only required when ACLs are enabled for the Consul server. It is used to fetch an ACL token from Parameter Store and is stored in the `CONSUL_HTTP_TOKEN` environment variable. Also see [Optional: Store the ACL Token in Parameter Store](#optional-store-the-acl-token-in-parameter-store)|
