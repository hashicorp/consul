...
1. Create an ACL policy that includes the following rule: <CodeBlockConfig filename="rules.hcl">
...
1. Create a configuration for registering the service. You can copy the following example and replace `<SERVICE_NAME>` with your Consul service name for the Lambda function: <CodeBlockConfig filename="lambda.json">
