...
This topic describes how to configure HTTP and TCP routes and attach them to Consul API gateway listeners in Kubernetes-orchestrated networks. Routes are rule-based configurations that allow external clients to send requests to services in the mesh. Refer to [Define API gateway routes on virtual machines](/consul/docs/access/api-gateway/define-route/vm) for instructions on defining routes on VMs.
...
Verify that your environment meets the requirements specified in [Technical specifications for Kubernetes](/consul/docs/access/api-gateway/tech-specs).

### OpenShift

If your Kubernetes-orchestrated network runs on OpenShift, verify that OpenShift is enabled for your Consul installation. Refer to [OpenShift requirements](/consul/docs/access/api-gateway/tech-specs#openshift-requirements) for additional information.
