...
First, review the namespaces available in the Consul datacenter. If you follow the example instructions to [configure Consul namespaces on VMs](/consul/docs/multi-tenant/namespace/vm/configure-namespaces), the datacenter contains two different namespaces: `app-team` and `db-team`.
