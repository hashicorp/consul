...
Client agents are disabled by default in Consul on Kubernetes. Instead, you can use an alternate service mesh configuration that deploys Envoy proxies but not client agents. Refer to [Simplified Service Mesh with Consul Dataplanes](/docs/connect/dataplane/index) for more information.

If you prefer to use client agents in your service mesh, the following architectural implications affect your deployments.
