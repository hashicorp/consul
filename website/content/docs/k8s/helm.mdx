...
- `dns` ((#v-dns)) - Configuration for DNS configuration within the Kubernetes cluster.
  ...
  - `enableRedirection` ((#v-dns-enableredirection)) (`boolean: false`) - If true, services using Consul service mesh will use Consul DNS
...
- `meshGateway` ((#v-meshgateway)) - Mesh Gateways enable Consul service mesh to work across Consul datacenters.
