...
If you configured your Consul agents to use [`ports.grpc_tls`](/consul/docs/agent/config/config-files#grpc_tls_port) instead of [`ports.grpc`](consul/docs/agent/config/config-files#grpc_port) and you want to upgrade a multi-datacenter deployment with Consul servers running outside of the Kubernetes cluster to v1.0.0 or higher, set [`externalServers.tlsServerName`](/consul/docs/k8s/helm#v-externalservers-tlsservername) to `server.<primary-datacenter>.domain`.
