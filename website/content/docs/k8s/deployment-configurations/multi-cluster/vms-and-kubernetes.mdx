...
1. If ACLs are enabled you must create an ACL replication token with the following rules:
   ...
   ```
1. If ACLs are enabled you'll also need to modify the [anonymous token](/consul/docs/security/acl/tokens#anonymous-token) policy to have the following permissions:
