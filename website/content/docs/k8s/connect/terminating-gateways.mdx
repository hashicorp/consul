...
```shell-session
$ kubectl port-forward service/consul-server -n consul 8500 &
```
...
```shell-session
$ kubectl port-forward service/consul-server -n consul 8501 &
```
...
```shell-session
$ export CONSUL_HTTP_TOKEN=$(kubectl get secret consul-bootstrap-acl-token --template='{{.data.token | base64decode }}' -n consul)
```
...
```shell-session
$ kubectl exec deploy/static-client -c static-client -- curl -vvvs https://example.com/
```
...
```shell-session
$ kubectl exec deploy/static-client -c static-client -- curl -vvvs --header "Host: example-https.com" http://localhost:1234/
```
