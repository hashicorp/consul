...
By default, the Consul service mesh runs in transparent proxy mode, which forces inbound and outbound traffic through the sidecar proxy, even though the service binds to all interfaces. Transparent proxy infers the location of upstream services using Consul service intentions, as well as allows you to use Kubernetes DNS as you normally would. 
