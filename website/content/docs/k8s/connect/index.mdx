...
In Kubernetes 1.24+ you will need to [create a Kubernetes secret](https://kubernetes.io/docs/concepts/configuration/secret/#service-account-token-secrets)
explicitly for each multi-port service which references the ServiceAccount and it *must* have the same name as the ServiceAccount:
