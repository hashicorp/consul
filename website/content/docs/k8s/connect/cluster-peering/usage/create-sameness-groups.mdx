...
This topic describes how to create a sameness group, which designates a set of admin partitions as functionally identical in a Consul deployment running Kubernetes. Adding an admin partition to a sameness group enables Consul to recognize services registered to different clusters as instances of the same service when they share a name and Consul namespace.
...
Sameness groups are a user-defined set of partitions with established cluster peering connections between each other. These partitions should have identical configurations, including configuration entries for service and proxy defaults.
...
Sameness groups are defined at the partition level. Consul assumes these partitions have identical configurations, including identical service names and identical Consul namespaces. This behavior occurs even when two partitions in the group contain functionally different services that share a common name and namespace. For example, if distinct services both named `api` were registered to two members of a sameness group, it could lead to errors because requests may be sent to the incorrect service.
...
To create a sameness group, you must write and apply sets of three CRDs for each partition that is a member of the group:
...
If you want all services to failover to other instances in the sameness group by default, set `spec.defaultForFailover=true` and list the group members in the order you want to use in a failover scenario. Refer to [failover with sameness groups](/consul/docs/connect/failover/sameness) for more information.
...
If you separately follow the steps in this documentation to create a cluster peering connection between two partitions and then add those partitions to a sameness group, you write and apply two separate exported services CRDs: one to export services to the peer, and a second to export services to other members of the group.

If you are peering clusters with the goal of creating a sameness group, it is possible to combine these workflows by using a single exported services CRD.

In this workflow, configure the `services[].consumers` block with the `samenessGroup` field instead of the `peer` field. Be aware that this scenario requires you to write the `SamenessGroup` CRD to Consul before you apply the `ExportedServices` CRD that references the sameness group.
...
If you separately follow the steps in this documentation to create a cluster peering connection between two partitions and then add those partitions to a sameness group, you write and apply two separate service intentions CRDs: one to authorize traffic to the peer, and a second to authorize traffic to other members of the group.

If you are peering clusters with the goal of creating a sameness group, it is possible to combine these workflows by using a single service intentions CRD.

Configure the `sources` block with the `samenessGroup` field instead of the `peer` field. Be aware that this scenario requires you to apply the `samenessGroup` CRD before you apply the `serviceIntentions` CRD that references the sameness group.

## Next steps

When `defaultForFailover=true` in a sameness group CRD, additional upstream configuration is not required. If the sameness group is not configured as the default for failover, you must specify the sameness group in an upstream annotation when scheduling the workload. Refer to the specifications for `consul.hashicorp.com/connect-service-upstreams` in the [Kubernetes annotations and labels reference](/consul/docs/k8s/annotations-and-labels#consul-hashicorp-com-connect-service-upstreams) for more information.