...
1. For both clusters apply the `Mesh` configuration entry values provided in [Mesh Gateway Specifications](/consul/docs/k8s/connect/cluster-peering/best-practices#mesh-gateway-recommendations) to allow establishing peering connections over mesh gateways. 
