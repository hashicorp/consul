...
1. Run the [`consul peering list` CLI command](/consul/commands/peering/list).
  ...
  dc2-default  ACTIVE   0              2              env=production
  dc2-web      PENDING  0              0
  ```
...
1. Run the [`consul peering read` CLI command](/consul/commands/peering/read).
  ...
  ```shell-session
  $ consul peering read -name dc2-default
  Name:         dc2-default
...
To confirm that you deleted your peering connection in `dc1-default`, query the `/health` HTTP endpoint:
...
1. Query the `/health` HTTP endpoint. Peered services with deleted connections should no longe appear.
  ...
  ```shell-session
  $ curl "localhost:8500/v1/health/connect/backend?peer=dc2-default"
  ```