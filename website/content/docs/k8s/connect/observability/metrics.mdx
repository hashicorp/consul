...
The Helm chart contains demo manifests for Prometheus for viewing metrics within the Consul UI as was as through other external observability tools such as Grafana. It can be installed with Helm via `prometheus.enabled`. This manifest is based on the community manifest for Prometheus.
The Prometheus deployment included with Consu Helm is designed to allow quick bootstrapping for trial and demo use cases and not for production use-cases.

Prometheus will be installed in the same namespace that Consul will be installed in and will be installed
and uninstalled along with the Consul installation. In addition, Grafana can be utilized with Prometheus and separately installed and configured option through its own installation via Helm (an example is provided in the following Learn Guide, [Layer 7 Observability with Prometheus, Grafana, and Kubernetes](https://learn.hashicorp.com/tutorials/consul/kubernetes-layer7-observability?in=consul/kubernetes)).
