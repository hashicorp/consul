...
* Using the following annotations on the Ingress controller's deployment, setup exclusion rules for it's ports.
  * `consul.hashicorp.com/transparent-proxy-exclude-inbound-ports` - Provides the ability to exclude a list of ports for
...
* If the Ingress controller acts as a LoadBalancer and routes directly to Pod IPs instead of KubeDNS a `ServiceDefault` CRD
