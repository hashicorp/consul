...
-> This topic has the following pre-requisites:  
  - Familiarity by the reader with [Ingress Controllers](https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/).
  - Consul 1.10+, Consul-k8s 0.26+, Consul-helm 0.32+ configured with [Transparent Proxy](/docs/connect/transparent-proxy) mode enabled.
