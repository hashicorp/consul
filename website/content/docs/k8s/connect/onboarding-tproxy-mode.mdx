...
description: Learn how to enable permissive mutual transport layer security (permissive mTLS) so that you can safely add services to your service mesh when transparent proxy is enabled. 
---

# Onboard services while in transparent proxy mode

This topic describes how to run Consul in permissive mTLS mode so that you can safely onboarding existing applications to Consul service mesh when transparent proxy mode is enabled.

## Background

When [transparent proxy mode](/consul/docs/k8s/transparent-proxy/) is enabled, all service-to-service traffic is secured by mTLS. Until the services that you want to add to the network are fully onboarded, your network may have a mix of mTLS and non-mTLS traffic, which can result in broken service-to-service communication. This is because sidecar proxies for existing mesh services reject traffic from services that are not yet onboarded.
...
We recommend enabling permissive mTLS as a temporary operating mode. After onboarding is complete, all services should be placed into `strict` mTLS mode to ensure all service-to-service communication is automatically secured by Consul service mesh. 
...
The workflow toggling mTLS settings depends on the applications you are onboarding and the order you intend to onboard them, but the following steps describe the general workflow:
...
Permissive mTLS is only supported for services running in transparent proxy mode.
...
You can toggle this setting back to `false` at any time, even if there are services currently in permissive mode. This allows you to decide at which point during the onboarding process to stop allowing services to use permissive mTLS. When the `MeshDestinationOnly` is set to `false`, you must configure all new services added to the mesh with  `MutualTLSMode=strict` for the Consul to securely route traffic throughout the mesh. 
...
You can toggle this setting back to `strict` at any time to ensure mTLS is required for incoming traffic to this service.

## Configure intentions

Intentions are mechanisms in Consul that control traffic between mesh services. Refer to [Service intentions](/consul/docs/connect/intentions) for additional information about intentions. 

We recommend creating intentions that allow services to accept only necessary traffic. You must identify the downstream services that send messages to the service you want to add to the mesh and create an intention to allow traffic from each of those downstream services to this service.
...
If the newly-added service was placed in permissive mTLS mode, then you should switch to strict mode when it is safe to do so. You should also revert the global settings that allow services to send and receive non-mTLS traffic.  

### Disable permissive mTLS mode

Configure the service to operate in strict mTLS mode after the service is no longer receiving incoming non-mTLS traffic. Once the downstream services that send messages to this service are all onboarded to the mesh, this service should no longer receive non-mTLS traffic.
...
- The `tcp.permissive_public_listener.*` statistics indicate non-mTLS traffic. If these statistics are unchanged for a sufficient period of time, that indicates the sidecar is not receiving non-mTLS traffic.
- The `tcp.public_listener.*` statistics indicate mTLS traffic. If incoming traffic is expected to this service and if these statistics are changing, then the sidecar is receiving mTLS traffic.
...
1. Verify that each downstream has an intention that allows it send traffic.

Once you’ve determined it is safe to move the service to strict mode, set `MutualTLSMode=strict` in the service defaults configuration entry. 
...
Run the `consul config list` and `consul config read` commands in each namespace in each admin partition and datacenter to verify that no services are using `permissive` mTLS mode. 

The following command returns any service-defaults that contain `'MutualTLSMode = "permissive"'`:

```shell-session
$ consul config list -kind service-defaults -filter 'MutualTLSMode == "permissive"'
```

The following command fetches the proxy defaults configuration entry. Verify that `MutualTLSMode = "permissive"` is not set in the proxy defaults configuration entry in each admin partition and datacenter. If `MutualTLSMode` is either empty or if the configuration entry is not found, then the mode is `strict` by default.
