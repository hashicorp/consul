...
The helm chart now contains manifests for Prometheus and Grafana. They can be selectively installed during the Helm install depending on the values of `prometheus.enabled` and `grafana.enabled`. These manifests are based on the community manifests for Prometheus and Grafana.
