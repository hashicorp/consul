...
This catalog API was designed prior to the introduction of Consul’s service mesh features. Since that time, workarounds for common networking needs developed. For example, while the catalog API does not support multi-port services natively, you can register a service with multiple ports by scheduling it in two containers. The following example code demonstrates how a multi-port service could be configured in Kubernetes in order to function with the existing Consul Catalog:
...
1. Enable the v2 catalog. Update the server agent configuration and then apply the change to your Kubernetes cluster.
...
1. Confirm multi-port functionality. Send test traffic to each port to confirm that traffic is authorized and routed correctly.
