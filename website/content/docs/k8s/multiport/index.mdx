...
This catalog API designed prior to the introduction of Consul’s service mesh features. Services and service instances are coupled in the catalog such that Consul's ACL system requires that a Kubernetes ServiceAccount match the service name. As a result, only one service can represent a workload in the Consul catalog.

Since that time, the cloud networking needs for applications developed and the Consul catalog supports workarounds for these needs. For example, [Kubernetes Pods with multiple ports](/consul/docs/k8s/connect#kubernetes-pods-with-multiple-ports) demonstrates how you can schedule a service with multiple ports so that Consul registers it as distinct services with their own service instances. However, this workaround results in additional resource consumption because Consul requires that each service and port use their own proxy and Consul dataplane in order to be recognized as distinct services.
...
- `Workload`, like [Kubernetes Workloads](https://kubernetes.io/docs/concepts/workloads/), is an application instance running in a set of one or Pods scheduled according to a Workload resource such as a Deployment or StatefulSet.
- `WorkloadIdentities`, like [Kubernetes Service Accounts](https://kubernetes.io/docs/concepts/security/service-accounts/), provide a distinct identity for a Workload to assume in a Kubernetes cluster.

This catalog structure enables Consul to associate a single Kubernetes workload with multiple Consul services.
