...
- `Workload`, like [Kubernetes Workloads](https://kubernetes.io/docs/concepts/workloads/), is an application instance running in a set of one or more Pods scheduled according to a Workload resource such as a Deployment or StatefulSet.
...
- `TrafficPermissions` is a replacement for the `ServiceIntentions` Custom Resource Definition (CRD). Traffic permissions replace service intentions all services in the v2 catalog, enabling authorization of L4 traffic according to workload identity instead of service identity.
