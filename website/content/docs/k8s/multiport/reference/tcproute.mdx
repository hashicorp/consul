...
Specifies rules for sidecar proxies to direct a service's TCP traffic within the service mesh, including filtering, retry, and timeout behavior.
...
Specifies a set of header modification rules applied to requests routed with the TCPRoute resource.
...
Specifies a set of header modification rules applied to responses routed with the TCPRoute resource.
...
Specifies retry logic for routing TCP traffic.
...
Specifies timeout logic when routing TCP traffic
