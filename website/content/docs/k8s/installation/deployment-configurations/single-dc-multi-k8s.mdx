...
~> Requires consul-helm v0.33.0 or higher.
...
~> This deployment topology requires that your Kubernetes clusters have a flat network
...
Note that we are deploying with a secure configuration, with gossip encryption,
TLS for all components and ACLs. We are enabling the Consul Service Mesh and the controller for CRDs
so that we can use them to verify that our services can connect with each other across clusters.
...
To deploy this, first we need to generate the Gossip encryption key and save it as a Kubernetes secret.
...
~> Note that the Helm release name must be unique for each Kubernetes cluster.
That is because the Helm chart will use the Helm release name as a prefix for the
ACL resources that it creates, such as tokens and auth methods, and if the names of the Helm releases
are the same, the Helm installation in subsequent clusters will clobber existing ACL resources.

Once installation finishes and all components are running and ready,
...
~> Note that if multiple Kubernetes clusters will be joined to the Consul Datacenter, then the following instructions below will need to be repeated for each additional Kubernetes cluster. 

Now we can switch to the second Kubernetes cluster where we will deploy only the Consul clients
that will join the servers and clients in the first cluster.
...
```
To deploy clients, we will use the following Helm configuration:
...
externalServers:
  ...
  # The address of the kube API server of the this Kubernetes cluster
...
Next, you need to set up the `externalServers` configuration.
...
which you can see by running `kubectl get nodes -owide`.
...
that is present in the Consul server's certificate. We need to set it because we're
connecting to the Consul servers over the node IP, but that IP isn't present in the server's certificate.
To make sure that the hostname verification succeeds during the TLS handshake, we need to pass set the TLS
server name to a DNS name that *is* present in the certificate.

Next, we need to set `externalServers.k8sAuthMethodHost` to the address of the Kubernetes API server.
This should be the address that is reachable from the first cluster, and so it cannot be the internal DNS
available in each Kubernetes cluster. That is needed so that `consul login` via auth method will work
...
whenever `consul login` is called, and to verify service accounts from the second cluster it needs to be able to
reach Kube API in that cluster.
...
In order to use it, we need to provide a way for Consul to reach the first Kubernetes cluster.
To do that, we save `kubeconfig` for the first cluster as a Kubernetes secret in the second cluster
...
~> Note that the kubeconfig you're providing to the client should have minimal permissions.
The cloud auto-join provider will only need to list pods.
...
Now that we have our Consul cluster in multiple k8s cluster up and running, we will
...
Then deploy `static-client` in the second cluster with the following configuration:
