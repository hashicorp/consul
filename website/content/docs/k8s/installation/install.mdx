...
1. Issue the `install` subcommand to install Consul on Kubernetes:
   ...
   Refer to the [Consul K8s CLI reference](/docs/k8s/k8s-cli) for details about all commands and available options. Without any additional options passed, the `consul-k8s` CLI will install Consul on Kubernetes with the the Helm chart default values. Below is an example that installs Consul on Kubernetes with Service Mesh and CRDs enabled. 
...
1. (Optional) Run `consul-k8s status` command to quickly glance at the status of the installed cluster. 
