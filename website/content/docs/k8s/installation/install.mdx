...
1. Prior to installing via Helm, ensure that the `consul` Kubernetes namespace does not exist. Installing on a dedicated namespace
   is recommended. You can also install Consul dedicated of your choosing by modifying the value of the namespace flag for the Helm install. 
...
1. Issue the following command to install Consul with the default configuration using Helm 3.2+:
