...
The recommended way to install Consul on Kubernetes for single cluster use cases is via the [Consul K8S CLI](/docs/k8s/k8s-cli). 
You can install Consul on Kubernetes using the Consul K8s CLI tool after installing the CLI and running it against a Kubernetes cluster. 
Ensure that `kubectl` is already configured to authenticate to the Kubernetes cluster using a valid `kubeconfig` file. 
The Consul K8s CLI is currently available as an beta release and is not recommended for production environments.
...
1. Next, install the Consul K8s CLI with the `hashicorp/tap/consul` formulae.
   ```shell-session
...
For multi-cluster installs that involve cross-partition of cross datacenter communication, installing via the Consul on Kubernetes 
Helm chart is the recommended process. This will install and configure all the necessary components to run Consul. The configuration 
enables you to run a server cluster, a client cluster, or both.
