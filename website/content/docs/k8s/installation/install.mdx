...
-> ** NOTE: ** To deploy a pervious version of Consul on Kubernetes via the CLI, you'll need to first download the specific version of the CLI that matches the version of the Control Plane that you'd like to deploy. Please follow [Install a specific version of Consul K8s CLI](/docs/k8s/k8s-cli#install-a-specific-version-of-consul-k8s-cli).
