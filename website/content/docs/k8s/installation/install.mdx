...
~> If you're looking to install a specific version of Consul on Kubernetes via the CLI, you'll need to first download the specific version of the CLI that matches the version of the Control Plane that you'd like to deploy as described in [Install a specific version of Consul K8s CLI](#install-a-specific-version-of-consul-k8s-cli).
