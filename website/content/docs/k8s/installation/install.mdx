...
The minimal settings to enable [Consul Service Mesh]((/docs/k8s/connect)) would be captured in the following `vaules.yaml` config file:
...
By default, Consul Service Mesh is enabled on all namespaces within Kubernetes. However if you'd like to control which namespaces allow connect-inject to occur, you can alternatively use a label on a namespace to denote where to enable Consul Service Mesh. 
...
### Making changes to your Consul on Kubernetes deployment 
