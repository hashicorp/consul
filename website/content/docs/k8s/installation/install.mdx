...
By default, Consul Service Mesh is enabled on almost all namespaces within a Kubernetes cluster. You can restrict this to a subset of namespaces by specifying a `namespaceSelector` that matches a label attached to each namespace denoting whether to enable Consul service mesh.
<CodeBlockConfig filename="config.yaml">
