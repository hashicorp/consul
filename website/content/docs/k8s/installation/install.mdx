...
By default, the Helm installation of Consul on Kubernetes uses [Consul Dataplanes](/docs/connect/dataplane) instead of client agents.

If you use client agents in your deployment, you can access agents, using the [downward API](https://kubernetes.io/docs/tasks/inject-data-application/downward-api-volume-expose-pod-information/). The Consul HTTP API should be accessed by communicating with a client agent running on the same node. While technically any listening agent can respond to the HTTP API, communicating with the local agent has important caching behavior, and allows you to use the simpler [`/agent` endpoints for services and checks](/api-docs/agent).
