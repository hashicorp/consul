...
- Set up per Consul data center
  ...
  - Configuring a Kubernetes Auth Method in Vault to authenticate and operations from the Consul data center
...
```
Next, set you Vault Kubernetes Auth Method, with the host, CA Cert, and token that has access to your Consul data center cluster:
