...
Prior to creating auth roles for the Consul servers and clients, ensure that the Vault Kubernetes auth method is enabled:
...
The Vault PKI Engine must be enabled in order to leverage Vault for issuiing Consul Server TLS certificates. More details of configuring this is found in [Bootstrapping the PKI Engine](https://www.consul.io/docs/k8s/installation/vault/server-tls#bootstrapping-the-pki-engine) under the Server TLS section. 
