...
-> **Note:** To generate the `<Allowed-domains-string>` use the following template

```shell-session
name = global.name if set, or <Your-release-name>-consul
ns = <Consul installation namespace>
Sprintf("%s.consul,%s-server,%s-server.%s,%s-server.%s.svc", global.datacenter, $name, $name, $ns, $name, $ns)
```
...
the Kube auth role in a CSV e.g.  `policies=consul-server,consul-gossip`
