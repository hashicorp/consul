...
### Configuring VAULT_ADDR to run commands from your local machine
...
```
Next, we will need to get the token and CA cert from the Vault service account secret. Since it is in the same cluster and namespace as the Consul dc1 datacenter,we will be able to configure the Vault Kubernetes Auth Method to use it to authenticate secret requests from service accounts in the Consul dc1 datacenter.

```shell
$ export K8S_DC2_CA_CERT="$(kubectl get secret `kubectl get serviceaccounts vault-dc1 -o jsonpath='{.secrets[0].name}'` -o jsonpath='{.data.ca\.crt}' | base64 -d)"
```

```shell
$ export K8S_DC2_JWT_TOKEN="$(kubectl get secret `kubectl get serviceaccounts vault-dc1 -o jsonpath='{.secrets[0].name}'` -o jsonpath='{.data.token}' | base64 -d)"
```
###### Configure Auth Method with JWT token of the Vault service account
