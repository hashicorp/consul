...
1. Install Vault Injector in your Consul k8s cluster, which is used for accessing secrets from Vault. 
    ...
    -> In dc1, you will not have to configure `injector.externalvaultaddr` value because the Vault Server is in the same dc1 cluster.
...
####  dc2 Datacenter
1. Install Vault Injector in dc2 datacenter

    -> In dc2, you will have to configure `externalvaultaddr` value point to the external address of the Vault server in dc1.
...
1. Create a service account with access to Kubenetes API in dc2.  For the dc2 auth method, you first need to create a service account that allows Vault server in dc1 cluster to talk to the Kubernetes API in the dc2 cluster.
...
1. Configure the Vault Kubernetes auth role in the Consul on Kubernetes helm chart. Now you can configure and install Consul in dc1 datacenter.
