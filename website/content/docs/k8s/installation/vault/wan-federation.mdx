...
-> **Note:** For demonstration purposes, you will deploy Vault server in dev mode, although for production installations, this is not recommended. 
...
1. Enable Vault KV Secrets Engine - Version 2
    ...
    Therefore, you must enable the [Vault KV secrets engine - Version 2](https://www.vaultproject.io/docs/secrets/kv/kv-v2).

    ```shell-session
    $ vault secrets enable -path=consul kv-v2
    ```

1. Enable Vault PKI Engine
...
#### dc1 Datacenter
1. Install Vault Injector in your Consul k8s cluster
    A minimal valid installation of Vault Kubernetes must include the Agent Injector which is used for accessing secrets from Vault. 
...
1. Create a service account with access to Kubenetes API in dc2
    For the dc2 auth method, you first need to create a service account that allows Vault server in dc1 cluster to talk to the Kubernetes API in the dc2 cluster.
...
1. Configure Auth Method with JWT token of service account
