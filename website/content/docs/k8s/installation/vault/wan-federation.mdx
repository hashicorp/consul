...
You will need to create auth roles for the consul-k8s components so that they access secrets that they will need. For each auth method in Vault, you will need roles for:
...
For dc2 datacenter, you will need to get the address of the mesh gateway from the **dc1** cluster.  Change your Kubernetes current context to target dc1 and set the `MESH_GW_HOST` environment variable that you will use in the Consul Helm chart for dc2 datacenter.
