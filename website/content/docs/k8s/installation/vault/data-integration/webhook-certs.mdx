...
## Prerequisites
Prior to setting up this data integration between Vault and Consul on Kubernetes, you will need to:  
1. Read and complete the steps in the [Systems Integration](/docs/k8s/installation/vault/systems-integration) section of [Vault as a Secrets Backend](/docs/k8s/installation/vault).
1. Read the [Data Integration Overview](/docs/k8s/installation/vault/data-integration) section of [Vault as a Secrets Backend](/docs/k8s/installation/vault).
...
First, we need to bootstrap the Vault cluster by enabling and configuring the PKI Secrets Engine to be able to serve TLS certificates for the controller and connect inject webhooks.
...
When Vault is configured as the controller and connect inject Webhook Certificate Provider on Kubernetes: 
  ...
  - controller and connect inject each locally update their mutatingwebhookconfigurations so that Kubernetes can relay events.
  - Vault will manage certificate rotation and each of these webhooks will seamlessly received rotated certificates.
...
[certificate issuing URL](https://www.vaultproject.io/api/secret/pki#generate-certificate) so Consul controller and connect inject can
fetch a new certificate/key pair and provide it to the Kubernetes mutatingwebhookconfiguration.
...
Create a Vault role for the controller PKI engine will set the default certificate issuance parameters:
...
Create a Vault role for the connect inject PKI engine will set the default certificate issuance parameters:
