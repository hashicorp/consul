...
## Overview
To use an Vault as the Service Mesh Certificate Provider on Kubernetes, we will need to modify the steps outlined in the [Data Integration](/docs/k8s/installation/vault/data-integration):
...
## Set up per Consul data center
### Create a Vault Kubernetes auth roles that link the policy to each Consul on Kubernetes service account that requires access
