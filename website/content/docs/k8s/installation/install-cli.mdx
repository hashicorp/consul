...
You must install the correct version of the CLI for your Consul on Kubernetes deployment. To deploy a previous version of Consul on Kubernetes, you will need to first download the specific version of the CLI that matches the version of the control plane that you would like to deploy. Refer to [Install a specific version of Consul K8s CLI](#install-a-specific-version-of-the-cli) for details.
...
Complete the following instructions install a specific version of the CLI so that your tool is compatible with your Consul on Kubernetes  control plane. 
