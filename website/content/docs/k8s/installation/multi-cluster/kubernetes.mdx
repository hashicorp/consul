...
If ACLs are enabled, you must next determine the Kubernetes API URL of your _primary_ cluster. The API URL of the
primary cluster must be specified in the config files for all secondary clusters because secondary clusters need
to create global Consul ACL tokens (tokens that are valid in all datacenters) and these tokens can only be created
by the primary datacenter.
...
Keep track of this URL, you'll need it later.
...
```shell-session
$ kubectl get secret consul-federation -n consul --output yaml > consul-federation-secret.yaml
```
...
global:
  ...
  federation:
    enabled: true
    k8sAuthMethodHost: <primary-kubernetes-api-url>
...
1. If ACLs are enabled, change the value of `global.federation.k8sAuthMethodHost` to the full URL (including `https://`) of the primary
   Kubernetes API.
...
```shell-session
$ kubectl exec statefulset/consul-server -n consul -- consul catalog services -datacenter dc2
consul
mesh-gateway
```
