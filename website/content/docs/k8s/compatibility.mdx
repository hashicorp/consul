...
Consul Kubernetes supports Red Hat OpenShift using the `global.openshift` stanza which must be set to `true` for support on OpenShift. Please see [Deploy Consul on RedHat OpenShift](https://developer.hashicorp.com/consul/tutorials/kubernetes/kubernetes-openshift-red-hat) for full instructions on deploying to OpenShift. 
