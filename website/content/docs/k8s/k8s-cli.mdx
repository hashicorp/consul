...
## Install the Consul K8s CLI (latest)
...
1. Download the Consul K8s CLI using the following `curl` command. 

   ```shell-session
   curl --location https://releases.hashicorp.com/consul-k8s/0.39.0/consul-k8s_0.39.0_darwin_amd64.zip --output consul-k8s-cli.zip
   ```
   
1. Unzip the zip file ouput to extract the `consul-k8s` CLI binary. 
...
1. Add the path to your directory
   ...
   ```shell-session
   export PATH=$PATH:$HOME/.consul-k8s/consul-k8s
   ```
