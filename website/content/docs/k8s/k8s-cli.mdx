...
The following example commands install Consul on Kubernetes using custom values, files, or strings that could be set to be utilized by the underlying Consul on Kubernetes Helm chart. These commands work similar to the `helm install` [flags](https://helm.sh/docs/helm/helm_install/#helm-install) that are used to customize the Consul on Kubernetes install. 
