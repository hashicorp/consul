...
The `upgrade` command upgrades the Consul on Kubernetes installation to the current version of the `consul-k8s` cli. Prior to running `consul-k8s upgrade`, a `brew upgrade consul-k8s` should be issued to upgrade the `consul-k8s` CLI prior to upgrading the Consul on Kubernetes components. 
