...
- `consul.hashicorp.com/connect-service-upstreams` - The list of upstream
  ...
  this annotation is optional. This annotation can be either _labeled_ or _unlabeled_. We recommend the labeled format because it has a more consistent syntax and can be used to reference cluster peers as upstreams.
    ...
    - **Labeled**:
      ...
      - Sameness group: Place the sameness group name after the service name followed by `.sg` to specify a sameness group for an upstream service. When a sameness group CRD has `spec.defaultForFailover` set to `true`, this field is not required. Refer to [sameness group configuration entry reference](/consul/docs/connect/config-entries/sameness-group) for more information

        ```yaml
        annotations:
          "consul.hashicorp.com/connect-service-upstreams":"[service-name].[group-name].sg:[port]"
        ```
