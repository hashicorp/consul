...
**Make configuration changes** if using [`ports.grpc`](/consul/docs/reference/agent/configuration-file#grpc_port) in conjunction with any of the following settings that enables encryption:
...
for encrypting communication over gRPC. The existing [`ports.grpc`](/consul/docs/reference/agent/configuration-file#grpc_port) configuration **no longer supports encryption**. As of version 1.14,
[`ports.grpc_tls`](/consul/docs/reference/agent/configuration-file#grpc_tls_port) is the only port that serves encrypted gRPC traffic.
...
For the step by step upgrade procedures see the [Upgrading to 1.10.0](/consul/docs/consul/docs/upgrade/instructions/upgrade-to-1-10-x) documentation.
...
The [`enable_central_service_config`](/consul/docs/reference/agent/configuration-file#enable_central_service_config)
...
See the [Server Performance](/consul/docs/reference/server) guide for more details.
