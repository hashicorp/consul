...
This topic describes considerations and strategies for upgrading Consul deployments running on OpenShift clusters. In addition to upgrading the version of Consul, you may need to update your Helm chart or the release version of the Helm chart.
...
1. Perform a `helm upgrade` and make sure that you specify the correct chart version:
...
1. List all available versions.
  ...
  In this example, chart version `1.4.8` with Consul binary version `1.18.2` is being used.
...
1. If you are updating your `values.yaml` file, do so now. For example, edit the `global.image` stanza to `image: registry.connect.redhat.com/hashicorp/consul:1.20.0-ubi`

1. Take the same `helm upgrade` command you were planning to issue but perform `helm diff upgrade` instead of `helm upgrade`:

  ```shell-session
  $ helm diff upgrade consul hashicorp/consul --namespace consul --version 1.6.0 --values /path/to/your/values.yaml
...
1. To see only updated objects, add `| grep "has changed"`:
  ...
  ```shell-session
  $ helm diff upgrade consul hashicorp/consul --namespace consul --version 1.6.0 --values /path/to/your/values.yaml | grep "has changed"
  default, consul-server, StatefulSet (apps) has changed:
  ```
