...
Lastly, if you are using Consul service mesh, you should set [`enable_central_service_config = true`](/consul/docs/agent/config/config-files#enable_central_service_config)
...
- [Upgrading Specific Versions](/consul/docs/upgrading/upgrade-specific) 
...
Temporarily modify your Consul server configuration so that their [log_level](/consul/docs/agent/config/config-files#_log_level) 
