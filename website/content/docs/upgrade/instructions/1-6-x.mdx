...
older than 1.2.4, please review our [Upgrading to 1.2.4](/consul/docs/upgrade/instructions/1-2-x)
...
are called out in our [Specific Version Details](/consul/docs/upgrade/version-specific#consul-1-6-3)
...
- 1.6.3 introduced the [http_max_conns_per_client](/consul/docs/reference/agent#http_max_conns_per_client) limit. This defaults to 200. Prior to this, connections per client were unbounded. [[GH#7159](https://github.com/hashicorp/consul/issues/7159)]
...
- `acl_datacenter` is now named `primary_datacenter` (review our [docs](/consul/docs/reference/agent#primary_datacenter) for more info)
- `acl_default_policy`, `acl_down_policy`, `acl_ttl`, `acl_*_token` and `enable_acl_replication` options are now specified like this (review our [docs](/consul/docs/reference/agent#acl) for more info):
  ```hcl
  acl {
    enabled                  = true/false
    default_policy           = "..."
