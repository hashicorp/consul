...
older than 1.2.4, please review our [Upgrading to 1.2.4](/consul/docs/consul/docs/upgrade/instructions/upgrade-to-1-2-x)
...
- 1.6.3 introduced the [http_max_conns_per_client](/consul/docs/agent/config/config-files#http_max_conns_per_client) limit. This defaults to 200. Prior to this, connections per client were unbounded. [[GH#7159](https://github.com/hashicorp/consul/issues/7159)]
...
**3.** Upgrade DC2 agents to version 1.6.10 by following our [General Upgrade Process](/consul/docs/consul/docs/upgrade/instructions/general-process). _**Leave all DC1 agents at 1.2.4.**_ You should start observing log messages like this after that:
...
**5.** Upgrade DC1 agents to version 1.6.10 by following our [General Upgrade Process](/consul/docs/consul/docs/upgrade/instructions/general-process).
...
- `acl_datacenter` is now named `primary_datacenter` (review our [docs](/consul/docs/agent/config/config-files#primary_datacenter) for more info)
- `acl_default_policy`, `acl_down_policy`, `acl_ttl`, `acl_*_token` and `enable_acl_replication` options are now specified like this (review our [docs](/consul/docs/agent/config/config-files#acl) for more info):
  ```hcl
  acl {
    enabled                  = true/false
    default_policy           = "..."
