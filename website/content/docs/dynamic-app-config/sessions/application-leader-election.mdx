...
- If ACLs are enabled, a token with the following permissions:
  ...
  You expose the token using the `CONSUL_HTTP_TOKEN` environment variable.
- The `curl` command
...
In case of success, the command exits with exit code `0` and outputs the following:
...
If the lock was already acquired by another node, the command exits with exit code `1` and outputs the following:
