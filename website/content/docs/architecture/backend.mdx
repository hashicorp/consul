...
Consul uses the Raft protocol to manage [server consensus](/consul/docs/concept/consensus), which in turn enables Consul's [reliability and fault tolerance](/consul/docs/concept/reliability) as well as the ability to return [consistent service and health information](/consul/docs/concept/consistency).

The Raft index provides a record of the cluster's state. It tracks interactions between Consul servers as they conduct elections, registers service instances into [the Consul catalog](/consul/docs/concept/catalog), and updates the catalog with the results of service node health checks.
...
Previous versions of Consul used BoltDB as the default LogStore backend. Refer to the [WAL LogStore backend overview](/consul/docs/deploy/server/wal) for more information. To use BoltDB instead of WAL, refer to [Revert storage backend to BoltDB from WAL](/consul/docs/deploy/server/wal/revert/bolt-db).