...
This page describes the Consul agent, which is the core process of Consul.
...
In addition to the core agent operations, server nodes participate in the [consensus quorum](/consul/docs/concept/consensus).
...
1. **An agent joins a cluster**, which enables the agent to discover agent peers.
   Agents join clusters on startup when the [`join`](/consul/commands/join) command is issued or according the [auto-join configuration](/consul/docs/deploy/server/vm/cloud-auto-join).
...
-> **Note:** Updating the catalog is only possible if the servers can still [form a quorum](/consul/docs/concept/consensus).
...
You can define a set of rate limiting configurations that help operators protect Consul servers from excessive or peak usage. The configurations enable you to gracefully degrade Consul servers to avoid a global interruption of service. Consul supports global server rate limiting, which lets configure Consul servers to deny requests that exceed the read or write limits. Refer to [Traffic Rate Limits Overview](/consul/docs/manage/rate-limit).
...
- [Server Performance](/consul/docs/reference/server)
