...
The server agents are deployed as a `StatefulSet` and use persistent volume claims to store the server state. This state ensures that the [node ID](/consul/docs/reference/agent/configuration-file#node_id) is persisted so that servers can be rescheduled onto new IP addresses without causing issues. 
