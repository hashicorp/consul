...
Each Consul datacenter maintains its own catalog of services and their health. By default, the information is not replicated across datacenters. Cluster peering and WAN federation are two multi-datacenter deployment models that enable service connectivity across datacenters.
