...
despite the failure of one or more components. Voting server agent configuration determines Consul's fault tolerance.
...
- <EnterpriseInline />[Use redundancy zones to improve fault tolerance](#redundancy-zones).
...
The cloud or on-premise infrastructure underlying your [Consul datacenter](/consul/docs/install/glossary#datacenter) may be split into several availability zones.
...
Consul Enterprise provides a way to improve fault tolerance without this performance penalty. Refer to [Redundancy zones](#redundancy-zones) for details.
