...
This topic describes how to configure the `property-override` extension so that you can set and remove individual properties on the Envoy resources Consul generates. Refer to [Configure Envoy proxy properties](/consul/docs/reference/proxy/envoy-extensions/usage/property-override) for usage information.
...
The `Path` field does not support addressing array elements or protobuf map field entries. Refer to [Constructing paths](/consul/docs/reference/proxy/envoy-extensions/usage/property-override#constructing-paths) for information about how to construct paths. 
...
-> **Note:** Using separate patches per field preserves any existing configuration of other fields in `outlier_detection` that may be directly set by Consul, such as [`enforcing_consecutive_5xx`](https://developer.hashicorp.com/consul/docs/reference/proxy/envoy#enforcing_consecutive_5xx).
