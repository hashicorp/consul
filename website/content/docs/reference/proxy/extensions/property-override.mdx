...
Using separate patches per field preserves any existing configuration of other fields in `outlier_detection` that may be directly set by Consul, such as [`enforcing_consecutive_5xx`](/consul/docs/reference/proxy/envoy#enforcing_consecutive_5xx).
