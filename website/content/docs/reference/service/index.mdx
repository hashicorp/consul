...
- [`port`](#port): number
- [`ports`](#ports): map (service discovery only)
  - [`name`](#ports): string
  - [`port`](#ports): number 
...
| :-------- | :------------------------------------------------------------------------ | :------ | :------ |
| `name`    | Specifies a name for the port. This value is addressable with Consul DNS. | string  | None    |
| `port`    | Specifies a numeric port where the service is available.                  | number  | None    |
| `default` | Sets a port as the default when one is not included.                      | boolean | `false` |
...
When `tagged_address` is not configured in a service definition, Consul uses the `address` and `port` properties in the definition. When a multi-port service is defined using the `ports` parameter, Consul uses the port configured as the default in the `ports.default` parameter.
