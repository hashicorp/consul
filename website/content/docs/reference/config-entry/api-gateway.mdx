...
description: >-
  Learn how to configure a Consul API gateway on VMs.
---
...
Specifies the Enterprise [namespace](/consul/docs/segment/namespace) where the certificate can be found.
...
Specifies the Enterprise [admin partition](/consul/docs/segment/admin-partition) where the certificate can be found.
...
Listeners = [
    {
        ...
        port     = 9001
        protocol = "http"
        # override and default are backed by the same type of data structure, refer to  the following section for more on how they interact
        override = {
            JWT = {
                Providers = [
                    {
