...
The `-config-file` or `-config-dir` flag is required. Use the flag to specify the [CTS instance configuration file](/consul/docs/reference/cts/configuration) or directory containing several configuration files to start a CTS cluster. 
