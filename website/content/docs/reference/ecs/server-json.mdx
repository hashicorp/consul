...
This topic provides reference information about the JSON schema used to build the `config.tf` file. Refer to [Configure Consul server settings](/consul/docs/register/service/ecs/automatic#configure-consul-server-settings) for information about how Consul on ECS uses the JSON schema. 
