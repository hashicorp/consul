...
- [`working_dir`](#workfing-dir): string
- [`port`](#port): number
- [`id`](#id): string
- [`syslog`](#syslog): map
  ...
  - [`name`](#syslog): string
- [`buffer_period`](#bufferperiod): map
...
- [`tls`](#tls): map
  ...
  - [`ca_cert`](#tls): string
- [`license_path`](#licensepath): string | <EnterpriseAlert inline /> | deprecated, use `license` instead  
...
| `auto_retrieval` |  Map that contains the `enabled` field. Set `enabled` to `true` to enable CTS to automatically retrieve a license. Refer to [Eanble auto-retrieval](/consul/docs/cts/configure#auto-retrieval) for additional information. | Review [Auto-Retrieval](/consul/docs/nia/configuration#auto-retrieval) for defaults. |
...
The `task` block contains the following blocks:
...
The `condition "services"` triggers the task when CTS detects services that match either the regular expression configured in `regexp` field or services listed by name in `names` field. Either `regexp` or `names` must be configured, but not both.
...
- You cannot configure the task with the `services` field (deprecated).
- You cannot configure the task with a `module_input "services"` or `source_input "services"` (deprecated) block.
...
The `module_input "services"` specifies services registered with Consul that are tagged with metadata that the task uses as Terraform module inputs. Refer to [Services module input](/consul/docs/cts/create-terraform-module#services-module-input) for additional information.
...
<!-- TODO distribute this information across all pertinent conf items -->
