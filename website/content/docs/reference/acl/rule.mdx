...
For more detailed information, refer to  the [Consul Sentinel documentation](/consul/docs/agent/sentinel).
...
* Provide an ACL token with `service` and `check` definitions at registration time.
  ...
  Refer to the [services](/consul/docs/register/service/vm/define) and [checks](/consul/docs/register/service/vm/health-checks) documentation for examples. You can also pass tokens to the [HTTP API](/consul/api-docs) for operations that require them.
...
2. Providing an ACL token with service and check definitions at registration
   ...
   [checks](/consul/docs/register/service/vm/health-checks). Tokens may also be passed to the [HTTP
   ...
   documentation](/consul/docsreference-cli/agent#_data_dir) for information about securing
   access.
...
Refer to [ACL requirements for intentions](/consul/docs/secure-mesh/intentions/create#acl-requirements)
