...
# Access Control List (ACL) JSON Web Token (JWT) auth method configuration reference
...
main [auth method documentation](/consul/docs/secure-consul/acl/auth-methods).

Both the [`jwt`](/consul/docs/secure-consul/acl/auth-methods/jwt) and the
[`oidc`](/consul/docs/secure-consul/acl/auth-methods/oidc) auth method types allow additional
