...
- `enable_script_checks` Equivalent to the [`-enable-script-checks` command-line flag](/consul/docs/agent/config/cli-flags#_enable_script_checks).
  ...
  ACLs must be enabled for agents and the `enable_script_checks` option must be set to `true` to enable script checks in Consul 0.9.0 and later. Refer to  [Registering and Querying Node Information](/consul/docs/security/acl/acl-rules#registering-and-querying-node-information) for related information.
...
- `encrypt_verify_incoming` - This is an optional
  ...
  Refer to  [this section](/consul/docs/security/encryption#configuring-gossip-encryption-on-an-existing-cluster)
