...
For information about how to start Consul agents, refer to [Starting the Consul Agent](/consul/docs/agent#starting-the-consul-agent).
...
- `acl_replication_token` ((#acl_replication_token_legacy)) - **Deprecated
  ...
  [`acl_down_policy`](/consul/docs/reference/agent/configuration-file#acl_down_policy) is set to "extend-cache", tokens not
...
- `dns_config` This object allows a number of sub-keys
  to be set which can tune how DNS queries are serviced. Refer to [DNS caching](/consul/docs/discover/dns/scale) for more information.
