...
- `auto_config` This object allows setting options for the `auto_config` feature.
  ...
  - `authorization` This object controls how a Consul server will authorize `auto_config`
    ...
    - `static` This object controls configuring the static authorizer setup in the Consul
      ...
      Auth Method](/consul/docs/secure/acl/auth-methods/jwt).
...
- `enable_script_checks` Equivalent to the [`-enable-script-checks` command-line flag](/consul/commands/agent#_enable_script_checks).
  ...
  ACLs must be enabled for agents and the `enable_script_checks` option must be set to `true` to enable script checks in Consul 0.9.0 and later. See [Registering and Querying Node Information](/consul/docs/secure/acl/rules#registering-and-querying-node-information) for related information.
...
- `acl` ((#acl)) - This object allows a number of sub-keys to be set which
  ...
  - `enable_token_replication` ((#acl_enable_token_replication)) - By default
    ...
    [auth methods](/consul/docs/secure/acl/auth-methods) in connected secondary datacenters.
  ...
  - `tokens` ((#acl_tokens)) - This object holds all of the configured
    ...
    - `default` ((#acl_tokens_default)) - When provided, this agent will
      ...
      instead of the [anonymous token](/consul/docs/secure/acl/tokens#anonymous-token).
    ...
    - `dns` ((#acl_tokens_dns)) - Specifies the token that agents use to request information needed to respond to DNS queries.
      ...
      strongly recommend using the `dns` token. Create DNS tokens using the [templated policy](/consul/docs/secure/acl/token/dns-token#create_a_dns_token)
...
- `acl_replication_token` ((#acl_replication_token_legacy)) - **Deprecated
  ...
  running Consul 0.7 or later. When provided, this will enable [ACL replication](/consul/docs/secure/acl/enable-federated-datacenters)
...
- `domain` Equivalent to the [`-domain` command-line flag](/consul/commands/agent#_domain).
...
- `encrypt_verify_incoming` - This is an optional
  ...
  Refer to [Enable gossip encryption for existing datacenters](/consul/docs/secure/encryption/gossip/enable/existing)
...
- `rejoin_after_leave` Equivalent to the [`-rejoin` command-line flag](/consul/commands/agent#_rejoin).

- `retry_join` - Equivalent to the [`-retry-join`](/consul/commands/agent#retry-join) command-line flag.

- `retry_interval` Equivalent to the [`-retry-interval` command-line flag](/consul/commands/agent#_retry_interval).

- `retry_max` - Equivalent to the [`-retry-max`](/consul/commands/agent#_retry_max) command-line flag.

- `retry_join_wan` Equivalent to the [`-retry-join-wan` command-line flag](/consul/commands/agent#_retry_join_wan). Takes a list of addresses to attempt joining to WAN every [`retry_interval_wan`](#_retry_interval_wan) until at least one join works.

- `retry_interval_wan` Equivalent to the [`-retry-interval-wan` command-line flag](/consul/commands/agent#_retry_interval_wan).
...
- `disable_host_node_id` Equivalent to the [`-disable-host-node-id` command-line flag](/consul/commands/agent#_disable_host_node_id).
...
- `ui` - **This field is deprecated in Consul 1.9.0. Refer to  the [`ui_config.enabled`](#ui_config_enabled) field instead.**
  Equivalent to the [`-ui`](/consul/commands/agent#_ui) command-line flag.
...
- `ui_dir` - **This field is deprecated in Consul 1.9.0. Refer to  the [`ui_config.dir`](#ui_config_dir) field instead.**
  Equivalent to the [`-ui-dir`](/consul/commands/agent#_ui_dir) command-line
