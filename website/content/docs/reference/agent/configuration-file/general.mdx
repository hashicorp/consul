...
- `reporting`<EnterpriseAlert inline /> - This option allows options for HashiCorp reporting.
    ...
    - `snapshot_retention_time` ((#snapshot_retention_time))  - This option controls the time the license utilization snapshots will be retained in Consul's internal Raft-replicated state. The default value is `9600h` (~400 days). Snapshot are taken automatically every 24 hours, even if reporting to HashiCorp is disabled, so using the default value will keep up to 400 reports in Consul's internal Raft-replicated state.   
