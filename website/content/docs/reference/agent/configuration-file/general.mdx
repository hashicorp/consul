...
- `disable_kv_key_validation` (boolean, default is `false`): When set to `true`, it disables KV key validation that prevents path traversal attacks and other security issues. We strongly recommend keeping validation enabled (false) for security reasons unless you have a specific legacy compatibility requirement.
