...
- `reporting`<EnterpriseAlert inline /> - This option allows options for HashiCorp reporting.
    ...
    - `snapshot_retention_time` ((#snapshot_retention_time))  - This option controls the time Consul retains the license utilization snapshots in its internal Raft-replicated state. The default value is `9600h`, which is approximately 400 days. Snapshot are taken automatically every 24 hours, even when reporting to HashiCorp is disabled, so using Consul will keep 400 reports in its internal Raft-replicated state by default.   
