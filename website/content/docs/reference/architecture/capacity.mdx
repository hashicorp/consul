...
| ------------- | ------------------------- | ------------------------ | ------------------------ |
| Read-heavy    | Instances of type `m5.4xlarge (AWS)`, `Standard_D16s_v3 (Azure)`, `n2-standard-16 (GCP)` | Raft RPCs calls, DNS queries, key/value retrieval | [Read replicas](/consul/docs/manage/read-replica) |
...
You may need to adjust Raft to suit your specific environment. Adjust the [`raft_multiplier` configuration](/consul/docs/reference/agent/configuration-file#raft_multiplier) to define the trade-off between leader stability and time to recover from a leader failure.
