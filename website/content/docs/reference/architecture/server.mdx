...
In Consul 0.7, the default server [performance parameters](/consul/docs/reference/agent/configuration-file#performance)
...
[performance parameters](/consul/docs/reference/agent/configuration-file#performance) back to Consul's original
...
- Spurious leader elections can be caused by networking
  ...
  parameters](/consul/docs/reference/agent/configuration-file#performance) configuration now gives you tools
...
- For DNS-heavy workloads, configuring all Consul agents in a cluster with the
  [`allow_stale`](/consul/docs/reference/agent/configuration-file#allow_stale) configuration option will allow reads to
...
- In Consul 0.9.3 and later, a new [`limits`](/consul/docs/reference/agent/configuration-file#limits) configuration is
...
If the leader server is out of memory or the disk is full, the server eventually stops responding, loses its election and cannot move past its last commit time. However, by configuring `max_stale` and setting it to a large value, Consul will continue to respond to queries during such outage scenarios. ([max_stale documentation](/consul/docs/reference/agent/configuration-file#max_stale)).
...
**Read-heavy** clusters may take advantage of the [enhanced reading](/consul/docs/manage/read-replica) feature (Enterprise) for better scalability. This feature allows additional servers to be introduced as non-voters. Being a non-voter, the server will still participate in data replication, but it will not block the leader from committing log entries.
...
To prevent any CPU spikes from a misconfigured client, RPC requests to the server should be [rate limited](/consul/docs/reference/agent/configuration-file#limits)
...
[`csr_max_per_second`](/consul/docs/reference/agent/configuration-file#ca_csr_max_per_second) and
[`csr_max_concurrent`](/consul/docs/reference/agent/configuration-file#ca_csr_max_concurrent).
