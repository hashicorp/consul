...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `cloud` ((#v-global-cloud)) - Enables installing an HCP Consul Central self-managed cluster.
    ...
    - `enabled` ((#v-global-cloud-enabled)) (`boolean: false`) - If true, the Helm chart links a [self-managed cluster to HCP](/hcp/docs/consul/self-managed).
      Use this to either [configure a new cluster](/hcp/docs/consul/self-managed/new)
      or [link an existing one](/hcp/docs/consul/self-managed/existing).

      Do not enable this setting for [HCP Consul Dedicated clusters](/hcp/docs/consul/dedicated).
      It is strictly for linking self-managed clusters.

    - `resourceId` ((#v-global-cloud-resourceid)) - The resource ID of the HCP Consul Central cluster to link to. For example:
  ...
  - `dualStack` ((#v-global-dualstack))
    ...
    - `defaultEnabled` ((#v-global-dualstack-defaultenabled)) (`boolean: false`)
  ...
  - `snapshotAgent` ((#v-server-snapshotagent)) - <EnterpriseAlert inline /> Values for setting up and running
    ...
    - `extraVolumes` ((#v-server-snapshotagent-extravolumes)) (`array<map>`) - A list of extra volumes to mount onto the snapshot agent. Use this block
      ...
      - `name` - Name of the configMap or secret to mount. This specification also controls
        the path it mounts to. Consul mounts the volume to `/consul/userconfig/<name>`.

      The snapshot agent does not attempt to load any volumes passed in this stanza.
...
- `client` ((#v-client)) - Values that configure running a Consul client on Kubernetes nodes.
  ...
  - `livenessProbe` ((#v-client-livenessprobe)) (`string: ""`) - Override the default livenessProbe for the DaemonSet consul-client container.
  ...
  - `readinessProbe` ((#v-client-readinessprobe)) (`string: ""`) - Override the default readinessProbe for the DaemonSet consul-client container.
    Refer to the Kubernetes documentation on how to [Configure Liveness, Readiness and Startup Probes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/)
...
- `dns` ((#v-dns)) - Configuration for DNS configuration within the Kubernetes cluster.
  ...
  - `proxy` ((#v-dns-proxy)) - Configures Consul DNS proxy for Kubernetes deployment.

    - `enabled` ((#v-dns-proxy-enabled)) (`boolean: false`) - True if you want to enable Consul DNS proxy for Kubernetes.
...
- `syncCatalog` ((#v-synccatalog)) - Configure the catalog sync process to sync K8S with Consul
  ...
  - `syncLoadBalancerEndpoints` ((#v-synccatalog-syncloadbalancerendpoints)) (`boolean: false`) - If true, Consul syncs LoadBalancer service endpoints instead of ingress addresses. 
...
- `connectInject` ((#v-connectinject)) - Configures the automatic Connect sidecar injector.
  ...
  - `apiGateway` ((#v-connectinject-apigateway)) - Configuration settings for the Consul API Gateway integration.
    ...
    - `managedGatewayClass` ((#v-connectinject-apigateway-managedgatewayclass)) - Configuration settings for the GatewayClass installed by Consul on Kubernetes.
      ...
      - `resourceJob` ((#v-connectinject-apigateway-managedgatewayclass-resourcejob)) - This value contains settings related to the `gateway_resources_job` that runs on Helm install.
        ...
        - `resources` ((#v-connectinject-apigateway-managedgatewayclass-resourcejob-resources)) (`map`) - The resource request, such as CPU and memory, for the `server-acl-init` and `server-acl-init-cleanup` pods.
          This should be a YAML map corresponding to a Kubernetes
          [ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.27/#resourcerequirements-v1-core)
          object.
  ...
  - `cni` ((#v-connectinject-cni)) - Configures consul-cni plugin for Consul Service mesh services
    ...
    - `autorotateToken` ((#v-connectinject-cni-autorotatetoken)) (`boolean: false`) - If true, then use projected service account tokens for the CNI plugin. Uses auto-rotated projected token every time when interacting with Kubernetes API.
    ...
    - `tokenExpiration` ((#v-connectinject-cni-tokenexpiration)) (`Integer: 3600`) - Set the CNI plugin ServiceAccount token validity period. Applicable only when `autorotateToken` is true.
      This may not work in some environments this may not work, such as Azure 1.30+ with OIDC enabled, as providers may have native expiration limits.
      Optional and configured to 1 hour by default. Value is in seconds.
  ...
  - `dataplaneAsSidecarInitContainer` ((#v-connectinject-dataplaneassidecarinitcontainer))
    ...
    - `defaultEnabled` ((#v-connectinject-dataplaneassidecarinitcontainer-defaultenabled)) (`boolean: false`) - If true, the  Consul dataplane runs as an init container with the restart policy set to "Always" and uses startup probe to determine readiness.
      Also, Kubelet waits for the Consul dataplane init container to be ready before starting the application container.

    - `initialProbeCheckDelaySeconds` ((#v-connectinject-dataplaneassidecarinitcontainer-initialprobecheckdelayseconds)) (`integer: 1`) - Configures how long the k8s startup probe waits initially before performing the first check.

    - `probeCheckPeriodSeconds` ((#v-connectinject-dataplaneassidecarinitcontainer-probecheckperiodseconds)) (`integer: 1`) - Configures how often the k8s startup probe checks the readiness of the consul-dataplane init container.
...
- `telemetryCollector` ((#v-telemetrycollector))
  ...
  - `cloud` ((#v-telemetrycollector-cloud))
    ...
    - `resourceId` ((#v-telemetrycollector-cloud-resourceid)) - The resource ID of the HCP Consul Central cluster to push metrics for. For example:
      `organization/27109cd4-a309-4bf3-9986-e1d071914b18/project/fcef6c24-259d-4510-bb8d-1d812e120e34/hashicorp.consul.global-network-manager.cluster/consul-cluster`

      This is used for HCP Consul Central-linked or HCP Consul Dedicated clusters where `global.cloud.resourceId` is unset. For example, when using external servers
    ...
    - `clientId` ((#v-telemetrycollector-cloud-clientid)) - The client ID portion of a [service principal](/hcp/docs/hcp/iam/service-principal) with authorization to push metrics to HCP.
    ...
    - `clientSecret` ((#v-telemetrycollector-cloud-clientsecret)) - The client secret portion of a [service principal](/hcp/docs/hcp/iam/service-principal) with authorization to push metrics to HCP.
