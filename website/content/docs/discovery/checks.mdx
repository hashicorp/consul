...
- `UDP + Interval` - These checks make the client send periodically UDP datagrams
  to the specified IP/hostname and port, waiting `interval` amount of time 
  between attempts (e.g. 30 seconds), if any response from the UDP server is 
  received, the server is marked as healthy, 
  any other case sets the status as `critical`.
  By default, UDP checks will be configured with a request timeout of 10 seconds.
  It is possible to configure a custom UDP check timeout value by specifying the
  `timeout` field in the check definition. If any timeout on read exists, this will
