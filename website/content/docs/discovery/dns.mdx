...
```text
[<tag>.]<service>.service[.<datacenter-or-peer>].<domain>
```

The `tag` is optional. If no tag is provided, no filtering is done on tag.

The `datacenter-or-peer` is also optional.
If `datacenter-or-peer` is not provided, the datacenter of the Consul agent is assumed.
A provided value must be the name a WAN federated datacenter or a cluster peer
of the Consul agent acting as the DNS server.
To lookup a mesh-participating service that's imported from a cluster peer,
refer to [service virtual IP lookup](#service-virtual-ip-lookups) instead.
...
primary in a WAN federated datacenter or cluster peer named `dc2`,
we could query `primary.postgresql.service.dc2.consul.`
...
- No filtering on service tag:
   ...
   ```text
  _<service>._tcp[.service][.<datacenter-or-peer>].<domain>
  ```

- Filtering on service tag specified in the RFC 2782 protocol field:

   ```text
  _<service>._<tag>[.service][.<datacenter-or-peer>].<domain>
  ```
...
To lookup a mesh-participating service that's imported from a cluster peer,
refer to [service virtual IP lookup for Consul Enterprise](#service-virtual-ip-lookups-for-consul-enterprise) instead.

#### Canonical format

Use the following query formats to specify namespace, partition, datacenter, or peer
...
- Lookup a service, optionally in a cluster peer:
  ...
  `api.foo.ns.billing.peer.k8s-app-1.partition.consul`.
...
- Specify both namespace and datacenter:
  ...
  ```text
  [<tag>.]<service>.service.<namespace>.<datacenter-or-peer>.<domain>
  ```
...
- The downstream service has [Transparent Proxy](/docs/connect/transparent-proxy) enabled
...
To lookup services outside the mesh or not imported from a cluster peer,
refer to non-virtual [service lookups](#service-lookups) instead.
...
- Specify peer and optional namespace in the lookup:
