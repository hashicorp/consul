...
- Lookup a service in a cluster peer:
  ...
  The `<peer>` label identifies the cluster peer in which
  to lookup the specified `<service>`.
...
- Lookup a service in a WAN federated datacenter:
  ...
  We recommend including the optional `.dc` label to improve readability.
  The `<datacenter>` label identifies the WAN federated datacenter in which
  to lookup the specified `<service>`.

You can filter the results to only include instances of a service with a given tag
by specifying `<tag>` at the start of the lookup.
If you do not specify a tag, the lookup returns all instances of the service.
Refer to the [service configuration documentation](/consul/docs/discovery/services#service)
for information about adding tags to your service.
...
You can use [RFC 2782](https://tools.ietf.org/html/rfc2782) lookups to query for services
as an alternative to [standard lookups](#standard-lookup).
...
- Lookup a local service with filtering on service tag
  specified in the RFC 2782 protocol field:

   ```text
  _<service>._<tag>[.service].<domain>
  ```

We recommend including the optional `.service` label to improve readability.
...
- Lookup a service, optionally in a cluster peer:
  ...
  This format supports `.service` and `.virtual` service lookup types.
  ...
  a Consul agent in the `default` partition acts as the DNS server,
  the `k8s-app-1` partition is in the same datacenter,
  and the `k8s-app-1` partition has a cluster peer named `billing`.
  To perform a lookup of service `api` in namespace `foo`
