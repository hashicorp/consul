...
- Filtering on service tag specified *after* the RFC 2782 protocol field

  ```text
  _<service>._<protocol>.<tag>.service[.<datacenter>].<domain>
  ```
...
To filter results on a service tag, specify the tag according to the previous examples.
