...
The `address` field can be used to specify a service-specific IP address or hostname. 

Because hostname can be used, **we do not perform any validation of this field server side**.

By default, the IP address of the agent is used, and this does not need to be provided.
