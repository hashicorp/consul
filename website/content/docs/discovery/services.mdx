...
1. **Define services and health checks**: A service definition lets you define various aspects of the service, including how it is discovered by other services in the network. You can define health checks in the service definitions to verify the health of the service. Refer to [Define Services](/consul/docs/services/usage/define-services) and [Define Health Checks](/consul/docs/services/usage/define-health-checks) for additional information.

1. **Register services and health checks**: After defining your services and health checks, you must register them with a Consul agent. Refer to [Register Services and Health Checks](/consul/docs/services/usage/register-services-and-health-checks) for additional information.

1. **Discover services**: After registering your services and health checks, other services in your network can use the DNS to perform static or dynamic lookups to access your service. Refer to [Discover Consul Nodes and Services Overview](/consul/docs/services/usage/discovery/discovery-overview) for additional information about the different ways to discover services in your datacenters.

### Multiple services
If the services in your network have common characteristics, such as sharing the same admin partition or namespace or connecting to the same upstreams, you can create and register a `service-defaults` configuration entry to set several default or baseline configurations for services you want to register with Consul. You can also define override configurations for specific upstreams or service instances. Refer to [Define Service Defaults](/consul/docs/services/usage/define-services#define-service-defaults) for additional information.
