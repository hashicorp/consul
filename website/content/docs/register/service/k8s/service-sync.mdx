...
When using service discovery features, Consul automatically syncs its catalog with the Kubernetes service catalog. Both single and multi-port services are available. If a Kubernetes service has multiple ports, Consul registers the first port in the series as the service's default port.

For Consul service mesh, the catalog must register the service at a single port. Consul uses the services default port.

You can override the default service port with annotation:
