...
Refer to [Lambda Function Registration Requirements](/consul/docs/lambda/registration) for additional information about registering Lambda functions into Consul.
...
Refer to [Invoke Lambda Functions from Services](/consul/docs/connect/lambda/invoke-function) for details.
...
Refer to [Invoke Services from Lambda Functions](/consul/docs/lambda/invoke-from-lambda) for additional information about registering Lambda functions into Consul.

Consul mesh gateways are required to send requests from Lambda functions to mesh services. Refer to [Mesh Gateways](/consul/docs/link/mesh-gateway/) for additional information.
