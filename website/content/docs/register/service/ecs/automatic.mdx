...
- Enable [TLS encryption](/consul/docs/secure-consul/encryption#rpc-encryption-with-tls) on your Consul servers so that they can communicate securely with Consul containers over gRPC.
- Enable [access control lists (ACLs)](/consul/docs/secure-consul/acl) on your Consul servers. ALCs provide authentication and authorization for access to Consul servers on the mesh.
...
- [Create a service token](/consul/docs/secure-consul/acl/token/service-token)
...
- [WAN Federation via Mesh Gateways](/consul/docs/link/mesh-gateway/enable)
- [Service-to-service Traffic Across Datacenters](/consul/docs/link/mesh-gateway/federation)
...
1. On the Consul server, create a policy that grants the following access for the controller:
    ...
    The policy allows Consul to generate a token linked to the policy. Refer to [Create a service token](/consul/docs/secure-consul/acl/token/service-token) for instructions.
1. Create a token and link it to the ACL controller policy. Refer to the [ACL tokens documentation](/consul/docs/secure-consul/acl/tokens) for instructions.
...
- [Configure the ECS task bind address](/consul/docs/register/service/ecs/configure-task-bind-address)
...
Configure the following options in the `gateway-task` to enable [WAN federation through mesh gateways](/consul/docs/link/mesh-gateway/enable).
...
When federating Consul datacenters over the WAN with ACLs enabled, enable [ACL Token replication](/consul/docs/secure-consul/acl/federation) on all server and client agents in all datacenters.
