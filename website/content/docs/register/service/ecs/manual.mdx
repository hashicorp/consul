...
1. Create an ECS task role and configure an `iam:GetRole` permission so that it can fetch itself. Refer to [IAM Policies](/consul/docs/secure/acl/auth-methods/aws-iam#iam-policies) for instructions.
...
Refer to the [auth method configuration parameters documentation](/consul/docs/secure/acl/auth-methods/aws-iam#config-parameters) for additional information.
