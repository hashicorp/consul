---
layout: docs
page_title: Register services on ECS (Consul Enterprise)
description: >-
...
# Register services on ECS (Consul Enterprise)
...
!> **Warning:** Consul Enterprise is currently only fully supported when [ACLs are enabled](/consul/docs/register/service/ecs/automatic#secure-configuration-requirements).
...
Consul on ECS supports [admin partitions](/consul/docs/segment/admin-partition) and [namespaces](/consul/docs/segment/namespace) when Consul Enterprise servers and clients are used. These features have the following requirements:
...
Services are assigned to admin partitions and namespaces through the use of [task tags](/consul/docs/register/service/ecs/manual#configure-task-tags).
