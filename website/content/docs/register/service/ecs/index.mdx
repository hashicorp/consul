...
- [Create a service token](/consul/docs/secure/acl/token/service-token)
...
1. On the Consul server, create a policy that grants the following access for the controller:
    ...
    The policy allows Consul to generate a token linked to the policy. Refer to [Create a service token](/consul/docs/secure/acl/token/service-token) for instructions.
1. Create a token and link it to the ACL controller policy. Refer to the [ACL tokens documentation](/consul/docs/secure/acl/tokens) for instructions.
...
When federating Consul datacenters over the WAN with ACLs enabled, enable [ACL Token replication](/consul/docs/secure/acl/federation) on all server and client agents in all datacenters.
