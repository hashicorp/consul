...
The following example creates a mesh service called `example-mesh-service`. Routes that use `example-mesh-service` as a backend will route traffic to the Consul service in the local datacenter named `echo`.
