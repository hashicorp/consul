...
Specifies the value to add to privileged ports ( ports < 1024). If this value is set, any privileged port on the gateway
will have this value added to it. This is useful to avoid running containers with privileged ports.
...
### openshiftSCCName
Specifies the Openshift Security Context Constraint for the gateway-class-config to use
for gateway containers
* Type: string
* Required: optional
