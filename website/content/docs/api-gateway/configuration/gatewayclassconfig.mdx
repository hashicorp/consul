...
Specifies the value to add to privileged ports ( ports < 1024). If this value is set, any privileged port on the gateway will have this value added to it. This is useful to avoid running containers with privileged ports.
...
### openshiftSCCName
Specifies the Openshift Security Context Constraint for the `GatewayClassConfig` to use. View available default security context constraints on [Openshift](https://docs.openshift.com/container-platform/4.13/authentication/managing-security-context-constraints.html#default-sccs_configuring-internal-oauth).
* Type: string
* Required: optional
