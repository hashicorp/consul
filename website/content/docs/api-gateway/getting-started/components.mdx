...
kind: Gateways
metadata:
  name: example-gateway  # This name will be referenced when configuring other components described below
  namespace: consul
spec:
  ...
  - protocol: HTTPS
    ...
    allowedRoutes:
      namespaces:
        from: Same  # Only routes within the "consul" namespace may attach to this Gateway
    tls:
      certificateRefs:
        - name: consul-server-cert  # This Gateway can reference a Secret containing the TLS certificate
```
