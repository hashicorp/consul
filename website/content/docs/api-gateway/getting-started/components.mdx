...
metadata:
  ...
  # This HTTPRoute is in the same namespace as the Gateway above so it is allowed to attach
  namespace: default
spec:
  parentRefs:
  # This HTTPRoute should attach to the Gateway defined above
  - name: example-gateway
  rules:
...
As demonstrated above, a `HTTPRoute` can have multiple rules. Each rule can match requests based on path, header or query parameter.
To match based on a header value, you would use a rule such as:
...
Similarly, to match based on a query parameter, you would use a rule such as:
...
You can also combine these examples into a single rule that requires both a header and query parameter to match.
...
for every request based only on the list of rules.
...
Optionally, a `HTTPRoute` can include filters to specify additional processing steps such as setting, adding, or removing headers.
...
In the examples above, the resources that we've configured all reside in the same namespace; however, that will not always be the case.

By default, relationships across the namespace boundary are not allowed. A `ReferencePolicy` enables cross-namespace references between objects.
There are two scenarios where the `ReferencePolicy` comes into play:

1. A `HTTPRoute` or `TCPRoute` in one namespace references a backend `Service` in another namespace
1. A `Gateway` in one namespace references a certificate `Secret` in another namespace
...
### Route References Backend in Different Namespace

The `HTTPRoute` below in `namespace1` needs to reference a backend in `namespace2`. The accompanying `ReferencePolicy` in `namespace2` explicitly grants
access to `HTTPRoutes` from `namespace1` to any `Service` in `namespace2`.

<CodeBlockConfig filename="example-cross-namespace-route.yaml">
...
### Gateway References Secret in Different Namespace

The `Gateway` below in `namespace1` needs to reference a `Secret` in `namespace2`. The accompanying ReferencePolicy` in `namespace2` explicitly grants
access to `Gateways` from `namespace1` to the specific `Secret` in `namespace2`.

<CodeBlockConfig filename="example-cross-namespace-gateway.yaml">
...
spec:
  ...
  - protocol: HTTPS
    ...
    tls:
      certificateRefs:
        - name: certificate
          namespace: namespace2  # References a Secret in another namespace
...
spec:
  from:
    - group: gateway.networking.k8s.io
      kind: Gateway
      namespace: namespace1  # Matches the namespace containing the Gateway
  to:
    - group: ""
      kind: Secret
      name: certificate  # Optionally scopes access to a specific Secret
```
</CodeBlockConfig>
