...
## Pre-requisites
- Consul and Consul-API-Gateway have been sucesfully installed on your cluster
- `connectInject` was set to true when Consul was installed
- Your firewall rules have been configured so HTTP and HTTPS is allowed
...
The above gateway configuration creates a gateway that explicitly allows routes from `All namespaces` to connect to the gateway. Additionally, it also creates a service named `example-http-service` that routes to any pod with the `app: example-service` label, a deployment that serves a `hashicorp/echo-service` pod for the service to route to, and a `ServiceDefaults` object to configure the service to serve http.
Finally, it creates an`HTTPRoute` named `example-route` to attach to `example-gateway`.

In order to apply this stack to your cluster save the above file to your local machine, and run the following command.
...
in the previous step into the browser.


## Gotchas

### Gateway never gets a status

Sometimes its possible to create a gateway that never reports a status, and will be stuck
without an address assigned, as follows.

```log
NAME              CLASS                ADDRESS                  READY   AGE
example-gateway   consul-api-gateway                                    24m
```

Usually this happens because consul wasn't installed properly, or because the Gateway was created
before the controller was ready. Try deleting and recreating your gateway...

```shell-session
$ kubectl delete gateway example-gatewat
$ kubectl apply -f example.yaml
```

If that doesn't work, try uninstalling and reinstalling consul.

If that doesn't work, it's possible that you had values in your values.yaml that do not match your kubernetes infrastructure.
For example, the default number of Consul server replicas is 3, but if you only have 2 nodes on your cluster
running Consul will never successfully elect a leader and your Gateway will never reconcile and report a status. If you reinstall
consul on a cluster with three nodes available, it should install successfully.
