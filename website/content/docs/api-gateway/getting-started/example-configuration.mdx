...
If that doesn't work, try uninstalling and reinstalling consul. Likely you had values in your values.yaml that do not match
your kubernetes infrastructure. For example, the default number of Consul server replicas is 3, but if you only have two nodes
running Consul will never successfully elect a leader and your Gateway will never reconcile and report a status.
