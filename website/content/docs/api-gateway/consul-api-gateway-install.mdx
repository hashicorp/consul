...
If you wish to create a route for a `backendRef` in a different namespace, you will need to additionally
...
The following example creates a route named `example-route` in namespace `gateway-namespace` with a `backendRef` in namespace `service-namespace`. Traffic is allowed because of the `ReferencePolicy` named `reference-policy` in namespace `service-namespace` which allows traffic from `HTTPRoutes` in `gateway-namespace` to `Services` in `service-namespace`.
