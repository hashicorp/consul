...
| [`ReferencePolicy`](https://gateway-api.sigs.k8s.io/v1alpha2/references/spec/#gateway.networking.k8s.io/v1alpha2.ReferencePolicy) | <ul><li>Required for allowing any reference from an `HTTPRoute` or `TCPRoute` to a Kubernetes `core/v1/Service` in a different namespace.</li><li>A route which has an unpermitted BackendRef due to the lack of a ReferencePolicy will set a `ResolvedRefs` status to `False` with the reason `RefNotPermitted`, and will be rejected by the gateway listener.</li><li>WARNING: If a route BackendRef becomes unpermitted (due to deleting a ReferencePolicy or adding a new unpermitted BackendRef to an existing route) this will cause the entire route to be removed from the gateway listener.</li></ul> |
