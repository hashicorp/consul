...
**gRPCs API**. By default, Consul uses gRPC over TLS to expose the xDS API to Envoy proxies.  We recommend using port `8503` for gRPC because it allows some tools to work automatically. Refer to [`grpc_tls_port`](/consul/docs/agent/config/config-files#grpc_tls_port) for additional information.
