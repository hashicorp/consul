...
## Service Mesh Certificate Signing CPU Limits ((#connect-certificate-signing-cpu-limits))

If you enable Consul's [service mesh capabilities](/docs/connect),the leader server will need
...
instances are in the cluster's service mesh. `csr_max_per_second` is likely best
