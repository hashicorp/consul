...
// AgentBootstrapGossipKey is used when delivering the gossip key for an agent.
type AgentBootstrapGossipKey struct {
	SecretKey string
}

// MetricsInfo is used to store different types of metric values from the agent.
...
// BootstrapGossipKey is used to deliver the gossip encryption key to the local
// agent which has encrypt_key_from_api and disable_keyring_file both true. This
// allows externally managing the key so it's never on disk, but requires that the
// external manager can deliver the correct key which is the current primary key
// used in the cluster. Key is expected to be exactly 16 or 32 bytes encoded in
// base64 as generated by consul keygen.
func (a *Agent) BootstrapGossipKey(key string, q *WriteOptions) error {
	r := a.c.newRequest("PUT", "/v1/agent/bootstrap-gossip-key")
	r.setWriteOptions(q)
	r.obj = &AgentBootstrapGossipKey{
		SecretKey: key,
	}
	_, resp, err := requireOK(a.c.doRequest(r))
	if err != nil {
