function tag_release {
  ...
  elif is_set "$RELEASE_UNSIGNED"; then
    (
  ...
  elif have_gpg_key ${gpg_key}; then
    (
...
function package_binaries {
  ...
  for platform in $(find "${sdir}" -mindepth 1 -maxdepth 1 -type d); do
...
function check_release_one {
  ...
  pushd "$1" >/dev/null
  for actual_fname in $(ls); do
    local found=0
    for i in "${!expected_files[@]}"; do
      local expected_fname="${expected_files[i]}"
      if test "${expected_fname}" == "${actual_fname}"; then
  ...
  if test $ret -eq 0; then
    if ! shasum -c -s "${CONSUL_PKG_NAME}_${2}_SHA256SUMS"; then
  ...
  if test $ret -eq 0 && is_set "${3}"; then
    if ! gpg --verify "${CONSUL_PKG_NAME}_${2}_SHA256SUMS.sig" "${CONSUL_PKG_NAME}_${2}_SHA256SUMS" >/dev/null 2>&1; then
...
function build_release {
  ...
  if ! is_set "${RELEASE_UNSIGNED}"; then
    if ! have_gpg_key "${gpg_key}"; then
      err "ERROR: Aborting build because no useable GPG key is present. Set RELEASE_UNSIGNED=1 to bypass this check"
      return 1
    fi
  fi

  if ! is_git_clean "${sdir}" true && ! is_set "${ALLOW_DIRTY_GIT}"; then
  ...
  if test -z "${set_vers}"; then
    set_vers=$(get_version "${sdir}" false false)
    set_release=$(parse_version "${sdir}" true false true)
  fi

  if is_set "${do_tag}" && ! set_release_mode "${sdir}" "${set_vers}" "${set_date}" "${set_release}"; then
  ...
  if is_set "${do_build}"; then
    ...
    if is_set "${do_tag}"; then
      git add "${sdir}/agent/uiserver/dist"
      if test $? -ne 0; then
        err "ERROR: Failed to git add the uiserver/dist directory"
        return 1
      fi
    fi
  fi
