function supported_osarch {
  ...
  #   * - not supported
  local osarch="$1"
  for valid in $(go tool dist list); do
    if test "${osarch}" = "${valid}"; then
      return 0
    fi
  done
  return 1
...
function build_ui {
  ...
  if test ${ret} -eq 0; then
    ...
    if test "${logo_type}" != "${ui_logo_type}"; then
...
function build_consul {
  ...
  if is_set "${CONSUL_DEV}"; then
    if test -z "${XC_OS}"; then
      XC_OS=$(go env GOOS)
    fi

    if test -z "${XC_ARCH}"; then
  ...
  local use_modules=0
  if go help modules >/dev/null 2>&1; then
    use_modules=1
  elif test -n "${GO111MODULE}"; then
    use_modules=1
  fi

  local volume_mount=
  if is_set "${use_modules}"; then
