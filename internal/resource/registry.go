...
// Scope returns the tenancy scope of a resource.
func (r *Registration) Scope() pbresource.Scope {
	return GetScope(r.Proto)
}
...
func (r *TypeRegistry) Register(registration Registration) {
	...
	if registration.Scope() == pbresource.Scope_UNKNOWN && !isUndefinedScopeAllowed(typ) {
		panic(fmt.Sprintf("Scope annotation required for %s protobuf message definition. Got: %q", registration.Proto.ProtoReflect().Descriptor().Name(), registration.Scope()))
	}
...
func GetScope(message proto.Message) pbresource.Scope {
	m := message.ProtoReflect()
	desc := m.Descriptor()
	options := desc.Options()
	if !proto.HasExtension(options, pbresource.E_Scope) {
		return pbresource.Scope_UNKNOWN
	}
	scope := proto.GetExtension(options, pbresource.E_Scope).(pbresource.Scope)
	return scope
}
