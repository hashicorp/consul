...
var (
	...
	undefinedScopeAllowed = map[string]bool{
		storage.UnversionedTypeFrom(pbinternalresource.TombstoneType).String(): true,
	}
)
...
func (r *TypeRegistry) Register(request RegisterRequest) {
	registration := Registration{
		Type:  request.Proto.GetResourceType(),
		Scope: request.Proto.GetResourceScope(),
		Proto: request.Proto,
		ACLs: &ACLHooks{
			Read:  defaultACLReadHook,
			Write: defaultACLWriteHook,
			List:  defaultACLListHook,
		},
	...
	if request.ACLs != nil {
		if request.ACLs.Read != nil {
			registration.ACLs.Read = request.ACLs.Read
		}
		if request.ACLs.Write != nil {
			registration.ACLs.Write = request.ACLs.Write
		}
		if request.ACLs.List != nil {
			registration.ACLs.List = request.ACLs.List
		}
	}

	// default validation to a no-op
	if request.Validate != nil {
		registration.Validate = request.Validate
	}

	// default mutate to a no-op
	if request.Mutate != nil {
		registration.Mutate = request.Mutate
	}
...
func defaultACLWriteHook(authz acl.Authorizer, authzContext *acl.AuthorizerContext, id *pbresource.Resource) error {
	return authz.ToAllowAuthorizer().OperatorWriteAllowed(authzContext)
}

func defaultACLListHook(authz acl.Authorizer, authzContext *acl.AuthorizerContext) error {
	return authz.ToAllowAuthorizer().OperatorReadAllowed(&acl.AuthorizerContext{})
}

func defaultValidateHook(resource *pbresource.Resource) error {
	return nil
}

func defaultMutateHook(resource *pbresource.Resource) error {
	return nil
}
