...
func TestMutateHTTPRoute(t *testing.T) {
	...
	run := func(t *testing.T, tc testcase) {
		res := resourcetest.Resource(HTTPRouteType, "api", registry).
			WithData(t, tc.route).
			Build()
			...
			route: &pbmesh.HTTPRoute{
				...
				Rules: []*pbmesh.HTTPRouteRule{{
					BackendRefs: []*pbmesh.HTTPBackendRef{{
						BackendRef: newBackendRef(catalog.ServiceType, "api", "", registry),
					}},
				}},
			},
		},
...
func getXRouteParentRefTestCases(registry resource.Registry) map[string]xRouteParentRefTestcase {
	return map[string]xRouteParentRefTestcase{
		...
		"parent ref with nil ref": {
			refs: []*pbmesh.ParentReference{
				newParentRef(catalog.ServiceType, "api", "", registry),
				{
					Ref:  nil,
					Port: "http",
				},
		...
		"parent ref with bad type ref": {
			refs: []*pbmesh.ParentReference{
				newParentRef(catalog.ServiceType, "api", "", registry),
				newParentRef(catalog.WorkloadType, "api", "", registry),
			},
		...
		"parent ref with section": {
			refs: []*pbmesh.ParentReference{
				newParentRef(catalog.ServiceType, "api", "", registry),
				{
					Ref:  resourcetest.Resource(catalog.ServiceType, "web", registry).Reference("section2"),
					Port: "http",
				},
			},
		...
		"duplicate exact parents": {
			refs: []*pbmesh.ParentReference{
				newParentRef(catalog.ServiceType, "api", "http", registry),
				newParentRef(catalog.ServiceType, "api", "http", registry),
			},
...
func getXRouteBackendRefTestCases(registry resource.Registry) map[string]xRouteBackendRefTestcase {
	return map[string]xRouteBackendRefTestcase{
		...
		"backend ref with nil ref": {
			refs: []*pbmesh.BackendReference{
				newBackendRef(catalog.ServiceType, "api", "", registry),
				{
					Ref:  nil,
					Port: "http",
				},
		...
		"backend ref with bad type ref": {
			refs: []*pbmesh.BackendReference{
				newBackendRef(catalog.ServiceType, "api", "", registry),
				newBackendRef(catalog.WorkloadType, "api", "", registry),
			},
		...
		"backend ref with datacenter": {
			refs: []*pbmesh.BackendReference{
				newBackendRef(catalog.ServiceType, "api", "", registry),
				{
					Ref:        newRef(catalog.ServiceType, "db", registry),
					Port:       "http",
					Datacenter: "dc2",
				},
			},
...
func newBackendRef(typ *pbresource.Type, name, port string, registry resource.Registry) *pbmesh.BackendReference {
...
func newParentRef(typ *pbresource.Type, name, port string, registry resource.Registry) *pbmesh.ParentReference {
