...
func TestWrite_Update(t *testing.T) {
	...
	destination2.ServiceRef = resourcetest.Resource(catalogapi.ServiceType, "test-service-2").ReferenceNoSection()
	...
	anotherProxyID := resourcetest.Resource(meshapi.ProxyStateTemplateType, "service-workload-def").ID()
	destination3 := testDestination(anotherProxyID)
	destination3.ServiceRef = resourcetest.Resource(catalogapi.ServiceType, "test-service-3").
...
func TestWrite_Delete(t *testing.T) {
	...
	// Add another destination for the same proxy ID.
	destination2 := testDestination(proxyID)
	destination2.ServiceRef = resourcetest.Resource(catalogapi.ServiceType, "test-service-2").
		WithTenancy(resource.DefaultNamespacedTenancy()).ReferenceNoSection()
	cache.WriteDestination(destination2)
	...
	cache.DeleteDestination(
		resourcetest.Resource(catalogapi.ServiceType, "does-not-exist").
			WithTenancy(resource.DefaultNamespacedTenancy()).ReferenceNoSection(),
		"doesn't-matter")
...
func TestReadDestinationsByServiceAllPorts(t *testing.T) {
	...
	destination3.ServiceRef = resourcetest.Resource(catalogapi.ServiceType, "other-service").
		WithTenancy(resource.DefaultNamespacedTenancy()).ReferenceNoSection()
	cache.WriteDestination(destination3)
...
func testDestination(proxyID *pbresource.ID) intermediate.CombinedDestinationRef {
	return intermediate.CombinedDestinationRef{
		ServiceRef: resourcetest.Resource(catalogapi.ServiceType, "test-service").
			WithTenancy(resource.DefaultNamespacedTenancy()).ReferenceNoSection(),
		Port: "tcp",
		ExplicitDestinationsID: resourcetest.Resource(meshapi.UpstreamsType, "test-servicedestinations").
			WithTenancy(resource.DefaultNamespacedTenancy()).ID(),
