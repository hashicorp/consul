...
func (suite *nodeHealthControllerTestSuite) SetupTest() {
	...
	suite.nodeNoHealth = resourcetest.Resource(catalogapi.NodeType, "test-node-no-health").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodePassing = resourcetest.Resource(catalogapi.NodeType, "test-node-passing").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeWarning = resourcetest.Resource(catalogapi.NodeType, "test-node-warning").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeCritical = resourcetest.Resource(catalogapi.NodeType, "test-node-critical").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeMaintenance = resourcetest.Resource(catalogapi.NodeType, "test-node-maintenance").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id
	...
	for _, node := range []*pbresource.ID{suite.nodePassing, suite.nodeWarning, suite.nodeCritical, suite.nodeMaintenance} {
		for idx, health := range precedenceHealth {
			if nodeHealthDesiredStatus[node.Name] >= health {
				resourcetest.Resource(catalogapi.HealthStatusType, fmt.Sprintf("test-check-%s-%d", node.Name, idx)).
	...
	resourcetest.Resource(catalogapi.DNSPolicyType, "test-policy").
...
func (suite *nodeHealthControllerTestSuite) TestGetNodeHealthNoNode() {
	...
	ref := resourceID(catalogapi.NodeType, "foo")
...
func (suite *nodeHealthControllerTestSuite) TestReconcileNodeNotFound() {
	...
	err := suite.ctl.Reconcile(context.Background(), suite.runtime, controller.Request{
		ID: resourceID(catalogapi.NodeType, "not-found"),
	})
	require.NoError(suite.T(), err)
}
...
func (suite *nodeHealthControllerTestSuite) TestController() {
	...
	resourcetest.Resource(catalogapi.NodeType, suite.nodePassing.Name).
	...
	resourcetest.Resource(catalogapi.HealthStatusType, "failure").
