...
func WorkloadHealthController() *controller.Controller {
	nodeIndexer := indexers.DecodedSingleIndexer[*pbcatalog.Workload](
		"node",
		index.ReferenceOrIDFromArgs,
		nodeIndexer,
	)

	return controller.NewController(StatusKey, pbcatalog.WorkloadType, nodeIndexer).
		WithWatch(pbcatalog.HealthStatusType, dependency.MapOwnerFiltered(pbcatalog.WorkloadType)).
		WithWatch(pbcatalog.NodeType, dependency.CacheListMapper(pbcatalog.WorkloadType, nodeIndexer.Name())).
...
func (r *workloadHealthReconciler) Reconcile(ctx context.Context, rt controller.Runtime, req controller.Request) error {
	...
	if resource.EqualStatus(workload.Status[StatusKey], newStatus, false) {
