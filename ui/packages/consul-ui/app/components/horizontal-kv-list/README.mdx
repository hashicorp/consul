...
`<ConsulCopyButtton />` components are commonly added to the value, and can be added
...
<figure>
  ...
  <dl>
    ...
    {{#if true}}
      <dt {{tooltip}} class='lock-delay'>
        Lock Delay
      </dt>
      <dd>10ms</dd>
    {{/if}}
    <dt {{tooltip}} class='ttl'>
      TTL
    </dt>
    <dd>
      <ConsulCopyButtton @value={{'1m30s10ms'}} @name='TTL' />
      1m30s10ms
    </dd>
    <dt>
      Invisible title
...
<hr />
<figure>
  <figcaption>...or they can all be specified as individual lists (for if you have a component that
    is already wrapped in a dl, such as our TagList)</figcaption>
...
<figure class='reversed'>
  <figcaption>When reversing, you'll probably want to use indiviudual dl's so as to not reverse the
    order of the KV pairs</figcaption>
  <dl>
    <dt class='kind-terminating-gateway'>Kind</dt>
    <dd>
      Terminating Gateway
...
<figure>
  <figcaption>
    Conditionally showing multiple key/values sometimes means that you insert an empty `dl` into the
    DOM
  </figcaption>
