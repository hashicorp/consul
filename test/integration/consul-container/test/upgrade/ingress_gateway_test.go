	...
	tests := func(t *testing.T) {
		...
		t.Run("fortio name", func(t *testing.T) {
			...
			libassert.AssertFortioNameWithClient(t,
				fmt.Sprintf("https://localhost:%d/%s", int(portRouterMapped.Num()), nameS1), nameS1, reqHost, httpClient)
			libassert.AssertFortioNameWithClient(t,
				fmt.Sprintf("https://localhost:%d/%s", int(portRouterMapped.Num()), nameS2), nameS2, reqHost, httpClient)
		})
