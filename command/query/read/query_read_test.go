...
func TestQueryRead_noTabs(t *testing.T) {
	t.Parallel()
...
func TestQueryRead(t *testing.T) {
	...
	var result []api.PreparedQueryDefinition
	output := ui.OutputWriter.Bytes()
	require.NoError(t, json.Unmarshal(output, &result))
	require.Len(t, result, 1)
	require.Equal(t, "web", result[0].Name)
}

func TestQueryRead_InvalidArgs(t *testing.T) {
	t.Parallel()
