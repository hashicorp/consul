...
func TestConfigRead_Warning(t *testing.T) {
	t.Parallel()
	...
	require.Contains(t, ui.ErrorWriter.String(), "WARNING: Found MutualTLSMode=permissive.")
	...
	require.Equal(t, api.ServiceDefaults, svc.Kind)
	require.Equal(t, "web", svc.Name)
	require.Equal(t, api.MutualTLSModePermissive, svc.MutualTLSMode)
}
