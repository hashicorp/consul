...
type Registry func() (string, Factory)

// Register adds a new CLI sub-command to the registry.
func Register(name string, fn Factory) Registry {
	return func() (string, Factory) {
		return name, fn
	}
}

func createRegistry(ui cli.Ui, cmdRegistries ...Registry) map[string]mcli.CommandFactory {
	m := make(map[string]mcli.CommandFactory)
	for _, fn := range cmdRegistries {
		name, thisFn := fn()
