...
func TestGenerateConfig(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			GRPCPort: 9999,
			Files: map[string]string{
				"ca.crt":  readTestData(t, "test-static-root.golden"),
				"tls.crt": readTestData(t, "test-static-cert.golden"),
				"tls.key": readTestData(t, "test-static-key.golden"),
			},
			Flags: []string{
				"-proxy-id",
				"test-proxy",
			...
			WantArgs: BootstrapTplArgs{
				...
				AgentTLS:              true,
				AgentTLSCertFile:      "@@TEMPDIR@@tls.crt",
				AgentTLSKeyFile:       "@@TEMPDIR@@tls.key",
				AgentTLSServerName:    "foobar.service.consul",
				AdminAccessLogPath:    "/dev/null",
				AdminBindAddress:      "127.0.0.1",
				AdminBindPort:         "19000",
				LocalAgentClusterName: xds.LocalAgentClusterName,
